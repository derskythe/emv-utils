...
static const uint8_t basic_atr_t1_test[] = {0x3B, 0xE0, 0x00, 0xFF, 0x81,
                                            0x31, 0x7C, 0x41, 0x92};
static const uint8_t non_emv_atr[] = {0x3B, 0xDA, 0x18, 0xFF, 0x81, 0xB1, 0xFE,
                                      0x75, 0x1F, 0x03, 0x00, 0x31, 0xF5, 0x73,
                                      0xC7, 0x8A, 0x40, 0x00, 0x90, 0x00, 0xB0};
static const uint8_t complex_atr_t0_test[] = {
    0x3B, 0xF4, 0x13, 0x00, 0xFF, 0xD0, 0x00, 0x0A,
    0x3F, 0x03, 0x00, 0xDE, 0xAD, 0xBE, 0xEF, 0xDC};
static const uint8_t complex_atr_t1_test[] = {
    0x3B, 0xF4, 0x13, 0x00, 0xFF, 0x91, 0x01, 0x71,
    0x7C, 0x41, 0x00, 0xDE, 0xAD, 0xBE, 0xEF, 0xE6};
...
int main(void) {
  ...
  atr_t0_test[5] =
      (atr_t0_test[5] & 0xF0) + 0x02; // TD1 protocol type must be T=0 or T=1
  atr_t0_test[sizeof(atr_t0_test) - 1] ^= atr_t0_test[5];
  r = emv_atr_parse(atr_t0_test, sizeof(atr_t0_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t0_test[6] =
      (atr_t0_test[6] & 0xF0) +
      0x01; // TA2 protocol must be the same as the first indicated protocol
  atr_t0_test[sizeof(atr_t0_test) - 1] ^= atr_t0_test[6];
  r = emv_atr_parse(atr_t0_test, sizeof(atr_t0_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t0_test[8] =
      (atr_t0_test[8] & 0xF0) +
      0x00; // TD2 protocol type must be T=15 if TD1 protocol type was T=0
  atr_t0_test[sizeof(atr_t0_test) - 1] ^= atr_t0_test[8];
  r = emv_atr_parse(atr_t0_test, sizeof(atr_t0_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t0_test[9] =
      (atr_t0_test[9] & 0xC0) +
      0x08; // Class indicator Y of TA3 for T=15 must be in the range 1 to 7
  atr_t0_test[sizeof(atr_t0_test) - 1] ^= atr_t0_test[9];
  r = emv_atr_parse(atr_t0_test, sizeof(atr_t0_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t1_test[5] =
      (atr_t1_test[5] & 0xF0) + 0x02; // TD1 protocol type must be T=0 or T=1
  atr_t1_test[sizeof(atr_t1_test) - 1] ^= atr_t1_test[5];
  r = emv_atr_parse(atr_t1_test, sizeof(atr_t1_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t1_test[6] =
      (atr_t1_test[6] & 0xF0) +
      0x00; // TA2 protocol must be the same as the first indicated protocol
  atr_t1_test[sizeof(atr_t1_test) - 1] ^= atr_t1_test[6];
  r = emv_atr_parse(atr_t1_test, sizeof(atr_t1_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t1_test[7] =
      (atr_t1_test[7] & 0xF0) +
      0x00; // TD2 protocol type must be T=1 if TD1 protocol type was T=1
  atr_t1_test[sizeof(atr_t1_test) - 1] ^= atr_t1_test[7];
  r = emv_atr_parse(atr_t1_test, sizeof(atr_t1_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
  ...
  atr_t1_test[9] =
      (atr_t1_test[9] & 0xF0) + 0x06; // TB3 for T=1 CWI must be 5 or less
  atr_t1_test[sizeof(atr_t1_test) - 1] ^= atr_t1_test[9];
  r = emv_atr_parse(atr_t1_test, sizeof(atr_t1_test));
  if (r == 0) {
    fprintf(stderr, "emv_atr_parse() succeeded for invalid ATR\n");
