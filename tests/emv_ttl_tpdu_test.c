...
static const struct xpdu_t test_tpdu_case_1_normal[] = {
    {
        5,
        (uint8_t[]){0x12, 0x34, 0x56, 0x78, 0x00},
        2,
        (uint8_t[]){0x90, 0x00},
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_1_error[] = {
    {
        5, (uint8_t[]){0x12, 0x34, 0x56, 0x78, 0x00}, 2,
        (uint8_t[]){0x6A, 0x81}, // Function not supported
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_2_normal[] = {
    {
        5,
        (uint8_t[]){0x00, 0xB2, 0x01, 0x0C, 0x00}, // READ RECORD 1,1
        2,
        (uint8_t[]){0x6C, 0x1C},
    },
    {
        5,
        (uint8_t[]){0x00, 0xB2, 0x01, 0x0C, 0x1C},
        0x1F,
        (uint8_t[]){0xB2, 0x70, 0x1A, 0x61, 0x18, 0x4F, 0x07, 0xA0,
                    0x00, 0x00, 0x00, 0x03, 0x10, 0x10, 0x50, 0x0A,
                    0x56, 0x49, 0x53, 0x41, 0x20, 0x44, 0x45, 0x42,
                    0x49, 0x54, 0x87, 0x01, 0x01, 0x90, 0x00},
    },
    {0}};
static const uint8_t test_tpdu_case_2_normal_data[] = {
    0x70, 0x1A, 0x61, 0x18, 0x4F, 0x07, 0xA0, 0x00, 0x00, 0x00,
    0x03, 0x10, 0x10, 0x50, 0x0A, 0x56, 0x49, 0x53, 0x41, 0x20,
    0x44, 0x45, 0x42, 0x49, 0x54, 0x87, 0x01, 0x01,
};
...
static const struct xpdu_t test_tpdu_case_2_error_late[] = {
    ...
    {
        5, (uint8_t[]){0x00, 0xB2, 0x01, 0x0C, 0x1C}, 2,
        (uint8_t[]){0x65, 0x81}, // Memory failure
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_3_normal[] = {
    {
        5,
        (uint8_t[]){0x00, 0x82, 0x00, 0x00, 0x04}, // EXTERNAL AUTHENTICATE
        1,
        (uint8_t[]){0x82},
    },
    {
        4,
        (uint8_t[]){0xde, 0xad, 0xbe, 0xef}, // deadbeef
        2,
        (uint8_t[]){0x90, 0x00},
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_4_normal[] = {
    {
        5,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x0E}, // SELECT
        1,
        (uint8_t[]){0xA4},
    },
    {
        14,
        (uint8_t[]){0x31, 0x50, 0x41, 0x59, 0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44,
                    0x44, 0x46, 0x30, 0x31},
        2,
        (uint8_t[]){0x61, 0x26},
    },
    {
        5,
        (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x26},
        0x29,
        (uint8_t[]){0xC0, 0x6F, 0x24, 0x84, 0x0E, 0x31, 0x50, 0x41, 0x59,
                    0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44, 0x44, 0x46, 0x30,
                    0x31, 0xA5, 0x12, 0x88, 0x01, 0x01, 0x5F, 0x2D, 0x08,
                    0x65, 0x6E, 0x65, 0x73, 0x66, 0x72, 0x64, 0x65, 0x9F,
                    0x11, 0x01, 0x01, 0x90, 0x00},
    },
    {0}};
static const uint8_t test_tpdu_case_4_normal_data[] = {
    0x6F, 0x24, 0x84, 0x0E, 0x31, 0x50, 0x41, 0x59, 0x2E, 0x53,
    0x59, 0x53, 0x2E, 0x44, 0x44, 0x46, 0x30, 0x31, 0xA5, 0x12,
    0x88, 0x01, 0x01, 0x5F, 0x2D, 0x08, 0x65, 0x6E, 0x65, 0x73,
    0x66, 0x72, 0x64, 0x65, 0x9F, 0x11, 0x01, 0x01,
};
...
static const struct xpdu_t test_tpdu_case_4_error_late[] = {
    ...
    {
        5, (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x26}, 2,
        (uint8_t[]){0x65, 0x81}, // Memory failure
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_2_normal_advanced[] = {
    ...
    {
        // This deviates slightly from Annex A5 because the TTL has no reason to
        // respond with a smaller Le The ICC can nonetheless force a partial
        // response and indicate the number of remaining bytes
        5,
        (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x1C},
        0x10,
        (uint8_t[]){0xC0, 0x70, 0x1A, 0x61, 0x18, 0x4F, 0x07, 0xA0, 0x00, 0x00,
                    0x00, 0x03, 0x10, 0x10, 0x61, 0x0F},
    },
    {
        5,
        (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x0F},
        0x12,
        (uint8_t[]){0xC0, 0x50, 0x0A, 0x56, 0x49, 0x53, 0x41, 0x20, 0x44, 0x45,
                    0x42, 0x49, 0x54, 0x87, 0x01, 0x01, 0x90, 0x00},
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_4_normal_advanced[] = {
    ...
    },
    {
        ...
        (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x26},
        0x12,
        (uint8_t[]){0xC0, 0x6F, 0x24, 0x84, 0x0E, 0x31, 0x50, 0x41, 0x59, 0x2E,
                    0x53, 0x59, 0x53, 0x2E, 0x44, 0x44, 0x61, 0x14},
    },
    {
        5,
        (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x14},
        0x1A,
        (uint8_t[]){0xC0, 0x46, 0x30, 0x31, 0xA5, 0x12, 0x88, 0x01, 0x01,
                    0x5F, 0x2D, 0x08, 0x65, 0x6E, 0x65, 0x73, 0x66, 0x72,
                    0x64, 0x65, 0x9F, 0x11, 0x01, 0x01, 0x90, 0x00},
    },
    {0}};
...
static const struct xpdu_t test_tpdu_case_4_warning1[] = {
    ...
    },
    {
        ...
        (uint8_t[]){0x31, 0x50, 0x41, 0x59, 0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44,
                    0x44, 0x46, 0x30, 0x31},
        2,
        (uint8_t[]){0x62, 0x86}, // No input available from a sensor on the card
    },
    {
        5,
        (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x00},
        2,
        (uint8_t[]){0x6C, 0x26},
    },
    {
...
static const struct xpdu_t test_tpdu_case_4_warning2[] = {
    ...
    {
        5, (uint8_t[]){0x00, 0xC0, 0x00, 0x00, 0x26}, 0x29,
        (uint8_t[]){0xC0, 0x6F, 0x24, 0x84, 0x0E, 0x31, 0x50, 0x41, 0x59,
                    0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44, 0x44, 0x46, 0x30,
                    0x31, 0xA5, 0x12, 0x88, 0x01, 0x01, 0x5F, 0x2D, 0x08,
                    0x65, 0x6E, 0x65, 0x73, 0x66, 0x72, 0x64, 0x65, 0x9F,
                    0x11, 0x01, 0x01, 0x62, 0x86}, // No input available from a
                                                   // sensor on the card
    },
    {0}};
...
int main(void) {
  ...
  r = emv_ttl_trx(&ttl, (uint8_t[]){0x12, 0x34, 0x56, 0x78}, 4, r_apdu,
                  &r_apdu_len, &sw1sw2);
  if (r) {
  ...
  r = emv_ttl_read_record(&ttl, 1, 1, data, &data_len, &sw1sw2);
  if (r) {
    fprintf(stderr, "emv_ttl_trx() failed; r=%d\n", r);
    return 1;
  }
  ...
  r = emv_ttl_trx(&ttl,
                  (uint8_t[]){0x00, 0x82, 0x00, 0x00, 0x04, 0xde, 0xad, 0xbe,
                              0xef}, // EXTERNAL AUTHENTICATE
                  9, r_apdu, &r_apdu_len, &sw1sw2);
  if (r) {
    fprintf(stderr, "emv_ttl_trx() failed; r=%d\n", r);
    return 1;
  }
  ...
  r = emv_ttl_select_by_df_name(&ttl, PSE, sizeof(PSE) - 1, data, &data_len,
                                &sw1sw2);
  if (r) {
    fprintf(stderr, "emv_ttl_trx() failed; r=%d\n", r);
    return 1;
  ...
  printf("\nTesting APDU case 4 (TPDU mode); error processing (early 2nd "
         "exchange)...\n");
  ...
  // Test APDU case 2; normal processing (using both '61' and '6C' procedure
  // bytes)
  printf("\nTesting APDU case 2 (TPDU mode); normal processing (using both "
         "'61' and '6C' procedure bytes)...\n");
  ...
  printf("\nTesting APDU case 4 (TPDU mode); normal processing (using multiple "
         "'61' procedure bytes)...\n");
  ...
  printf("\nTesting APDU case 4 (TPDU mode); warning processing ('62' then "
         "'6C')...\n");
  ...
  printf("\nTesting APDU case 4 (TPDU mode); warning processing ('61' then "
         "'62')...\n");
