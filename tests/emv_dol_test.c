...
const uint8_t test1_dol[] = {0x9F, 0x02, 0x06, 0x9F, 0x03, 0x06, 0x9F,
                             0x1A, 0x02, 0x95, 0x05, 0x5F, 0x2A, 0x02,
...
const uint8_t test2_dol[] = {0x9F, 0x02, 0x06, 0x9F, 0x03,
                             0x06, 0x9F, 0x1A, 0x02, 0x95};
...
const uint8_t test3_dol[] = {0x9F, 0x02, 0x06, 0x9F, 0x03, 0x06, 0x9F,
                             0x1A, 0x02, 0x95, 0x05, 0x5F, 0x2A, 0x02,
                             0x9A, 0x03, 0x9C, 0x01, 0x9F, 0x37};
const struct emv_dol_entry_t test3_dol_entries[] = {
    {0x9F02, 6}, {0x9F03, 6}, {0x9F1A, 2}, {0x95, 5},
    {0x5F2A, 2}, {0x9A, 3},   {0x9C, 1},
};

const uint8_t test4_dol[] = {0x9F, 0x02, 0x06, 0x9F, 0x03, 0x06, 0x9F,
                             0x1A, 0x02, 0x95, 0x05, 0x5F, 0x2A, 0x02,
                             0x9A, 0x03, 0x9C, 0x01, 0x9F, 0x37, 0x04};
const int test4_data_len = 6 + 6 + 2 + 5 + 2 + 3 + 1 + 4;

const uint8_t test5_dol[] = {0x9F, 0x02, 0x06, 0x9F, 0x03, 0x06, 0x9F,
                             0x1A, 0x02, 0x95, 0x05, 0x5F, 0x2A, 0x02,
                             0x9A, 0x03, 0x9C, 0x01, 0x9F, 0x37};

const uint8_t test6_dol[] = {0x9F, 0x02, 0x06, 0x9F, 0x03, 0x06, 0x9F,
                             0x1A, 0x02, 0x95, 0x05, 0x5F, 0x2A, 0x02,
                             0x9A, 0x03, 0x9C, 0x01, 0x9F, 0x37, 0x04};
...
const uint8_t test6_data[] = {
    0x00, 0x01, 0x23, 0x45, 0x67, 0x89, 0x00, 0x09, 0x87, 0x65,
    0x43, 0x21, 0x05, 0x28, 0x12, 0x34, 0x55, 0x43, 0x21, 0x09,
    0x78, 0x24, 0x02, 0x17, 0x09, 0xDE, 0xAD, 0xBE, 0xEF,
};

const uint8_t test7_dol[] = {
    0x9F, 0x02, 0x03,             // Shorter than original data length
    0x9F, 0x03, 0x07,             // Longer than original length
    0x9F, 0x1A, 0x02, 0x95, 0x06, // Longer than original length
    0x5F, 0x2A, 0x02, 0x9A, 0x00, // Zero length
...
const uint8_t test7_data[] = {
    ...
    0x05, 0x28, 0x12, 0x34, 0x55, 0x43, 0x21,
    0x00, // Longer than original length
    0x09, 0x78,
...
int main(void) {
  ...
  for (size_t i = 0;
       i < sizeof(test1_dol_entries) / sizeof(test1_dol_entries[0]); ++i) {
  ...
  if (r != 0) {
    fprintf(
        stderr,
        "emv_dol_itr_next() unexpectedly did not report end-of-data; r=%d\n",
        r);
    return 1;
  }
  printf("Success\n");
  ...
  for (size_t i = 0;
       i < sizeof(test2_dol_entries) / sizeof(test2_dol_entries[0]); ++i) {
    r = emv_dol_itr_next(&itr, &entry);
    if (r <= 0) {
      fprintf(stderr, "emv_dol_itr_next() failed; r=%d\n", r);
      return 1;
  ...
  if (r >= 0) {
    fprintf(stderr,
            "emv_dol_itr_next() unexpectedly did not report error; r=%d\n", r);
    return 1;
  }
  printf("Success\n");
  ...
  for (size_t i = 0;
       i < sizeof(test3_dol_entries) / sizeof(test3_dol_entries[0]); ++i) {
    r = emv_dol_itr_next(&itr, &entry);
    if (r <= 0) {
      fprintf(stderr, "emv_dol_itr_next() failed; r=%d\n", r);
      return 1;
  ...
  if (r >= 0) {
    fprintf(stderr,
            "emv_dol_compute_data_length() unexpectedly did not report error; "
            "r=%d\n",
            r);
    return 1;
  }
  ...
  r = populate_source(test6_source1,
                      sizeof(test6_source1) / sizeof(test6_source1[0]),
                      &source1);
  if (r) {
    fprintf(stderr, "populate_source() failed; r=%d\n", r);
    return 1;
  }
  r = populate_source(test6_source2,
                      sizeof(test6_source2) / sizeof(test6_source2[0]),
                      &source2);
  if (r) {
    fprintf(stderr, "populate_source() failed; r=%d\n", r);
    return 1;
  ...
  r = emv_dol_build_data(test6_dol, sizeof(test6_dol), &source1, &source2, data,
                         &data_len);
  if (r) {
  ...
  r = populate_source(test7_source1,
                      sizeof(test7_source1) / sizeof(test7_source1[0]),
                      &source1);
  if (r) {
    fprintf(stderr, "populate_source() failed; r=%d\n", r);
    return 1;
  }
  r = populate_source(test7_source2,
                      sizeof(test7_source2) / sizeof(test7_source2[0]),
                      &source2);
  if (r) {
    fprintf(stderr, "populate_source() failed; r=%d\n", r);
    return 1;
  ...
  data_len = sizeof(data);
  r = emv_dol_build_data(test7_dol, sizeof(test7_dol), &source1, &source2, data,
                         &data_len);
  if (r) {
    fprintf(stderr, "emv_dol_build_data() failed; r=%d\n", r);
    return 1;
