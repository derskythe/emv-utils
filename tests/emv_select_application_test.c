...
// Test data taken from test_sorted_app_priority[] in
// emv_build_candidate_list_test.c
...
static const struct xpdu_t test_select_app1[] = {
    {
        ...
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x01, 0x00}, // SELECT A0000000031001
...
static const struct xpdu_t test_select_app7[] = {
    {
        13,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x04,
                    0x10, 0x10, 0x00}, // SELECT A0000000041010
    ...
    },
    {
        ...
        51,
        (uint8_t[]){0x6F, 0x5D, 0x84, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x04, 0x10,
                    0x10, 0xA5, 0x52, 0x50, 0x10, 0x44, 0x45, 0x42, 0x49, 0x54,
                    0x20, 0x4D, 0x41, 0x53, 0x54, 0x45, 0x52, 0x43, 0x41, 0x52,
                    0x44, 0x9F, 0x12, 0x10, 0x44, 0x65, 0x62, 0x69, 0x74, 0x20,
                    0x4D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x43, 0x61, 0x72, 0x64,
                    0x87, 0x01, 0x01, 0x9F, 0x11, 0x01, 0x01, 0x5F, 0x2D, 0x04,
                    0x6E, 0x6C, 0x65, 0x6E, 0xBF, 0x0C, 0x1C, 0x9F, 0x5D, 0x03,
                    0x01, 0x00, 0x00, 0x9F, 0x0A, 0x04, 0x00, 0x01, 0x01, 0x01,
                    0x9F, 0x4D, 0x02, 0x0B, 0x0A, 0x9F, 0x6E, 0x07, 0x05, 0x28,
                    0x00, 0x00, 0x30, 0x30, 0x00, 0x90, 0x00}, // FCI
    },
    {0}};
...
static const struct xpdu_t test_select_app4[] = {
    {
        13,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x04, 0x00}, // SELECT A0000000031004
...
static const struct xpdu_t test_select_app3[] = {
    {
        13,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x03, 0x00}, // SELECT A0000000031003
...
static const struct xpdu_t test_select_app5[] = {
    {
        13,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x05, 0x00}, // SELECT A0000000031005
        32, (uint8_t[]){0x6F, 0x1C, 0x85, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                        0x10, 0x05, 0xA5, 0x11, 0x50, 0x05, 0x56, 0x20, 0x50,
                        0x41, 0x59, 0x87, 0x01, 0x01, 0x5F, 0x2D, 0x04, 0x6E,
                        0x6C, 0x65, 0x6E, 0x90, 0x00}, // Invalid FCI
    },
    {0}};
...
static const struct xpdu_t test_select_app2[] = {
    {
        13,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x02, 0x00}, // SELECT A0000000031002
        32, (uint8_t[]){0x6F, 0x1C, 0x84, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                        0x10, 0x05, 0xA5, 0x11, 0x50, 0x05, 0x56, 0x20, 0x50,
                        0x41, 0x59, 0x87, 0x01, 0x01, 0x5F, 0x2D, 0x04, 0x6E,
                        0x6C, 0x65, 0x6E, 0x90, 0x00}, // FCI for A0000000031005
    },
    {0}};
...
static const struct xpdu_t test_select_app6[] = {
    {
        13,
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x06, 0x00}, // SELECT A0000000031006
        32, (uint8_t[]){0x6F, 0x1C, 0x84, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                        0x10, 0x05, 0xA5, 0x11, 0x50, 0x05, 0x56, 0x20, 0x50,
                        0x41, 0x59, 0x87, 0x01, 0x01, 0x5F, 0x2D, 0x04, 0x6E,
                        0x6C, 0x65, 0x6E, 0x90, 0x00}, // FCI for A0000000031005
...
static const struct xpdu_t test_select_app_success[] = {
    {
        ...
        (uint8_t[]){0x00, 0xA4, 0x04, 0x00, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                    0x10, 0x03, 0x00}, // SELECT A0000000031003
        32, (uint8_t[]){0x6F, 0x1C, 0x84, 0x07, 0xA0, 0x00, 0x00, 0x00,
                        0x03, 0x10, 0x03, 0xA5, 0x11, 0x50, 0x05, 0x56,
                        0x20, 0x50, 0x41, 0x59, 0x87, 0x01, 0x01, 0x5F,
                        0x2D, 0x04, 0x6E, 0x6C, 0x65, 0x6E, 0x90, 0x00}, // FCI
    },
    {0}};

static bool verify_app_list_numbers(const struct emv_app_list_t *app_list,
...
int main(void) {
  ...
  if (!verify_app_list_numbers(&app_list, (unsigned int[]){1, 2, 3, 4, 5, 6, 7},
                               7)) {
  ...
  if (r != EMV_ERROR_INVALID_PARAMETER) {
    fprintf(stderr,
            "Unexpected emv_select_application() result; error %d: %s\n", r,
            r < 0 ? emv_error_get_string(r) : emv_outcome_get_string(r));
    r = 1;
    goto exit;
  }
  ...
  printf("\nTest selection of first application index and application not "
         "found...\n");
  ...
  if (!verify_app_list_numbers(&app_list, (unsigned int[]){2, 3, 4, 5, 6, 7},
                               6)) {
    fprintf(stderr, "Invalid remaining candidate application list\n");
    for (struct emv_app_t *app = app_list.front; app != NULL; app = app->next) {
      print_emv_app(app);
  ...
  printf("\nTest selection of last application index and application "
         "blocked...\n");
  ...
  printf("\nTest DF Name mismatch during application selection and expecting "
         "empty candidate application list...\n");
  ...
  r = emv_debug_init(EMV_DEBUG_SOURCE_NONE, EMV_DEBUG_NONE, NULL);
  if (r) {
    printf("Failed to initialise EMV debugging\n");
    return 1;
  }
  ...
  if (!selected_app) {
    fprintf(stderr,
            "emv_select_application() failed to populate selected_app\n");
    r = 1;
    goto exit;
  }
  ...
  if (!verify_app_list_numbers(&app_list, (unsigned int[]){1, 2, 4, 5, 6, 7},
                               6)) {
    fprintf(stderr, "Invalid remaining candidate application list\n");
    for (struct emv_app_t *app = app_list.front; app != NULL; app = app->next) {
      print_emv_app(app);
