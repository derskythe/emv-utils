...
const struct emv_tlv_t test_source1_data[] = {
    ...
    {{{EMV_TAG_9A_TRANSACTION_DATE, 3, (uint8_t[]){0x24, 0x02, 0x17}, 0}},
     NULL},
    {{{EMV_TAG_5F2A_TRANSACTION_CURRENCY_CODE, 2, (uint8_t[]){0x09, 0x78}, 0}},
     NULL},
    {{{EMV_TAG_9F02_AMOUNT_AUTHORISED_NUMERIC, 6,
       (uint8_t[]){0x00, 0x01, 0x23, 0x45, 0x67, 0x89}, 0}},
     NULL},
    {{{EMV_TAG_9F03_AMOUNT_OTHER_NUMERIC, 6,
       (uint8_t[]){0x00, 0x09, 0x87, 0x65, 0x43, 0x21}, 0}},
     NULL},
};
const struct emv_tlv_t test_source2_data[] = {
    {{{EMV_TAG_9F1A_TERMINAL_COUNTRY_CODE, 2, (uint8_t[]){0x05, 0x28}, 0}},
     NULL},
    {{{EMV_TAG_9F33_TERMINAL_CAPABILITIES, 3, (uint8_t[]){0x60, 0xF0, 0xC8},
       0}},
     NULL},
    {{{EMV_TAG_9F37_UNPREDICTABLE_NUMBER, 4,
       (uint8_t[]){0xDE, 0xAD, 0xBE, 0xEF}, 0}},
     NULL},
    {{{EMV_TAG_95_TERMINAL_VERIFICATION_RESULTS, 5,
       (uint8_t[]){0x12, 0x34, 0x55, 0x43, 0x21}, 0}},
     NULL},
};

static const uint8_t test1_fci[] = {0x6F, 0x12, 0x84, 0x07, 0xA0, 0x00, 0x00,
                                    0x00, 0x03, 0x10, 0x03, 0xA5, 0x07, 0x50,
                                    0x05, 0x44, 0x65, 0x62, 0x69, 0x74};
static const struct xpdu_t test1_apdu_list[] = {
    {
        8, (uint8_t[]){0x80, 0xA8, 0x00, 0x00, 0x02, 0x83, 0x00, 0x00}, // GPO
        18,
        (uint8_t[]){0x80, 0x0E, 0x78, 0x00, 0x08, 0x02, 0x02, 0x00, 0x10, 0x01,
                    0x04, 0x00, 0x18, 0x01, 0x02, 0x01, 0x90,
                    0x00}, // GPO response format 1
    },
    {0}};
static const uint8_t test1_aip_verify[] = {0x78, 0x00};
static const uint8_t test1_afl_verify[] = {0x08, 0x02, 0x02, 0x00, 0x10, 0x01,
                                           0x04, 0x00, 0x18, 0x01, 0x02, 0x01};

static const uint8_t test2_fci[] = {0x6F, 0x12, 0x84, 0x07, 0xA0, 0x00, 0x00,
                                    0x00, 0x03, 0x10, 0x03, 0xA5, 0x07, 0x50,
                                    0x05, 0x44, 0x65, 0x62, 0x69, 0x74};
static const struct xpdu_t test2_apdu_list[] = {
    {
        8, (uint8_t[]){0x80, 0xA8, 0x00, 0x00, 0x02, 0x83, 0x00, 0x00}, // GPO
        18,
        (uint8_t[]){0x77, 0x0E, 0x82, 0x02, 0x39, 0x00, 0x94, 0x08, 0x18, 0x01,
                    0x02, 0x01, 0x20, 0x02, 0x04, 0x00, 0x90,
                    0x00}, // GPO response format 2
    },
    {0}};
static const uint8_t test2_aip_verify[] = {0x39, 0x00};
static const uint8_t test2_afl_verify[] = {0x18, 0x01, 0x02, 0x01,
                                           0x20, 0x02, 0x04, 0x00};

static const uint8_t test3_fci[] = {0x6F, 0x18, 0x84, 0x07, 0xA0, 0x00, 0x00,
                                    0x00, 0x03, 0x10, 0x03, 0xA5, 0x0D, 0x50,
                                    0x05, 0x44, 0x65, 0x62, 0x69, 0x74, 0x9F,
                                    0x38, 0x03, 0x9F, 0x33, 0x03};
static const struct xpdu_t test3_apdu_list[] = {
    {
        11,
        (uint8_t[]){0x80, 0xA8, 0x00, 0x00, 0x05, 0x83, 0x03, 0x60, 0xF0, 0xC8,
                    0x00},          // GPO
        2, (uint8_t[]){0x69, 0x85}, // Conditions of use not satisfied
    },
    {0}};

static const uint8_t test4_fci[] = {0x6F, 0x18, 0x84, 0x07, 0xA0, 0x00, 0x00,
                                    0x00, 0x03, 0x10, 0x03, 0xA5, 0x0D, 0x50,
                                    0x05, 0x44, 0x65, 0x62, 0x69, 0x74, 0x9F,
                                    0x38, 0x03, 0x9F, 0x33, 0x03};
...
static const uint8_t test4_afl_verify[] = {0x08, 0x02, 0x02, 0x00, 0x10, 0x01,
                                           0x04, 0x00, 0x18, 0x01, 0x02, 0x01};

static const uint8_t test5_fci[] = {0x6F, 0x18, 0x84, 0x07, 0xA0, 0x00, 0x00,
                                    0x00, 0x03, 0x10, 0x03, 0xA5, 0x0D, 0x50,
                                    0x05, 0x44, 0x65, 0x62, 0x69, 0x74, 0x9F,
                                    0x38, 0x03, 0x9F, 0x33, 0xFF};
static const struct xpdu_t test5_apdu_list[] = {{0}};

static const uint8_t test6_fci[] = {0x6F, 0x18, 0x84, 0x07, 0xA0, 0x00, 0x00,
                                    0x00, 0x03, 0x10, 0x03, 0xA5, 0x0D, 0x50,
                                    0x05, 0x44, 0x65, 0x62, 0x69, 0x74, 0x9F,
                                    0x38, 0x03, 0x9F, 0x33, 0x03};
...
static int populate_source(const struct emv_tlv_t *tlv_array,
  ...
  for (size_t i = 0; i < tlv_array_count; ++i) {
    r = emv_tlv_list_push(source, tlv_array[i].tag, tlv_array[i].length,
                          tlv_array[i].value, 0);
    if (r) {
      return r;
    }
...
int main(void) {
  ...
  r = populate_source(test_source1_data,
                      sizeof(test_source1_data) / sizeof(test_source1_data[0]),
                      &source1);
  if (r) {
    fprintf(stderr, "populate_source() failed; r=%d\n", r);
    return 1;
  }
  r = populate_source(test_source2_data,
                      sizeof(test_source2_data) / sizeof(test_source2_data[0]),
                      &source2);
  if (r) {
    fprintf(stderr, "populate_source() failed; r=%d\n", r);
    return 1;
  ...
  r = emv_initiate_application_processing(&ttl, app, &source1, &source2, &icc);
  if (r) {
    fprintf(stderr,
            "emv_initiate_application_processing() failed; error %d: %s\n", r,
  ...
  aip =
      emv_tlv_list_find_const(&icc, EMV_TAG_82_APPLICATION_INTERCHANGE_PROFILE);
  if (!aip) {
  ...
  if (r != EMV_OUTCOME_GPO_NOT_ACCEPTED) {
    fprintf(stderr,
            ...
            "EMV_OUTCOME_GPO_NOT_ACCEPTED; error %d: %s\n",
