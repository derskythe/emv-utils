...
static void emv_txn_load_params(struct emv_txn_t *emv_txn, uint32_t txn_seq_cnt,
                                uint8_t txn_type, uint32_t amount,
                                uint32_t amount_other);
...
static struct argp_option argp_options[] = {
    ...
    {"txn-type", EMV_TOOL_PARAM_TXN_TYPE, "VALUE", 0,
     "Transaction type (two numeric digits, according to ISO 8583:1987 "
     "Processing Code)"},
    {"txn-amount", EMV_TOOL_PARAM_TXN_AMOUNT, "AMOUNT", 0,
     "Transaction amount (without decimal separator)"},
    {"txn-amount-other", EMV_TOOL_PARAM_TXN_AMOUNT_OTHER, "AMOUNT", 0,
     "Secondary transaction amount associated with cashback (without decimal "
     "separator)"},

    {NULL, 0, NULL, 0, "Debug options", 2},
    {"debug-verbose", EMV_TOOL_PARAM_DEBUG_VERBOSE, NULL, 0,
     "Enable verbose debug output. This will include the timestamp, debug "
     "source and debug level in the debug output."},
    {"debug-source", EMV_TOOL_PARAM_DEBUG_SOURCES_MASK, "x,y,z...", 0,
     "Comma separated list of debug sources. Allowed values are TTL, TAL, EMV, "
     "APP, ALL. Default is ALL."},
    {"debug-level", EMV_TOOL_PARAM_DEBUG_LEVEL, "LEVEL", 0,
     "Maximum debug level. Allowed values are NONE, ERROR, INFO, CARD, TRACE, "
     "ALL. Default is INFO."},
    ...
    {"mcc-json", EMV_TOOL_OVERRIDE_MCC_JSON, "path", OPTION_HIDDEN,
     "Override path of mcc-codes JSON file"},
...
static const char *debug_level_str[] = {
    "NONE", "ERROR", "INFO", "CARD", "TRACE", "ALL",
};
...
static error_t argp_parser_helper(int key, char *arg,
  ...
  case EMV_TOOL_PARAM_TXN_TYPE: {
    ...
    if (arg_len != 2) {
      argp_error(
          state,
          "Transaction type (--txn-type) argument must be 2 numeric digits");
    }
  ...
  case EMV_TOOL_PARAM_TXN_AMOUNT: {
    ...
    if (arg_len == 0) {
      argp_error(
          state,
          "Transaction amount (--txn-amount) argument must be numeric digits");
    }
    ...
    if (value > 0xFFFFFFFF) {
      argp_error(state, "Transaction amount (--txn-amount) argument must fit "
                        "in a 32-bit field");
    }
    txn_amount = value;
  ...
  case EMV_TOOL_PARAM_TXN_AMOUNT_OTHER: {
    ...
    if (arg_len == 0) {
      argp_error(state, "Secondary transaction amount (--txn-amount-other) "
                        "argument must be numeric digits");
    }
    ...
    if (value > 0xFFFFFFFF) {
      argp_error(state, "Secondary transaction amount (--txn-amount-other) "
                        "argument must fit in a 32-bit field");
    }
  ...
  case EMV_TOOL_PARAM_DEBUG_SOURCES_MASK: {
    ...
    while ((str = strtok(arg, ","))) {
      size_t i;
      for (i = 0; i < sizeof(debug_source_str) / sizeof(debug_source_str[0]);
           ++i) {
      ...
      if (i >= sizeof(debug_source_str) / sizeof(debug_source_str[0])) {
        ...
        argp_error(state,
                   "Unknown debug source (--debug-source) argument \"%s\"",
                   str);
      }
  ...
  case EMV_TOOL_PARAM_DEBUG_LEVEL: {
    for (size_t i = 0; i < sizeof(debug_level_str) / sizeof(debug_level_str[0]);
         ++i) {
    ...
    argp_error(state, "Unknown debug level (--debug-level) argument \"%s\"",
               arg);
  }
...
static void print_pcsc_readers(pcsc_ctx_t pcsc) {
  ...
  for (size_t i = 0; i < pcsc_count; ++i) {
    ...
    for (size_t j = 0; j < sizeof(feature_list) / sizeof(feature_list[0]);
         ++j) {
    ...
    unsigned int property_list[] = {
        PCSC_PROPERTY_wLcdLayout,   PCSC_PROPERTY_wLcdMaxCharacters,
        PCSC_PROPERTY_wLcdMaxLines, PCSC_PROPERTY_bMinPINSize,
        PCSC_PROPERTY_bMaxPINSize,  PCSC_PROPERTY_wIdVendor,
        PCSC_PROPERTY_wIdProduct,
    };

    // Determine whether there are any recognisable properties
    print_properties = false;
    for (size_t j = 0; j < sizeof(property_list) / sizeof(property_list[0]);
         ++j) {
      uint8_t value[256];
    ...
    if (print_properties) {
...

      {
        ...
        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_wLcdLayout, value,
                                     &value_len);
        if (r == 0 && value_len == 2) {
          if (property_comma) {
            printf(", ");
...

      {
        ...
        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_wLcdMaxCharacters,
                                     &wLcdMaxCharacters,
        ...
        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_wLcdMaxLines,
                                     &wLcdMaxLines, &wLcdMaxLines_len);
        if (r || wLcdMaxLines_len != 2) {
          // If property not found or invalid, zero property value
          wLcdMaxLines = 0;
        }

        if ((wLcdMaxCharacters && wLcdMaxLines) || wLcdMaxCharacters_len ||
            wLcdMaxLines_len) {
          if (property_comma) {
            printf(", ");
          }
...

      {
        ...
        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_bMinPINSize,
                                     &bMinPINSize, &bMinPINSize_len);
        if (r || bMinPINSize_len != 1) {
          // If property not found or invalid, zero property value
          bMinPINSize = 0;
        }

        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_bMaxPINSize,
                                     &bMaxPINSize, &bMaxPINSize_len);
...

      {
        ...
        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_wIdVendor,
                                     &wIdVendor, &wIdVendor_len);
        if (r || wIdVendor_len != 2) {
          // If property not found or invalid, zero property value
          wIdVendor = 0;
        }

        r = pcsc_reader_get_property(reader, PCSC_PROPERTY_wIdProduct,
                                     &wIdProduct, &wIdProduct_len);
...
static void emv_txn_load_params(struct emv_txn_t *emv_txn, uint32_t txn_seq_cnt,
  ...
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9F41_TRANSACTION_SEQUENCE_COUNTER,
                    4, emv_uint_to_format_n(txn_seq_cnt, buf, 4), 0);
  ...
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9A_TRANSACTION_DATE, 3, emv_date,
                    0);
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9F21_TRANSACTION_TIME, 3,
                    emv_time, 0);

  // Transaction currency
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_5F2A_TRANSACTION_CURRENCY_CODE, 2,
                    (uint8_t[]){0x09, 0x78}, 0); // Euro (978)
  emv_tlv_list_push(&emv_txn->params,
                    EMV_TAG_5F36_TRANSACTION_CURRENCY_EXPONENT, 1,
                    (uint8_t[]){0x02}, 0); // Currency has 2 decimal places

  // Transaction type and amount(s)
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9C_TRANSACTION_TYPE, 1, &txn_type,
                    0);
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9F02_AMOUNT_AUTHORISED_NUMERIC, 6,
                    emv_uint_to_format_n(amount, buf, 6), 0);
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_81_AMOUNT_AUTHORISED_BINARY, 4,
                    emv_uint_to_format_b(amount, buf, 4), 0);
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9F03_AMOUNT_OTHER_NUMERIC, 6,
                    emv_uint_to_format_n(amount_other, buf, 6), 0);
  emv_tlv_list_push(&emv_txn->params, EMV_TAG_9F04_AMOUNT_OTHER_BINARY, 4,
                    emv_uint_to_format_b(amount_other, buf, 4), 0);
}

static void emv_txn_load_config(struct emv_txn_t *emv_txn) {
  // Terminal config
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F01_ACQUIRER_IDENTIFIER, 6,
                    (uint8_t[]){0x00, 0x01, 0x23, 0x45, 0x67, 0x89},
                    0); // Unique acquirer identifier
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F1A_TERMINAL_COUNTRY_CODE, 2,
                    (uint8_t[]){0x05, 0x28}, 0); // Netherlands
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F1B_TERMINAL_FLOOR_LIMIT, 4,
                    (uint8_t[]){0x00, 0x00, 0x03, 0xE8}, 0); // 1000
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F16_MERCHANT_IDENTIFIER, 15,
                    (const uint8_t *)"0987654321     ",
                    0); // Unique merchant identifier
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F1C_TERMINAL_IDENTIFICATION, 8,
                    (const uint8_t *)"TID12345",
                    0); // Unique location of terminal at merchant
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F1E_IFD_SERIAL_NUMBER, 8,
                    (const uint8_t *)"12345678", 0); // Serial number
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F4E_MERCHANT_NAME_AND_LOCATION,
                    12, (const uint8_t *)"ACME Peanuts",
                    0); // Merchant Name and Location

  // Terminal Capabilities:
  // - Card Data Input Capability: IC with Contacts
  // - CVM Capability: Plaintext offline PIN, Enciphered online PIN, Signature,
  // Enciphered offline PIN, No CVM
  // - Security Capability: SDA, DDA, CDA
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F33_TERMINAL_CAPABILITIES, 3,
                    (uint8_t[]){0x20, 0xF8, 0xC8}, 0);

  // Merchant attended, offline with online capability
  emv_tlv_list_push(&emv_txn->terminal, EMV_TAG_9F35_TERMINAL_TYPE, 1,
                    (uint8_t[]){0x22}, 0);

  // Additional Terminal Capabilities:
  // - Transaction Type Capability: Goods, Services, Cashback, Cash, Inquiry,
  // Payment
  // - Terminal Data Input Capability: Numeric, Alphabetic and special character
  // keys, Command keys, Function keys
  // - Terminal Data Output Capability: Attended print, Attended display, Code
  // table 1 to 10
  emv_tlv_list_push(&emv_txn->terminal,
                    EMV_TAG_9F40_ADDITIONAL_TERMINAL_CAPABILITIES, 5,
                    (uint8_t[]){0xFA, 0x00, 0xF0, 0xA3, 0xFF}, 0);

  // Supported applications
  emv_tlv_list_push(&emv_txn->supported_aids, EMV_TAG_9F06_AID, 6,
                    (uint8_t[]){0xA0, 0x00, 0x00, 0x00, 0x03, 0x10},
                    EMV_ASI_PARTIAL_MATCH); // Visa
  emv_tlv_list_push(&emv_txn->supported_aids, EMV_TAG_9F06_AID, 7,
...
int main(int argc, char **argv) {
  ...
  if (txn_type != EMV_TRANSACTION_TYPE_INQUIRY && txn_amount == 0) {
    fprintf(stderr,
            "Transaction amount (--txn-amount) argument must be non-zero\n");
    argp_help(&argp_config, stdout, ARGP_HELP_STD_HELP, argv[0]);
  }

  if (txn_type == EMV_TRANSACTION_TYPE_CASHBACK && txn_amount_other == 0) {
    fprintf(stderr, "Secondary transaction amount (--txn-amount-other) must be "
                    "non-zero for cashback transaction\n");
    argp_help(&argp_config, stdout, ARGP_HELP_STD_HELP, argv[0]);
  }
  ...
  if (r > 0) {
    fprintf(stderr, "Failed to find iso-codes data; currency, country and "
                    "language lookups will not be possible\n");
  }

  r = emv_debug_init(debug_sources_mask, debug_level,
                     debug_verbose ? &print_emv_debug_verbose
                                   : &print_emv_debug);
  if (r) {
    printf("Failed to initialise EMV debugging\n");
    return 1;
  }
  emv_debug_trace_msg("Debugging enabled; debug_verbose=%d; "
                      "debug_sources_mask=0x%02X; debug_level=%u",
                      debug_verbose, debug_sources_mask, debug_level);
  ...
  r = emv_build_candidate_list(&emv_txn.ttl, &emv_txn.supported_aids,
                               &app_list);
  if (r < 0) {
    printf("ERROR: %s\n", emv_error_get_string(r));
    goto emv_exit;
  ...
  emv_txn.application_selection_required =
      emv_app_list_selection_is_required(&app_list);
  ...
  do {
    ...
    if (emv_txn.application_selection_required) {
      ...
      printf("\nSelect application:\n");
      for (struct emv_app_t *app = app_list.front; app != NULL;
           app = app->next) {
        ++app_count;
    ...
    r = emv_select_application(&emv_txn.ttl, &app_list, index,
    ...
    r = emv_initiate_application_processing(&emv_txn.ttl, emv_txn.selected_app,
                                            &emv_txn.params, &emv_txn.terminal,
                                            &emv_txn.icc);
    if (r < 0) {
      printf("ERROR: %s\n", emv_error_get_string(r));
      goto emv_exit;
    ...
    if (r > 0) {
      ...
      if (r == EMV_OUTCOME_GPO_NOT_ACCEPTED &&
          !emv_app_list_is_empty(&app_list)) {
        // Return to cardholder application selection/confirmation
