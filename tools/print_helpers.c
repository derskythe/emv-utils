...
void print_str_list(const char *str_list, const char *delim, const char *prefix,
                    unsigned int depth, const char *bullet,
                    const char *suffix) {
  const char *str;
...
void print_atr(const struct iso7816_atr_info_t *atr_info) {
  ...
  // Print ATR info
  printf("  TS  = 0x%02X: %s\n", atr_info->TS,
         iso7816_atr_TS_get_string(atr_info));
  printf("  T0  = 0x%02X: %s\n", atr_info->T0,
         iso7816_atr_T0_get_string(atr_info, str, sizeof(str)));
  for (size_t i = 1; i < 5; ++i) {
    if (atr_info->TA[i] || atr_info->TB[i] || atr_info->TC[i] ||
        atr_info->TD[i] || i < 3) {
      printf("  ----\n");
    }
  ...
  if (atr_info->K_count) {
    ...
    if (atr_info->status_indicator_bytes) {
      ...
      if (atr_info->status_indicator.SW1 || atr_info->status_indicator.SW2) {
        printf("  SW  = %02X%02X: (%s)\n", atr_info->status_indicator.SW1,
               atr_info->status_indicator.SW2,
               iso7816_sw1sw2_get_string(atr_info->status_indicator.SW1,
                                         atr_info->status_indicator.SW2, str,
                                         sizeof(str)));
      }
    }
  }
...
void print_atr_historical_bytes(const struct iso7816_atr_info_t *atr_info) {
  ...
  if (atr_info->T1 != ISO7816_ATR_T1_COMPACT_TLV_SI &&
    ...
    print_buf("  Historical bytes", atr_info->historical_bytes,
              atr_info->historical_bytes_len);
    return;
  }

  r = iso7816_compact_tlv_itr_init(atr_info->historical_bytes,
                                   atr_info->historical_bytes_len, &itr);
  if (r) {
  ...
  while ((r = iso7816_compact_tlv_itr_next(&itr, &tlv)) > 0) {
    printf("  %s (0x%X): [%u] ", iso7816_compact_tlv_tag_get_string(tlv.tag),
           tlv.tag, tlv.length);
    ...
    case ISO7816_COMPACT_TLV_CARD_SERVICE_DATA:
      r = iso7816_card_service_data_get_string_list(tlv.value[0], str,
                                                    sizeof(str));
      break;

    case ISO7816_COMPACT_TLV_CARD_CAPABILITIES:
      r = iso7816_card_capabilities_get_string_list(tlv.value, tlv.length, str,
                                                    sizeof(str));
      break;
...
void print_capdu(const void *c_apdu, size_t c_apdu_len) {
  ...
  r = emv_capdu_get_string(c_apdu, c_apdu_len, str, sizeof(str));
  if (r) {
...
void print_rapdu(const void *r_apdu, size_t r_apdu_len) {
  ...
  s = iso7816_sw1sw2_get_string(ptr[r_apdu_len - 2], ptr[r_apdu_len - 1], str,
                                sizeof(str));
  if (!s || !s[0]) {
...
void print_emv_tlv(const struct emv_tlv_t *tlv, const char *prefix,
  ...
  if (iso8825_ber_is_constructed(&tlv->ber)) {
    ...
    if (!value_str[0] || str_is_list(value_str) ||
      ...
      if (info.format == EMV_FORMAT_A || info.format == EMV_FORMAT_AN ||
...
static void print_emv_debug_internal(enum emv_debug_type_t debug_type,
