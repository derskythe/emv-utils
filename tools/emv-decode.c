...
static struct argp_option argp_options[] = {
    ...
    {"atr", EMV_DECODE_ATR, NULL, 0,
     "Decode ISO 7816 Answer-To-Reset (ATR), including initial character TS"},
    {"sw1sw2", EMV_DECODE_SW1SW2, NULL, 0,
     "Decode ISO 7816 Status bytes SW1-SW2, eg 9000"},
    ...
    {"mcc", EMV_DECODE_MCC, NULL, 0,
     "Decode Merchant Category Code (field 9F15)"},
    {"9F15", EMV_DECODE_MCC, NULL, OPTION_ALIAS},
    {"term-type", EMV_DECODE_TERM_TYPE, NULL, 0,
     "Decode Terminal Type (field 9F35)"},
    {"9F35", EMV_DECODE_TERM_TYPE, NULL, OPTION_ALIAS},
    {"term-caps", EMV_DECODE_TERM_CAPS, NULL, 0,
     "Decode Terminal Capabilities (field 9F33)"},
    {"9F33", EMV_DECODE_TERM_CAPS, NULL, OPTION_ALIAS},
    {"addl-term-caps", EMV_DECODE_ADDL_TERM_CAPS, NULL, 0,
     "Decode Additional Terminal Capabilities (field 9F40)"},
    {"9F40", EMV_DECODE_ADDL_TERM_CAPS, NULL, OPTION_ALIAS},
    {"cvm-list", EMV_DECODE_CVM_LIST, NULL, 0,
     "Decode Cardholder Verification Method (CVM) List (field 8E)"},
    {"8E", EMV_DECODE_CVM_LIST, NULL, OPTION_ALIAS},
    {"cvm-results", EMV_DECODE_CVM_RESULTS, NULL, 0,
     "Decode Cardholder Verification Method (CVM) Results (field 9F34)"},
    {"9F34", EMV_DECODE_CVM_RESULTS, NULL, OPTION_ALIAS},
    {"tvr", EMV_DECODE_TVR, NULL, 0,
     "Decode Terminal Verification Results (field 95)"},
    {"95", EMV_DECODE_TVR, NULL, OPTION_ALIAS},
    {"tsi", EMV_DECODE_TSI, NULL, 0,
     "Decode Transaction Status Information (field 9B)"},
    {"9B", EMV_DECODE_TSI, NULL, OPTION_ALIAS},
    {"issuer-app-data", EMV_DECODE_IAD, NULL, 0,
     "Decode Issuer Application Data (field 9F10)"},
    {"9F10", EMV_DECODE_IAD, NULL, OPTION_ALIAS},
    {"ttq", EMV_DECODE_TTQ, NULL, 0,
     "Decode Terminal Transaction Qualifiers (field 9F66)"},
    {"9F66", EMV_DECODE_TTQ, NULL, OPTION_ALIAS},
    {"ctq", EMV_DECODE_CTQ, NULL, 0,
     "Decode Card Transaction Qualifiers (field 9F6C)"},
    {"9F6C", EMV_DECODE_CTQ, NULL, OPTION_ALIAS},
    {"amex-cl-reader-caps", EMV_DECODE_AMEX_CL_READER_CAPS, NULL, 0,
     "Decode Amex Contactless Reader Capabilities (field 9F6D)"},
    {"9F6D", EMV_DECODE_AMEX_CL_READER_CAPS, NULL, OPTION_ALIAS},
    {"mastercard-third-party-data", EMV_DECODE_MASTERCARD_THIRD_PARTY_DATA,
     NULL, 0, "Decode Mastercard Third Party Data (field 9F6E)"},
    {"visa-ffi", EMV_DECODE_FFI, NULL, 0,
     "Decode Visa Form Factor Indicator (field 9F6E)"},
    {"amex-enh-cl-reader-caps", EMV_DECODE_AMEX_ENH_CL_READER_CAPS, NULL, 0,
     "Decode Amex Enhanced Contactless Reader Capabilities (field 9F6E)"},
    {"terminal-risk-management-data", EMV_DECODE_TERMINAL_RISK_MANAGEMENT_DATA,
     NULL, 0, "Decode Terminal Risk Management Data (field 9F1D)"},
    {"9F1D", EMV_DECODE_TERMINAL_RISK_MANAGEMENT_DATA, NULL, OPTION_ALIAS},

    {NULL, 0, NULL, 0, "Other:", 4},
    {"country", EMV_DECODE_ISO3166_1, NULL, 0,
     "Lookup country name by ISO 3166-1 alpha-2, alpha-3 or numeric code"},
    {"iso3166-1", EMV_DECODE_ISO3166_1, NULL, OPTION_ALIAS},
    {"currency", EMV_DECODE_ISO4217, NULL, 0,
     "Lookup currency name by ISO 4217 alpha-3 or numeric code"},
    {"iso4217", EMV_DECODE_ISO4217, NULL, OPTION_ALIAS},
    {"language", EMV_DECODE_ISO639, NULL, 0,
     "Lookup language name by ISO 639 alpha-2 or alpha-3 code"},
    {"iso639", EMV_DECODE_ISO639, NULL, OPTION_ALIAS},

    {"iso8859-x", EMV_DECODE_ISO8859_X, NULL, 0,
     "Decode INPUT as ISO8859 using the code page specified by 'x' and print "
     "as UTF-8"},
    ...
    {0, 0, NULL, 0,
     "INPUT is either a string of hex digits representing binary data, or "
     "\"-\" to read from stdin"},
...
static error_t argp_parser_helper(int key, char *arg,
  ...
  case ARGP_KEY_ARG: {
    ...
    if (arg_len == 1 && *arg == '-') {
      ...
      if (arg_len < 2) {
        argp_error(state,
                   "INPUT must consist of at least 1 byte (thus 2 hex digits)");
      }
  ...
  case EMV_DECODE_ISO8859_X: {
    printf("Use --iso8859-x where 'x' is the code page number, for example  "
           "--iso8859-5\n");
    exit(EXIT_SUCCESS);
    return 1;
  }
...
int main(int argc, char **argv) {
  ...
  case EMV_DECODE_MCC: {
    ...
    if (data_len != 2) {
      fprintf(stderr,
              "Merchant Category Code (MCC) must be 4-digit numeric code\n");
      break;
    }
  ...
  case EMV_DECODE_TERM_TYPE: {
    ...
    if (data_len != 1) {
      fprintf(stderr,
              "EMV Terminal Type (field 9F35) must be exactly 1 byte\n");
      break;
    }
  ...
  case EMV_DECODE_TERM_CAPS: {
    ...
    if (data_len != 3) {
      fprintf(
          stderr,
          "EMV Terminal Capabilities (field 9F33) must be exactly 3 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(stderr,
              "Failed to parse EMV Terminal Capabilities (field 9F33)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_ADDL_TERM_CAPS: {
    ...
    if (data_len != 5) {
      fprintf(stderr, "EMV Additional Terminal Capabilities (field 9F40) must "
                      "be exactly 5 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(stderr, "Failed to parse EMV Additional Terminal Capabilities "
                      "(field 9F40)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_CVM_LIST: {
    ...
    if (r) {
      fprintf(stderr, "Failed to parse EMV Cardholder Verification Method "
                      "(CVM) List (field 8E)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_CVM_RESULTS: {
    ...
    if (data_len != 3) {
      fprintf(stderr, "EMV Cardholder Verification Method (CVM) Results (field "
                      "9F34) must be exactly 3 bytes\n");
      break;
    }
  ...
  case EMV_DECODE_TVR: {
    ...
    if (data_len != 5) {
      fprintf(stderr, "EMV Terminal Verification Results (field 95) must be "
                      "exactly 5 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(stderr,
              "Failed to parse EMV Terminal Verification Results (field 95)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_TSI: {
    ...
    if (data_len != 2) {
      fprintf(stderr, "EMV Transaction Status Information (field 9B) must be "
                      "exactly 2 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(
          stderr,
          "Failed to parse EMV Transaction Status Information (field 9B)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_IAD: {
    ...
    if (data_len > 32) {
      fprintf(
          stderr,
          "EMV Issuer Application Data (field 9F10) may be up to 32 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(stderr,
              "Failed to parse EMV Issuer Application Data (field 9F10)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_TTQ: {
    ...
    if (data_len != 4) {
      fprintf(stderr, "EMV Terminal Transaction Qualifiers (field 9F66) must "
                      "be exactly 4 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(
          stderr,
          "Failed to parse EMV Terminal Transaction Qualifiers (field 9F66)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_CTQ: {
    ...
    if (data_len != 2) {
      fprintf(stderr, "EMV Card Transaction Qualifiers (field 9F6C) must be "
                      "exactly 2 bytes\n");
      break;
    }
    ...
    if (r) {
      fprintf(stderr,
              "Failed to parse EMV Card Transaction Qualifiers (field 9F6C)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_AMEX_CL_READER_CAPS: {
    ...
    if (data_len != 1) {
      fprintf(stderr, "Amex Contactless Reader Capabilities (field 9F6D) must "
                      "be exactly 1 byte\n");
      break;
    }
    ...
    if (r) {
      fprintf(stderr, "Failed to parse Amex Contactless Reader Capabilities "
                      "(field 9F6D)\n");
      break;
    }
    printf("%s\n", str);
  ...
  case EMV_DECODE_MASTERCARD_THIRD_PARTY_DATA: {
    ...
    r = emv_mastercard_third_party_data_get_string_list(data, data_len, str,
                                                        sizeof(str));
    if (r) {
      fprintf(stderr,
              "Failed to parse Mastercard Third Party Data (field 9F6E)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_FFI: {
    ...
    if (data_len != 4) {
      fprintf(
          stderr,
          "Visa Form Factor Indicator (field 9F6E) must be exactly 4 bytes\n");
      break;
    }

    r = emv_visa_form_factor_indicator_get_string_list(data, data_len, str,
                                                       sizeof(str));
    if (r) {
      fprintf(stderr,
              "Failed to parse Visa Form Factor Indicator (field 9F6E)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_AMEX_ENH_CL_READER_CAPS: {
    ...
    if (data_len != 4) {
      fprintf(stderr, "Amex Enhanced Contactless Reader Capabilities (field "
                      "9F6E) must be exactly 4 bytes\n");
      break;
    }

    r = emv_amex_enh_cl_reader_caps_get_string_list(data, data_len, str,
                                                    sizeof(str));
    if (r) {
      fprintf(stderr, "Failed to parse Amex Enhanced Contactless Reader "
                      "Capabilities (field 9F6E)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_TERMINAL_RISK_MANAGEMENT_DATA: {
    ...
    if (data_len != 8) {
      fprintf(stderr, "Terminal Risk Management Data (field 9F1D) must be "
                      "exactly 8 bytes\n");
      break;
    }

    r = emv_terminal_risk_management_data_get_string_list(data, data_len, str,
                                                          sizeof(str));
    if (r) {
      fprintf(stderr,
              "Failed to parse Terminal Risk Management Data (field 9F1D)\n");
      break;
    }
    printf("%s", str); // No \n required for string list
  ...
  case EMV_DECODE_ISO3166_1: {
    ...
    if (arg_str_len != 2 && arg_str_len != 3) {
      fprintf(stderr, "ISO 3166-1 country code must be alpha-2, alpha-3 or "
  ...
  case EMV_DECODE_ISO4217: {
    ...
    if (arg_str_len != 3) {
      fprintf(
          stderr,
          "ISO 4217 currency code must be alpha-3 or 3-digit numeric code\n");
      break;
    }
  ...
  case EMV_DECODE_ISO639: {
    ...
    if (arg_str_len != 2 && arg_str_len != 3) {
      fprintf(stderr,
              "ISO 639 currency code must be alpha-2 or alpha-3 code\n");
      break;
    }
