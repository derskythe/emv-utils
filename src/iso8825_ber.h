...
#define ISO8825_BER_TAG_HIGH_FORM                                              \
  (0x1F) ///< BER high tag number form; for tag numbers >= 31
#define ISO8825_BER_TAG_HIGH_FORM_MORE                                         \
  (0x80) ///< BER high tag number form: more octets to follow
#define ISO8825_BER_TAG_HIGH_FORM_NUMBER_MASK                                  \
  (0x7F) ///< BER high tag number form: next 7 bits of tag number

// Length encoding (see ISO 8825-1:2003, 8.1.3)
#define ISO8825_BER_LEN_INDEFINITE_FORM                                        \
  (0x80) ///< BER indefinite length form value
#define ISO8825_BER_LEN_LONG_FORM                                              \
  (0x80) ///< BER definite long length form bit; for length values > 127
#define ISO8825_BER_LEN_LONG_FORM_COUNT_MASK                                   \
  (0x7F) ///< BER definite long length form mask: number of length octets
...
#define ASN1_RELATIVE_OBJECT_IDENTIFIER                                        \
  (0x0D)                            ///< ASN.1 Relative object identifier type
...
#define ASN1_BMPSTRING                                                         \
  (0x1E) ///< ASN.1 Basic Multilingual Plane (BMP) string type
...
#define ASN1_OID_ITU_T_RECOMMENDED                                             \
  (0) ///< ASN.1 ITU-T arc for ITU-T and CCITT recommendations
#define ASN1_OID_ITU_T_QUESTION (1) ///< ASN.1 ITU-T arc for ITU-T Study Groups
#define ASN1_OID_ITU_T_ADMINISTRATION                                          \
  (2) ///< ASN.1 ITU-T arc for ITU-T X.121 Data Country Codes (DCCs)
#define ASN1_OID_ITU_T_NETWORK_OPERATOR                                        \
  (3) ///< ASN.1 ITU-T arc for ITU-T X.121 Data Network Identification Code
      ///< (DNICs)
#define ASN1_OID_ITU_T_IDENTIFIED_ORG                                          \
  (4) ///< ASN.1 ITU-T arc for ITU-T Telecommunication Standardization Bureau
      ///< (TSB) identified organisations
...
#define ASN1_OID_ISO_IDENTIFIED_ORG                                            \
  (2) ///< ASN.1 ISO arc for ISO identified organisations (ISO 6523)

// ASN.1 object identifier arcs for Joint ISO/IEC and ITU-T directory services
// (interesting ones only)
#define ASN1_OID_JOINT_DS (5) ///< ASN.1 joint-iso-itu-t directory services
#define ASN1_OID_JOINT_DS_ATTR_TYPE                                            \
  (4) ///< ASN.1 joint-iso-itu-t directory services attribyte type
#define ASN1_OID_JOINT_DS_OBJ_CLASS                                            \
  (6) ///< ASN.1 joint-iso-itu-t directory services object class

/// ISO 8825 TLV field
struct iso8825_tlv_t {
  unsigned int tag; ///< BER encoded tag, including class, primitive/structured
                    ///< bit, and tag number
 ...
 */
static inline uint8_t iso8825_ber_get_class(const struct iso8825_tlv_t *tlv) {
  return tlv ? (tlv->flags & ISO8825_BER_CLASS_MASK) : 0;
}
 ...
 */
static inline bool iso8825_ber_is_constructed(const struct iso8825_tlv_t *tlv) {
  return tlv && (tlv->flags & ISO8825_BER_CONSTRUCTED);
}
 ...
 */
int iso8825_ber_itr_init(const void *ptr, size_t len,
 ...
 * error.
 */
int iso8825_ber_itr_next(struct iso8825_ber_itr_t *itr,
 ...
 * @return Zero for success. Less than zero for error.
 */
int iso8825_ber_oid_decode(const void *ptr, size_t len,
