...
int emv_ttl_trx(struct emv_ttl_t *ctx, const void *c_apdu, size_t c_apdu_len,
  ...
  if (!ctx || !c_apdu || !c_apdu_len || !r_apdu || !r_apdu_len ||
      !*r_apdu_len || !sw1sw2) {
    return -1;
  }
  ...
  *sw1sw2 = 0;
  ...
  do {
    ...
    r = ctx->cardreader.trx(ctx->cardreader.ctx, tx_buf, tx_buf_len, rx_buf,
                            &rx_len);
    if (r) {
      return r;
    }
    ...
    case 0x6C: // Checking error: Wrong Le field; SW2 encodes the exact number
               // of available bytes
    ...
    default:
      ...
      if (!*sw1sw2) {
        ...
        *sw1sw2 = ((uint16_t)SW1 << 8) | SW2;
        ...
        // See EMV Contact Interface Specification v1.0, Annex A7 "Case 4
        // Command with Warning Condition"
        if (apdu_case == ISO7816_APDU_CASE_4S && rx_len == 2 &&
    ...
    if (ctx->cardreader.mode == EMV_CARDREADER_MODE_TPDU && rx_len > 3) {
    ...
    if (ctx->cardreader.mode == EMV_CARDREADER_MODE_APDU && rx_len > 2) {
      // Response data is only allowed for APDU cases 2 and 4
      if (apdu_case != ISO7816_APDU_CASE_2S &&
          apdu_case != ISO7816_APDU_CASE_4S) {
        return 11;
    ...
    if (tx_get_response) {
      ...
      if (SW1 == 0x61 || (SW1 == 0x62 && SW2 >= 0x02 && SW2 <= 0x80)) {
        Le = SW2;
      } else {
        Le = 0;
      }
...
int emv_ttl_select_by_df_name(struct emv_ttl_t *ctx, const void *df_name,
  ...
  r = emv_ttl_trx(ctx, &c_apdu, iso7816_apdu_case_4s_length(&c_apdu), r_apdu,
                  &r_apdu_len, sw1sw2);
  if (r) {
    return r;
  }
...
int emv_ttl_read_record(struct emv_ttl_t *ctx, uint8_t sfi,
  ...
  r = emv_ttl_trx(ctx, &c_apdu, sizeof(c_apdu), r_apdu, &r_apdu_len, sw1sw2);
  if (r) {
    return r;
  }
  if (r_apdu_len < 2) {
