...
static const uint8_t mastercard_maestro_uk_aid[] = {0xA0, 0x00, 0x00, 0x00,
                                                    0x05};
static const uint8_t mastercard_test_aid[] = {0xB0, 0x12, 0x34, 0x56, 0x78};

// American Express
static const uint8_t amex_aid[] = {0xA0, 0x00, 0x00, 0x00, 0x25};
static const uint8_t amex_china_credit_debit_aid[] = {0xA0, 0x00, 0x00, 0x07,
                                                      0x90};
...
static const uint8_t dk_girocard_aid[] = {0xA0, 0x00, 0x00, 0x03, 0x59,
                                          0x10, 0x10, 0x02, 0x80, 0x01};
...
#define emv_pix_match(aid, aid_len, pix_buf)                                   \
  (aid_len >= 5 + sizeof(pix_buf) &&                                           \
   memcmp(aid + 5, pix_buf, sizeof(pix_buf)) == 0)
...
int emv_afl_itr_next(struct emv_afl_itr_t *itr, struct emv_afl_entry_t *entry) {
  ...
  if ((afl[0] & EMV_AFL_SFI_MASK) == 0 ||
      (afl[0] & EMV_AFL_SFI_MASK) == EMV_AFL_SFI_MASK) {
  ...
  if (afl[3] > afl[2] - afl[1] + 1) {
    // AFL byte 4 must not exceed the number of records indicated by AFL byte 2
    // and byte 3
    return -7;
  }
...
enum emv_iad_format_t emv_iad_get_format(const uint8_t *iad, size_t iad_len) {
  ...
  // compliant applications See EMV 4.4 Book 3, Annex C9
  if (iad_len == EMV_IAD_CCD_LEN && iad[0] == EMV_IAD_CCD_BYTE1 &&
  ...
  if (iad_len >= EMV_IAD_VSDC_0_1_3_MIN_LEN &&
    ...
    switch ((iad[2] & EMV_IAD_VSDC_CVN_FORMAT_MASK) >>
            EMV_IAD_VSDC_CVN_FORMAT_SHIFT) {
    case 0:
      return EMV_IAD_FORMAT_VSDC_0;
    case 1:
      return EMV_IAD_FORMAT_VSDC_1;
    case 3:
      return EMV_IAD_FORMAT_VSDC_3;
    default:
      return EMV_IAD_FORMAT_INVALID;
    }
  }
  if (iad_len == EMV_IAD_VSDC_2_4_LEN && iad[0] == EMV_IAD_VSDC_2_4_BYTE1 &&
      !(iad[8] & 0xF0) // Unused nibble of Issuer Discretionary Data Option ID
  ) {
    switch ((iad[1] & EMV_IAD_VSDC_CVN_FORMAT_MASK) >>
            EMV_IAD_VSDC_CVN_FORMAT_SHIFT) {
    case 2:
      return EMV_IAD_FORMAT_VSDC_2;
    case 4:
      return EMV_IAD_FORMAT_VSDC_4;
    default:
      return EMV_IAD_FORMAT_INVALID;
    }
  }
