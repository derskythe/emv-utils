...
static int pcsc_reader_get_feature(struct pcsc_reader_t *reader,
                                   unsigned int feature, LPDWORD control_code);
static int pcsc_reader_internal_get_uid(pcsc_reader_ctx_t reader_ctx,
...
int pcsc_init(pcsc_ctx_t *ctx) {
  ...
  *ctx = malloc(sizeof(struct pcsc_t));
  ...
  result =
      SCardEstablishContext(SCARD_SCOPE_SYSTEM, NULL, NULL, &pcsc->context);
  ...
  result =
      SCardListReaders(pcsc->context, NULL, NULL, &pcsc->reader_strings_size);
  if (result != SCARD_S_SUCCESS) {
  ...
  result = SCardListReaders(pcsc->context, NULL, pcsc->reader_strings,
                            &pcsc->reader_strings_size);
  if (result != SCARD_S_SUCCESS) {
    fprintf(stderr, "SCardListReaders() failed; result=0x%x [%s]\n",
            (unsigned int)result, pcsc_stringify_error(result));
    pcsc_release(ctx);
  ...
  memset(pcsc->reader_states, 0,
         pcsc->reader_count * sizeof(SCARD_READERSTATE));
  for (size_t i = 0; i < pcsc->reader_count; ++i) {
...
static int pcsc_reader_populate_features(struct pcsc_reader_t *reader) {
  ...
  result = SCardConnect(reader->pcsc->context, reader->name, SCARD_SHARE_DIRECT,
                        0, &card, &protocol);
  if (result != SCARD_S_SUCCESS) {
    fprintf(stderr,
            "SCardConnect(SCARD_SHARE_DIRECT) failed; result=0x%x [%s]\n",
            (unsigned int)result, pcsc_stringify_error(result));
    r = -2;
    goto exit;
  }
  ...
  result = SCardControl(card, CM_IOCTL_GET_FEATURE_REQUEST, NULL, 0,
                        reader->features.buf, sizeof(reader->features.buf),
                        &reader->features.buf_len);
  if (result != SCARD_S_SUCCESS) {
    fprintf(
        stderr,
        "SCardControl(CM_IOCTL_GET_FEATURE_REQUEST) failed; result=0x%x [%s]\n",
        (unsigned int)result, pcsc_stringify_error(result));
    r = -3;
    goto exit;
  }
  ...
  while ((void *)pcsc_tlv - (void *)reader->features.buf <
         reader->features.buf_len) {
  ...
  r = pcsc_reader_get_feature(reader, PCSC_FEATURE_IFD_PIN_PROPERTIES,
                              &control_code);
  if (r < 0) {
    r = -6;
    goto exit;
  ...
  if (r == 0) {
    ...
    result = SCardControl(card, control_code, NULL, 0,
    ...
    if (result != SCARD_S_SUCCESS) {
      fprintf(stderr, "SCardControl(%04X) failed; result=0x%x [%s]\n",
              (unsigned int)control_code, (unsigned int)result,
  ...
  r = pcsc_reader_get_feature(reader, PCSC_FEATURE_IFD_DISPLAY_PROPERTIES,
                              &control_code);
  if (r < 0) {
    r = -9;
    goto exit;
  ...
  r = pcsc_reader_get_feature(reader, PCSC_FEATURE_GET_TLV_PROPERTIES,
                              &control_code);
  if (r < 0) {
    r = -12;
    goto exit;
  ...
  if (r == 0) {
    // Request reader TLV properties
    result = SCardControl(
        card, control_code, NULL, 0, reader->features.properties,
        sizeof(reader->features.properties), &reader->features.properties_len);
    if (result != SCARD_S_SUCCESS) {
      fprintf(stderr, "SCardControl(%04X) failed; result=0x%x [%s]\n",
              (unsigned int)control_code, (unsigned int)result,
              pcsc_stringify_error(result));
...
exit:
  ...
  if (result != SCARD_S_SUCCESS) {
    fprintf(stderr,
            "SCardDisconnect(SCARD_LEAVE_CARD) failed; result=0x%x [%s]\n",
            (unsigned int)result, pcsc_stringify_error(result));
    // Intentionally ignore errors
  }
...
bool pcsc_reader_has_feature(pcsc_reader_ctx_t reader_ctx,
...
int pcsc_reader_get_property(pcsc_reader_ctx_t reader_ctx,
                             unsigned int property, void *value,
                             size_t *value_len) {
  struct pcsc_reader_t *reader;
  const uint8_t *tlv;
  size_t tlv_len;
  ...
  if (reader->features.pin_properties_len) {
    if (property == PCSC_PROPERTY_wLcdLayout) {
      ...
      memcpy(value, &reader->features.pin_properties.wLcdLayout,
  ...
  if (reader->features.display_properties_len) {
    if (property == PCSC_PROPERTY_wLcdMaxCharacters) {
      ...
      memcpy(value, &reader->features.display_properties.wLcdMaxCharacters,
             sizeof(reader->features.display_properties.wLcdMaxCharacters));
      *value_len =
          sizeof(reader->features.display_properties.wLcdMaxCharacters);
      return 0;
    ...
    if (property == PCSC_PROPERTY_wLcdMaxLines) {
      ...
      memcpy(value, &reader->features.display_properties.wLcdMaxLines,
             sizeof(reader->features.display_properties.wLcdMaxLines));
...
int pcsc_reader_get_state(pcsc_reader_ctx_t reader_ctx, unsigned int *state) {
  ...
  result =
      SCardGetStatusChange(reader->pcsc->context, INFINITE, &reader_state, 1);
  if (result != SCARD_S_SUCCESS) {
...
int pcsc_wait_for_card(pcsc_ctx_t ctx, unsigned long timeout_ms, size_t *idx) {
  ...
  result = SCardGetStatusChange(pcsc->context, timeout_ms, pcsc->reader_states,
                                pcsc->reader_count);
...
static int pcsc_reader_internal_get_uid(pcsc_reader_ctx_t reader_ctx,
  ...
  r = pcsc_reader_trx(reader_ctx, pcsc_get_uid_capdu,
                      sizeof(pcsc_get_uid_capdu), rx_buf, &rx_len);
  if (r) {
    return r;
  }
  if (rx_len < 2) {
...
int pcsc_reader_connect(pcsc_reader_ctx_t reader_ctx) {
  ...
  result = SCardConnect(
      reader->pcsc->context, reader->name, SCARD_SHARE_EXCLUSIVE,
      SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, &reader->card, &reader->protocol);
  if (result != SCARD_S_SUCCESS) {
  ...
  result = SCardStatus(reader->card, NULL, NULL, &state, &reader->protocol,
                       reader->atr, &reader->atr_len);
  if (result != SCARD_S_SUCCESS) {
  ...
  if (reader->atr_len > 4 && reader->atr[0] == 0x3B &&
      (reader->atr[1] & 0x80) == 0x80 && reader->atr[2] == 0x80 &&
      reader->atr[3] == 0x01 &&
    ...
    r = pcsc_reader_internal_get_uid(reader_ctx, reader->uid, &reader->uid_len);
    if (r < 0) {
...
int pcsc_reader_get_atr(pcsc_reader_ctx_t reader_ctx, void *atr,
                        size_t *atr_len) {
  struct pcsc_reader_t *reader;
...
int pcsc_reader_trx(pcsc_reader_ctx_t reader_ctx, const void *tx_buf,
                    size_t tx_buf_len, void *rx_buf, size_t *rx_buf_len) {
  struct pcsc_reader_t *reader;
  LONG result;
  DWORD rx_len;
  ...
  case SCARD_PROTOCOL_T0:
    result = SCardTransmit(reader->card, SCARD_PCI_T0, tx_buf, tx_buf_len, NULL,
                           rx_buf, &rx_len);
    break;

  case SCARD_PROTOCOL_T1:
    result = SCardTransmit(reader->card, SCARD_PCI_T1, tx_buf, tx_buf_len, NULL,
                           rx_buf, &rx_len);
    break;
