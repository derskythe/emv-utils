...
static int json_array_visit_userfunc(json_object *jso, int flags,
  ...
  if (jso_index && json_object_is_type(jso, json_type_object)) {
    ...
    // Append object to the appropriate using the function pointer provided by
    // userarg
...
static bool build_country_list(json_object *json_root) noexcept {
  ...
  */
  ...
  r = json_c_visit(iso3166_1_obj, 0, &json_array_visit_userfunc,
                   (void *)&country_list_append);
  if (r) {
    return false;
  }
...
static bool build_currency_list(json_object *json_root) noexcept {
  ...
  */
  ...
  r = json_c_visit(iso4217_obj, 0, &json_array_visit_userfunc,
                   (void *)&currency_list_append);
  if (r) {
    return false;
  }
...
static bool build_language_list(json_object *json_root) noexcept {
  ...
  */
  ...
  r = json_c_visit(iso_639_2_obj, 0, &json_array_visit_userfunc,
                   (void *)language_list_append);
  if (r) {
    return false;
  }
