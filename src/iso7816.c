...
// Helper functions
static void
iso7816_atr_populate_default_parameters(struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TA1(uint8_t TA1,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TB1(uint8_t TB1,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TC1(uint8_t TC1,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TDi(unsigned int i, uint8_t TDi,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TA2(uint8_t TA2,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TB2(uint8_t TB2,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TC2(uint8_t TC2,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TAi(uint8_t protocol, unsigned int i, uint8_t TAi,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TBi(uint8_t protocol, unsigned int i, uint8_t TBi,
                                 struct iso7816_atr_info_t *atr_info);
static int iso7816_atr_parse_TCi(uint8_t protocol, unsigned int i, uint8_t TCi,
                                 struct iso7816_atr_info_t *atr_info);
...
static int
iso7816_atr_parse_historical_bytes(const void *historical_bytes,
...
int iso7816_atr_parse(const uint8_t *atr, size_t atr_len,
  ...
  for (unsigned int i = 1; i < 5; ++i) {
    uint8_t interface_byte_bits =
        atr_info->atr[atr_idx++]; // Y[i] value according to ISO7816
    ...
    if (interface_byte_bits & ISO7816_ATR_Tx_TAi_PRESENT) {
      ...
      // Parse TA1
      case 1:
        r = iso7816_atr_parse_TA1(*atr_info->TA[i], atr_info);
        break;
      // Parse TA2
      case 2:
        r = iso7816_atr_parse_TA2(*atr_info->TA[i], atr_info);
        break;
      // Parse TAi for i>=3
      default:
        r = iso7816_atr_parse_TAi(protocol, i, *atr_info->TA[i], atr_info);
        break;
      }
      if (r) {
        return r;
    ...
    if (interface_byte_bits & ISO7816_ATR_Tx_TBi_PRESENT) {
      ...
      // Parse TB1
      case 1:
        r = iso7816_atr_parse_TB1(*atr_info->TB[i], atr_info);
        break;
      // Parse TB2
      case 2:
        r = iso7816_atr_parse_TB2(*atr_info->TB[i], atr_info);
        break;
      // Parse TBi for i>=3
      default:
        r = iso7816_atr_parse_TBi(protocol, i, *atr_info->TB[i], atr_info);
        break;
      }
      if (r) {
        return r;
    ...
    if (interface_byte_bits & ISO7816_ATR_Tx_TCi_PRESENT) {
      ...
      // Parse TC1
      case 1:
        r = iso7816_atr_parse_TC1(*atr_info->TC[i], atr_info);
        break;
      // Parse TC2
      case 2:
        r = iso7816_atr_parse_TC2(*atr_info->TC[i], atr_info);
        break;
      // Parse TCi for i>=3
      default:
        r = iso7816_atr_parse_TCi(protocol, i, *atr_info->TC[i], atr_info);
        break;
      }
      if (r) {
        return r;
    ...
    if (interface_byte_bits & ISO7816_ATR_Tx_TDi_PRESENT) {
      atr_info->TD[i] =
          &atr_info->atr[atr_idx]; // preserve index for next loop iteration
      ...
      protocol = *atr_info->TD[i] &
                 ISO7816_ATR_Tx_OTHER_MASK; // T value according to ISO
                                            // 7816-3:2006, 8.2.3
  ...
  if (atr_info->K_count) {
    ...
    case ISO7816_ATR_T1_COMPACT_TLV_SI:
      ...
      atr_info->status_indicator_bytes =
          atr_info->historical_bytes + atr_info->historical_bytes_len;
      atr_info->status_indicator_bytes_len = 3;

      // Intentional fallthrough to COMPACT-TLV parsing

    case ISO7816_ATR_T1_COMPACT_TLV:
      r = iso7816_atr_parse_historical_bytes(
          atr_info->historical_bytes, atr_info->historical_bytes_len, atr_info);
      if (r) {
        return 5;
      }
      break;
...
iso7816_atr_populate_default_parameters(struct iso7816_atr_info_t *atr_info) {
  ...
  iso7816_atr_parse_TAi(ISO7816_PROTOCOL_T15, 3, ISO7816_CARD_CLASS_A_5V,
                        atr_info);
...
static int iso7816_atr_parse_TA1(uint8_t TA1,
  ...
  // Decode bit rate adjustment factor Di according to ISO 7816-3:2006, 8.3,
  // table 8
  switch (DI) {
  case 0x01:
    atr_info->global.Di = 1;
    break;
  case 0x02:
    atr_info->global.Di = 2;
    break;
  case 0x03:
    atr_info->global.Di = 4;
    break;
  case 0x04:
    atr_info->global.Di = 8;
    break;
  case 0x05:
    atr_info->global.Di = 16;
    break;
  case 0x06:
    atr_info->global.Di = 32;
    break;
  case 0x07:
    atr_info->global.Di = 64;
    break;
  case 0x08:
    atr_info->global.Di = 12;
    break;
  case 0x09:
    atr_info->global.Di = 20;
    break;
  ...
  // Clock rate conversion factor Fi and maximum clock frequency fmax according
  // to ISO 7816-3:2006, 8.3, table 7
  switch (FI) {
  case 0x00:
    atr_info->global.Fi = 372;
    atr_info->global.fmax = 4;
    break;
  case 0x10:
    atr_info->global.Fi = 372;
    atr_info->global.fmax = 5;
    break;
  case 0x20:
    atr_info->global.Fi = 558;
    atr_info->global.fmax = 6;
    break;
  case 0x30:
    atr_info->global.Fi = 744;
    atr_info->global.fmax = 8;
    break;
  case 0x40:
    atr_info->global.Fi = 1116;
    atr_info->global.fmax = 12;
    break;
  case 0x50:
    atr_info->global.Fi = 1488;
    atr_info->global.fmax = 16;
    break;
  case 0x60:
    atr_info->global.Fi = 1860;
    atr_info->global.fmax = 20;
    break;
  case 0x90:
    atr_info->global.Fi = 512;
    atr_info->global.fmax = 5;
    break;
  case 0xA0:
    atr_info->global.Fi = 768;
    atr_info->global.fmax = 7.5f;
    break;
  case 0xB0:
    atr_info->global.Fi = 1024;
    atr_info->global.fmax = 10;
    break;
  case 0xC0:
    atr_info->global.Fi = 1536;
    atr_info->global.fmax = 15;
    break;
  case 0xD0:
    atr_info->global.Fi = 2048;
    atr_info->global.fmax = 20;
    break;
...
static int iso7816_atr_parse_TB1(uint8_t TB1,
  ...
  // Programming voltage for active state according to ISO 7816-3:1997;
  // deprecated in ISO 7816-3:2006
  if (PI1 < 5 || PI1 > 25) {
  ...
  // Maximum programming current according to ISO 7816-3:1997; deprecated in ISO
  // 7816-3:2006
  switch (II) {
  case 0x00:
    atr_info->global.Ipp = 25;
    break;
  case 0x20:
    atr_info->global.Ipp = 50;
    break;
  case 0x40:
    atr_info->global.Ipp = 100;
    break;
  default:
    return 13;
  }
...
static int iso7816_atr_parse_TDi(unsigned int i, uint8_t TDi,
  ...
  if (i == 1) {
    ...
    if (T != ISO7816_PROTOCOL_T0 && T != ISO7816_PROTOCOL_T1) {
...
static int iso7816_atr_parse_TA2(uint8_t TA2,
  ...
  atr_info->global.specific_mode_protocol =
      (TA2 & ISO7816_ATR_TA2_PROTOCOL_MASK);

  // TA2 indicates whether the ETU duration should be implicitly known by the
  // reader Otherwise Fi/Di provided by TA1 applies
  atr_info->global.etu_is_implicit = (TA2 & ISO7816_ATR_TA2_IMPLICIT);

  // TA2 indicates whether the specific/negotiable mode may change (eg after
  // warm ATR)
...
static int iso7816_atr_parse_TBi(uint8_t protocol, unsigned int i, uint8_t TBi,
  ...
  // Global interface parameters
  if (protocol == ISO7816_PROTOCOL_T15) {
    // First TB for T=15 indicates the use of SPU by the card (ISO
    // 7816-3:2006, 8.3, page 20)
    if (TBi) {
...
static int iso7816_atr_parse_TCi(uint8_t protocol, unsigned int i, uint8_t TCi,
  ...
  // Protocol T=1 parameters
  if (protocol == ISO7816_PROTOCOL_T1) {
    // First TC for T=1 indicates the error detection code to be used (ISO
    // 7816-3:2006, 11.4.4)
    if (TCi & ISO7816_ATR_TCi_ERROR_MASK) {
      atr_info->protocol_T1.error_detection_code =
          ISO7816_ERROR_DETECTION_CODE_CRC;
    } else {
      atr_info->protocol_T1.error_detection_code =
          ISO7816_ERROR_DETECTION_CODE_LRC;
    }
  }
...
static void iso7816_compute_gt(struct iso7816_atr_info_t *atr_info) {
  ...
  for (unsigned int i = 1; i < 5; ++i) {
    if (atr_info->TD[i] && (*atr_info->TD[i] & ISO7816_ATR_Tx_OTHER_MASK) ==
                               ISO7816_PROTOCOL_T15) {
      T15_present = true;
      break;
    }
  }
...
static void iso7816_compute_wt(struct iso7816_atr_info_t *atr_info) {
  ...
  // WWT = 960 x D x WI ETUs (D and WI are returned in TA1 and TC2,
  // respectively)
  ...
  // BWT = 11etu + 2^BWI x 960 x Fd / f; where Fd is default F=372 and f is
  // frequency NOTE: This formula specifies the first term of the sum in ETUs,
  // but not the second part. Therefore, to convert the second term of the sum
  // to ETUs, we must divide the second term by ETU, as defined relative to F
  // and D. Thus: Given 1 ETU = F/D x 1/f (see ISO 7816-3:2006, 7.1): BWT =
  // 11etu + (2^BWI x 960 x Fd / f) / (F/D x 1/f)
...
iso7816_atr_parse_historical_bytes(const void *historical_bytes,
  ...
  r = iso7816_compact_tlv_itr_init(historical_bytes, historical_bytes_len,
                                   &itr);
  if (r) {
    return 24;
  }
...
iso7816_atr_TS_get_string(const struct iso7816_atr_info_t *atr_info) {
  ...
  switch (atr_info->TS) {
  case ISO7816_ATR_TS_DIRECT:
    return "Direct convention";
  case ISO7816_ATR_TS_INVERSE:
...
static size_t
iso7816_atr_Yi_write_string(const struct iso7816_atr_info_t *atr_info, size_t i,
...
iso7816_atr_TAi_get_string(const struct iso7816_atr_info_t *atr_info, size_t i,
  ...
  if (i == 1) {
    snprintf(str, str_len,
             ...
             atr_info->global.Fi / atr_info->global.Di, atr_info->global.fmax,
             ((unsigned long)(atr_info->global.fmax * 1000000)) /
                 (atr_info->global.Fi / atr_info->global.Di));
  ...
  if (i == 2) {
    if (atr_info->TA[2]) {
      snprintf(str, str_len, "Specific mode (%s); ETU%s; protocol T=%u",
               atr_info->global.specific_mode_may_change ? "mutable"
                                                         : "immutable",
  ...
  if (i >= 3 && atr_info->TA[i]) {
    ...
    if (T == ISO7816_PROTOCOL_T15) {
      ...
      case ISO7816_CLOCK_STOP_NOT_SUPPORTED:
        clock_stop_str = "Not supported";
        break;
      case ISO7816_CLOCK_STOP_STATE_L:
        clock_stop_str = "State L";
        break;
      case ISO7816_CLOCK_STOP_STATE_H:
        clock_stop_str = "State H";
        break;
      case ISO7816_CLOCK_STOP_NO_PREFERENCE:
        clock_stop_str = "No preference";
        break;
      }
...
iso7816_atr_TBi_get_string(const struct iso7816_atr_info_t *atr_info, size_t i,
  ...
  if (i == 1) {
    ...
    if (atr_info->TB[1] && atr_info->global.Vpp_connected) {
      snprintf(str, str_len, "Vpp is connected; Vpp=%umV; Ipp=%umA",
               atr_info->global.Vpp_course, atr_info->global.Ipp);
    } else {
  ...
  if (i == 2) {
    ...
    if (atr_info->TB[1] && atr_info->TB[2] && atr_info->global.Vpp_connected) {
      snprintf(str, str_len, "Vpp=%umV", atr_info->global.Vpp);
    } else {
      snprintf(str, str_len, "Vpp is not connected");
    }
  ...
  if (i >= 3 && atr_info->TB[i]) {
    ...
    if (T == ISO7816_PROTOCOL_T1) {
      snprintf(str, str_len, "CWT=%u; BWT=%u", atr_info->protocol_T1.CWT,
...
iso7816_atr_TCi_get_string(const struct iso7816_atr_info_t *atr_info, size_t i,
  ...
  // For TC1
  if (i == 1) {
    snprintf(str, str_len, "N=%u; GT=%u", atr_info->global.N,
             atr_info->global.GT);
  ...
  // For TC2
  if (i == 2) {
    snprintf(str, str_len, "WI=%u; WT=%u", atr_info->protocol_T0.WI,
...
iso7816_atr_T1_get_string(const struct iso7816_atr_info_t *atr_info) {
  ...
  if (atr_info->T1 > ISO7816_ATR_T1_COMPACT_TLV && atr_info->T1 <= 0x8F) {
    return "RFU";
  }
