...
enum emv_debug_source_t {
  EMV_DEBUG_SOURCE_NONE =
      0x00, ///< Source: None. Can be passed to @ref emv_debug_init().
  ...
  EMV_DEBUG_SOURCE_ALL =
      0xFF, ///< Source: All. Can be passed to @ref emv_debug_init().
};
...
enum emv_debug_type_t {
  EMV_DEBUG_TYPE_MSG =
      1, ///< Debug event contains only a string message and no data
  EMV_DEBUG_TYPE_DATA,  ///< Debug event contains binary data
  EMV_DEBUG_TYPE_TLV,   ///< Debug event contains ISO 8825-1 BER encoded data
  EMV_DEBUG_TYPE_ATR,   ///< Debug event contains ISO 7816 Answer To Reset (ATR)
                        ///< data
  EMV_DEBUG_TYPE_CAPDU, ///< Debug event contains ISO 7816 C-APDU (command APDU)
                        ///< data
  EMV_DEBUG_TYPE_RAPDU, ///< Debug event contains ISO 7816 R-APDU (response
                        ///< APDU) data
  EMV_DEBUG_TYPE_CTPDU, ///< Debug event contains ISO 7816 C-TPDU (request TPDU)
                        ///< data
  EMV_DEBUG_TYPE_RTPDU, ///< Debug event contains ISO 7816 R-TPDU (response
                        ///< TPDU) data
};
 ...
 */
typedef void (*emv_debug_func_t)(unsigned int timestamp,
                                 enum emv_debug_source_t source,
                                 enum emv_debug_level_t level,
                                 enum emv_debug_type_t debug_type,
                                 const char *str, const void *buf,
                                 size_t buf_len);

/**
 * Initialise EMV debug event function
 * @param sources_mask Bitmask of debug sources to pass to event function. See
 * @ref emv_debug_source_t
 * @param level Maximum debug level event to pass to event function
 * @param func Callback function to use for debug events
 */
int emv_debug_init(unsigned int sources_mask, enum emv_debug_level_t level,
                   emv_debug_func_t func);

/**
 * Internal EMV debugging implementation used by macros. Callers should use the
 * macros instead.
 * @param source Debug event source
 * @param level Debug event level
 * @param debug_type Debug event type
 ...
 */
void emv_debug_internal(enum emv_debug_source_t source,
                        enum emv_debug_level_t level,
                        enum emv_debug_type_t debug_type, const char *fmt,
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_error(fmt, ...)                                              \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_ERROR, EMV_DEBUG_TYPE_MSG,  \
                       fmt, NULL, 0, ##__VA_ARGS__);                           \
  } while (0)
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_info(fmt, ...)                                               \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_INFO, EMV_DEBUG_TYPE_MSG,   \
                       fmt, NULL, 0, ##__VA_ARGS__);                           \
  } while (0)
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_info_data(fmt, buf, buf_len, ...)                            \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_INFO, EMV_DEBUG_TYPE_DATA,  \
                       fmt, buf, buf_len, ##__VA_ARGS__);                      \
  } while (0)
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_info_tlv(fmt, buf, buf_len, ...)                             \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_INFO, EMV_DEBUG_TYPE_TLV,   \
                       fmt, buf, buf_len, ##__VA_ARGS__);                      \
  } while (0)
 ...
 */
#define emv_debug_atr_info(atr_info)                                           \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_CARD, EMV_DEBUG_TYPE_ATR,   \
                       "ATR", atr_info, sizeof(*atr_info));                    \
  } while (0)
...
#if defined(EMV_DEBUG_ENABLED) && !defined(EMV_DEBUG_CARD_DISABLED) &&         \
    !defined(EMV_DEBUG_APDU_DISABLED)
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_apdu(fmt, ...)                                               \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_CARD, EMV_DEBUG_TYPE_MSG,   \
                       fmt, NULL, 0, ##__VA_ARGS__);                           \
  } while (0)
 ...
 */
#define emv_debug_capdu(buf, buf_len)                                          \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_CARD, EMV_DEBUG_TYPE_CAPDU, \
                       "C-APDU", buf, buf_len);                                \
  } while (0)
 ...
 */
#define emv_debug_rapdu(buf, buf_len)                                          \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_CARD, EMV_DEBUG_TYPE_RAPDU, \
                       "R-APDU", buf, buf_len);                                \
  } while (0)

#else // EMV_DEBUG_ENABLED && !EMV_DEBUG_CARD_DISABLED &&
      // !EMV_DEBUG_APDU_DISABLED
#define emv_debug_apdu(fmt, ...)                                               \
  do {                                                                         \
  } while (0)
#define emv_debug_capdu(buf, buf_len)                                          \
...
#if defined(EMV_DEBUG_ENABLED) && !defined(EMV_DEBUG_CARD_DISABLED) &&         \
    !defined(EMV_DEBUG_TPDU_DISABLED)
 ...
 */
#define emv_debug_ctpdu(buf, buf_len)                                          \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_CARD, EMV_DEBUG_TYPE_CTPDU, \
                       "C-TPDU", buf, buf_len);                                \
  } while (0)
 ...
 */
#define emv_debug_rtpdu(buf, buf_len)                                          \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_CARD, EMV_DEBUG_TYPE_RTPDU, \
                       "R-TPDU", buf, buf_len);                                \
  } while (0)

#else // EMV_DEBUG_ENABLED && !EMV_DEBUG_CARD_DISABLED &&
      // !EMV_DEBUG_TPDU_DISABLED
#define emv_debug_ctpdu(buf, buf_len)                                          \
  do {                                                                         \
  } while (0)
#define emv_debug_rtpdu(buf, buf_len)                                          \
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_trace_msg(fmt, ...)                                          \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_TRACE, EMV_DEBUG_TYPE_MSG,  \
                       "%s[%u]: " fmt, NULL, 0, __FILE__, __LINE__,            \
                       ##__VA_ARGS__);                                         \
  } while (0)
 ...
 * @param ... Variable arguments for @c fmt
 */
#define emv_debug_trace_data(fmt, buf, buf_len, ...)                           \
  do {                                                                         \
    emv_debug_internal(EMV_DEBUG_SOURCE, EMV_DEBUG_TRACE, EMV_DEBUG_TYPE_DATA, \
                       fmt, buf, buf_len, ##__VA_ARGS__);                      \
  } while (0)

#else // EMV_DEBUG_ENABLED && !EMV_DEBUG_TRACE_DISABLED
#define emv_debug_trace_msg(...)                                               \
  do {                                                                         \
  } while (0)
#define emv_debug_trace_data(...)                                              \
  do {                                                                         \
  } while (0)
