...
static void iso7816_str_list_init(struct str_itr_t *itr, char *buf,
                                  size_t len) {
  itr->ptr = buf;
  itr->len = len;
}
...
static const char *iso7816_capdu_select_get_string(const uint8_t *c_apdu,
  ...
  if ((c_apdu[0] & ISO7816_CLA_PROPRIETARY) != 0 || c_apdu[1] != 0xA4) {
    // Not SELECT
    return NULL;
  }
  ...
  if (c_apdu[2] == 0x00 && c_apdu[3] == 0x00) { // If P1 and P2 are zero
    ...
    if (apdu_case_2s ||
        (apdu_case_4s && apdu_case_4s->Lc == 2 &&
         apdu_case_4s->data[0] == 0x3F && apdu_case_4s->data[1] == 0x00)) {
  ...
  if (file_id_printable) {
    snprintf(file_id_str, sizeof(file_id_str), "%.*s", apdu_case_4s->Lc,
             apdu_case_4s->data);
  } else {
    for (size_t i = 0; i < apdu_case_4s->Lc; ++i) {
      snprintf(file_id_str + (i * 2), sizeof(file_id_str) - 1 - (i * 2), "%02X",
               apdu_case_4s->data[i]);
    }
  }
  ...
  // Stringify
  snprintf(str, str_len, "SELECT %s %s", occurence_str, file_id_str);

  return str;
}

static const char *iso7816_capdu_read_record_get_string(const uint8_t *c_apdu,
                                                        size_t c_apdu_len,
                                                        char *str,
                                                        size_t str_len) {
  enum iso7816_apdu_case_t apdu_case;
  uint8_t P1;
  uint8_t P2;
  char record_str[32]; // "current record" / "record number XX" / "record
                       // identifer XX"
  ...
  if (P2 & ISO7816_READ_RECORD_P2_RECORD_NUMBER) {
    ...
    case ISO7816_READ_RECORD_P2_RECORD_SEQUENCE_ONE:
      snprintf(str, str_len, "READ RECORD from short EF %u, %s", short_ef_id,
               record_str);
      return str;

    case ISO7816_READ_RECORD_P2_RECORD_SEQUENCE_P1_TO_LAST:
      snprintf(str, str_len,
               "READ RECORD from short EF %u, "
               "all records from %s up to last",
               short_ef_id, record_str);
      return str;
    ...
    snprintf(str, str_len,
             "READ RECORD from short EF %u, "
             "%s occurence of %s",
             short_ef_id, occurence_str, record_str);
    return str;
  }
}

static const char *iso7816_capdu_put_data_get_string(const uint8_t *c_apdu,
                                                     size_t c_apdu_len,
                                                     char *str,
                                                     size_t str_len) {
  enum iso7816_apdu_case_t apdu_case;
  ...
  if ((c_apdu[1] & 0x01) == 0) {
    ...
    // Odd INS code
    if (P1 == 0x00 &&
        (P2 & 0xE0) == 0x00 && // Highest 11 bits of P1-P2 are unset
        (P2 & 0x1F) != 0x00 &&
        (P2 & 0x1F) != 0x1F // Lowest 5 bits are not equal
    ) {
...
const char *iso7816_capdu_get_string(const void *c_apdu, size_t c_apdu_len,
  ...
  if ((c_apdu_hdr[0] & ISO7816_CLA_PROPRIETARY) == 0) {
    ...
    switch (c_apdu_hdr[1]) {
    case 0x04:
      ins_str = "DEACTIVATE FILE";
      break;
    case 0x0C:
      ins_str = "ERASE RECORD";
      break;
    case 0x0E:
    case 0x0F:
      ins_str = "ERASE BINARY";
      break;
    case 0x10:
      ins_str = "PERFORM SCQL OPERATION";
      break;
    case 0x12:
      ins_str = "PERFORM TRANSACTION OPERATION";
      break;
    case 0x14:
      ins_str = "PERFORM USER OPERATION";
      break;
    case 0x20:
    case 0x21:
      ins_str = "VERIFY";
      break;
    case 0x22:
      ins_str = "MANAGE SECURITY ENVIRONMENT";
      break;
    case 0x24:
      ins_str = "CHANGE REFERENCE DATA";
      break;
    case 0x26:
      ins_str = "DISABLE VERIFICATION REQUIREMENT";
      break;
    case 0x28:
      ins_str = "ENABLE VERIFICATION REQUIREMENT";
      break;
    case 0x2A:
      ins_str = "PERFORM SECURITY OPERATION";
      break;
    case 0x2C:
      ins_str = "RESET RETRY COUNTER";
      break;
    case 0x44:
      ins_str = "ACTIVATE FILE";
      break;
    case 0x46:
      ins_str = "GENERATE ASYMMETRIC KEY PAIR";
      break;
    case 0x70:
      ins_str = "MANAGE CHANNEL";
      break;
    case 0x82:
      ins_str = "EXTERNAL AUTHENTICATE";
      break;
    case 0x84:
      ins_str = "GET CHALLENGE";
      break;
    case 0x86:
    case 0x87:
      ins_str = "GENERAL AUTHENTICATE";
      break;
    case 0x88:
      ins_str = "INTERNAL AUTHENTICATE";
      break;
    case 0xA0:
    case 0xA1:
      ins_str = "SEARCH BINARY";
      break;
    case 0xA2:
      ins_str = "SEARCH RECORD";
      break;
    case 0xA4:
      return iso7816_capdu_select_get_string(c_apdu, c_apdu_len, str, str_len);
    case 0xB0:
    case 0xB1:
      ins_str = "READ BINARY";
      break;
    case 0xB2:
    case 0xB3:
      return iso7816_capdu_read_record_get_string(c_apdu, c_apdu_len, str,
                                                  str_len);
    case 0xC0:
      ins_str = "GET RESPONSE";
      break;
    case 0xC2:
    case 0xC3:
      ins_str = "ENVELOPE";
    ...
    case 0xD0:
    case 0xD1:
      ins_str = "WRITE BINARY";
      break;
    case 0xD2:
      ins_str = "WRITE RECORD";
      break;
    case 0xD6:
    case 0xD7:
      ins_str = "UPDATE BINARY";
      break;
    case 0xDA:
    case 0xDB:
      return iso7816_capdu_put_data_get_string(c_apdu, c_apdu_len, str,
                                               str_len);
    case 0xDC:
    case 0xDD:
      ins_str = "UPDATE RECORD";
      break;
    case 0xE0:
      ins_str = "CREATE FILE";
      break;
    case 0xE2:
      ins_str = "APPEND RECORD";
      break;
    case 0xE4:
      ins_str = "DELETE FILE";
      break;
    case 0xE6:
      ins_str = "TERMINATE DF";
      break;
    case 0xE8:
      ins_str = "TERMINATE EF";
      break;
    case 0xFE:
      ins_str = "TERMINATE CARD USAGE";
      break;
...
const char *iso7816_sw1sw2_get_string(uint8_t SW1, uint8_t SW2, char *str,
                                      size_t str_len) {
  int r;
  char *str_ptr = str;
  ...
  if (((SW1 == 0x67 || SW1 == 0x6B || SW1 == 0x6D || SW1 == 0x6E ||
        SW1 == 0x6F) &&
       SW2 != 0x00) ||
  ...
  if (SW1 == 0x62) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "State of non-volatile memory is unchanged");
      break;
    case 0x81:
      snprintf(str_ptr, str_len, "Part of returned data may be corrupted");
      break;
    case 0x82:
      ...
      break;
    case 0x83:
      snprintf(str_ptr, str_len, "Selected file deactivated");
      break;
    case 0x84:
      snprintf(str_ptr, str_len,
               "File control information not formatted according to ISO "
               "7816-4:2005, 5.3.3");
      break;
    case 0x85:
      snprintf(str_ptr, str_len, "Selected file in termination state");
      break;
    case 0x86:
      snprintf(str_ptr, str_len,
  ...
  if (SW1 == 0x63) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "State of non-volatile memory has changed");
      break;
    case 0x81:
      snprintf(str_ptr, str_len, "File filled up by the last write");
      break;
    default:
      if ((SW2 & 0xF0) == 0xC0) {
  ...
  if (SW1 == 0x64) {
    ...
    case 0x00:
      ...
      break;
    case 0x01:
      snprintf(str_ptr, str_len, "Immediate response required by card");
      break;
    default:
      // Card-originated queries (see ISO 7816-4:2005, 8.6.1)
      if (SW2 >= 0x02 && SW2 <= 0x80) {
  ...
  if (SW1 == 0x65) {
    ...
    case 0x00:
      ...
      break;
    case 0x81:
      snprintf(str_ptr, str_len, "Memory failure");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x67) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "Wrong length");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x68) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "Functions in CLA not supported");
      break;
    case 0x81:
      snprintf(str_ptr, str_len, "Logical channel not supported");
      break;
    case 0x82:
      snprintf(str_ptr, str_len, "Secure messaging not supported");
      break;
    case 0x83:
      snprintf(str_ptr, str_len, "Last command of the chain expected");
      break;
    case 0x84:
      snprintf(str_ptr, str_len, "Command chaining not supported");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x69) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "Command not allowed");
      break;
    case 0x81:
      snprintf(str_ptr, str_len, "Command incompatible with file structure");
      break;
    case 0x82:
      snprintf(str_ptr, str_len, "Security status not satisfied");
      break;
    case 0x83:
      snprintf(str_ptr, str_len, "Authentication method blocked");
      break;
    case 0x84:
      snprintf(str_ptr, str_len, "Reference data not usable");
      break;
    case 0x85:
      snprintf(str_ptr, str_len, "Conditions of use not satisfied");
      break;
    case 0x86:
      snprintf(str_ptr, str_len, "Command not allowed (no current EF)");
      break;
    case 0x87:
      snprintf(str_ptr, str_len,
      ...
      break;
    case 0x88:
      snprintf(str_ptr, str_len, "Incorrect secure messaging data objects");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x6A) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "Wrong parameters P1-P2");
      break;
    case 0x80:
      snprintf(str_ptr, str_len,
      ...
      break;
    case 0x81:
      snprintf(str_ptr, str_len, "Function not supported");
      break;
    case 0x82:
      snprintf(str_ptr, str_len, "File or application not found");
      break;
    case 0x83:
      snprintf(str_ptr, str_len, "Record not found");
      break;
    case 0x84:
      snprintf(str_ptr, str_len, "Not enough memory space in the file");
      break;
    case 0x85:
      snprintf(str_ptr, str_len, "Nc inconsistent with TLV structure");
      break;
    case 0x86:
      snprintf(str_ptr, str_len, "Incorrect parameters P1-P2");
      break;
    case 0x87:
      snprintf(str_ptr, str_len, "Nc inconsistent with parameters P1-P2");
      break;
    case 0x88:
      snprintf(str_ptr, str_len, "Referenced data or reference data not found");
      break;
    case 0x89:
      snprintf(str_ptr, str_len, "File already exists");
      break;
    case 0x8A:
      snprintf(str_ptr, str_len, "DF name already exists");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x6D) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "Instruction code not supported or invalid");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x6E) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "Class not supported");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
  ...
  if (SW1 == 0x6F) {
    switch (SW2) {
    case 0x00:
      snprintf(str_ptr, str_len, "No precise diagnosis");
      break;
    default:
      snprintf(str_ptr, str_len, "Unknown");
      break;
...
int iso7816_card_service_data_get_string_list(uint8_t card_service_data,
                                              char *str, size_t str_len) {
  ...
  // BER-TLV data objects availability (see ISO 7816-4:2005, 8.1.1.2.3, table
  // 85)
  ...
  // EF.DIR and EF.ATR access services (see ISO 7816-4:2005, 8.1.1.2.3, table
  // 85)
  switch ((card_service_data & ISO7816_CARD_SERVICE_ACCESS_MASK)) {
  case ISO7816_CARD_SERVICE_ACCESS_READ_BINARY:
    iso7816_str_list_add(&itr, "EF.DIR and EF.ATR access services: by READ "
                               "BINARY command (transparent structure)");
    break;

  case ISO7816_CARD_SERVICE_ACCESS_READ_RECORD:
    iso7816_str_list_add(&itr, "EF.DIR and EF.ATR access services: by READ "
                               "RECORD(S) command (record structure)");
    break;

  case ISO7816_CARD_SERVICE_ACCESS_GET_DATA:
    iso7816_str_list_add(&itr, "EF.DIR and EF.ATR access services: by GET DATA "
                               "command (TLV structure)");
    break;

  default:
    iso7816_str_list_add(&itr,
                         "EF.DIR and EF.ATR access services: Unknown value");
    break;
  }
...
int iso7816_card_capabilities_get_string_list(const uint8_t *card_capabilities,
  ...
  case ISO7816_CARD_CAPS_BER_TLV_FF_INVALID:
    iso7816_str_list_add(
        &itr, "FF as first byte of BER-TLV tag is invalid / padding");
    break;
  ...
  data_unit_size_field =
      card_capabilities[1] & ISO7816_CARD_CAPS_DATA_UNIT_SIZE_MASK;
  ...
  // Command chaining, length fields, logical channels (see ISO
  // 7816-4:2005, 8.1.1.2.7, table 87)
  ...
  case ISO7816_CARD_CAPS_CHAN_NUM_ASSIGN_IFD:
    iso7816_str_list_add(
        &itr, "Logical channel number assignment: by the interface device");
    break;
  ...
  if (max_logical_channels == 0x7) {
    ...
    // Stringify
    char tmp[64];
    snprintf(tmp, sizeof(tmp), "Maximum number of logical channels: %u",
