...
#define PCSC_FEATURE_IFD_PIN_PROPERTIES                                        \
  (0x0A) ///< Interface Device (IFD) PIN handling properties
#define PCSC_FEATURE_IFD_DISPLAY_PROPERTIES                                    \
  (0x11) ///< Interface Device (IFD) display properties
#define PCSC_FEATURE_GET_TLV_PROPERTIES                                        \
  (0x12) ///< Interface Device (IFD) properties in Tag-Length-Value (TLV) form
...
typedef struct {
  ...
  uint8_t length;
  uint32_t value;
} __attribute__((packed)) PCSC_TLV_STRUCTURE;
...
typedef struct {
  ...
  uint8_t bTimeOut2;
} __attribute__((packed)) PIN_PROPERTIES_STRUCTURE;
...
} __attribute__((packed)) DISPLAY_PROPERTIES_STRUCTURE;
