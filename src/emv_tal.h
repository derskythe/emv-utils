...
enum emv_tal_error_t {
  ...
  EMV_TAL_ERROR_GPO_PARSE_FAILED =
      -6, ///< Failed to parse GET PROCESSING OPTIONS response
  EMV_TAL_ERROR_GPO_FIELD_NOT_FOUND =
      -7, ///< Failed to find mandatory field in GET PROCESSING OPTIONS response
  EMV_TAL_ERROR_AFL_INVALID = -8, ///< Application File Locator (AFL) is invalid
  EMV_TAL_ERROR_READ_RECORD_FAILED = -9, ///< READ RECORD failed
  EMV_TAL_ERROR_READ_RECORD_INVALID =
      -10, ///< READ RECORD provided invalid record
  EMV_TAL_ERROR_READ_RECORD_PARSE_FAILED =
      -11, ///< Faild to parse READ RECORD response
};
...
enum emv_tal_result_t {
  EMV_TAL_RESULT_PSE_NOT_FOUND =
      1,                      ///< Payment System Environment (PSE) not found
  EMV_TAL_RESULT_PSE_BLOCKED, ///< Payment System Environment (PSE) is blocked
  EMV_TAL_RESULT_PSE_SELECT_FAILED,    ///< Failed to select Payment System
                                       ///< Environment (PSE)
  EMV_TAL_RESULT_PSE_FCI_PARSE_FAILED, ///< Failed to parse File Control
                                       ///< Information (FCI) for Payment System
                                       ///< Environment (PSE)
  EMV_TAL_RESULT_PSE_SFI_NOT_FOUND,    ///< Failed to find Short File Identifier
                                       ///< (SFI) for Payment System Environment
                                       ///< (PSE)
  EMV_TAL_RESULT_PSE_SFI_INVALID, ///< Invalid Short File Identifier (SFI) for
                                  ///< Payment System Environment (PSE)
  EMV_TAL_RESULT_PSE_AEF_PARSE_FAILED, ///< Failed to parse Application
                                       ///< Elementary File (AEF) of Payment
                                       ///< System Environment (PSE)
  EMV_TAL_RESULT_PSE_AEF_INVALID, ///< Invalid Payment System Environment (PSE)
                                  ///< Application Elementary File (AEF) record
  ...
  EMV_TAL_RESULT_APP_FCI_PARSE_FAILED, ///< Failed to parse File Control
                                       ///< Information (FCI) for selected
                                       ///< application
  EMV_TAL_RESULT_GPO_CONDITIONS_NOT_SATISFIED, ///< Conditions of use not
                                               ///< satisfied for selected
                                               ///< application
  EMV_TAL_RESULT_ODA_RECORD_INVALID, ///< Offline data authentication not
                                     ///< possible due to an invalid record
};
