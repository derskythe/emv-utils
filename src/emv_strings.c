...
#define ATTRIBUTE_FORMAT_PRINTF(str_idx, va_idx)                               \
  __attribute__((format(printf, str_idx, va_idx)))
#elif defined(__GNUC__)
// Use gnu_printf for GCC and MSYS2's MinGW
#define ATTRIBUTE_FORMAT_PRINTF(str_idx, va_idx)                               \
  __attribute__((format(gnu_printf, str_idx, va_idx)))
#else
...
// Helper functions
static int emv_tlv_value_get_string(const struct emv_tlv_t *tlv,
                                    enum emv_format_t format,
                                    size_t max_format_len, char *value_str,
...
static int emv_country_alpha2_code_get_string(const uint8_t *buf,
                                              size_t buf_len, char *str,
                                              size_t str_len);
static int emv_country_alpha3_code_get_string(const uint8_t *buf,
                                              size_t buf_len, char *str,
                                              size_t str_len);
static int emv_country_numeric_code_get_string(const uint8_t *buf,
                                               size_t buf_len, char *str,
                                               size_t str_len);
static int emv_currency_numeric_code_get_string(const uint8_t *buf,
                                                size_t buf_len, char *str,
                                                size_t str_len);
static int emv_language_alpha2_code_get_string(const uint8_t *buf,
                                               size_t buf_len, char *str,
                                               size_t str_len);
static const char *emv_cvm_code_get_string(uint8_t cvm_code);
static int
emv_cvm_cond_code_get_string(uint8_t cvm_cond_code,
                             const struct emv_cvmlist_amounts_t *amounts,
                             char *str, size_t str_len);
static int emv_iad_ccd_append_string_list(const uint8_t *iad, size_t iad_len,
                                          struct str_itr_t *itr);
static int emv_iad_mchip_append_string_list(const uint8_t *iad, size_t iad_len,
                                            struct str_itr_t *itr);
static int emv_iad_vsdc_0_1_3_append_string_list(const uint8_t *iad,
                                                 size_t iad_len,
                                                 struct str_itr_t *itr);
static int emv_iad_vsdc_2_4_append_string_list(const uint8_t *iad,
                                               size_t iad_len,
                                               struct str_itr_t *itr);
static const char *
emv_mastercard_device_type_get_string(const char *device_type);
static const char *emv_arc_get_desc(const char *arc);
static int emv_csu_append_string_list(const uint8_t *csu, size_t csu_len,
                                      struct str_itr_t *itr);
static int emv_capdu_get_data_get_string(const uint8_t *c_apdu,
                                         size_t c_apdu_len, char *str,
                                         size_t str_len);
...
int emv_tlv_get_info(const struct emv_tlv_t *tlv, struct emv_tlv_info_t *info,
  ...
  case EMV_TAG_42_IIN:
    ...
    return emv_tlv_value_get_string(tlv, info->format, 6, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_4F_APPLICATION_DF_NAME:
    ...
    return emv_aid_get_string(tlv->value, tlv->length, value_str,
                              value_str_len);
  ...
  case EMV_TAG_50_APPLICATION_LABEL:
    ...
    return emv_tlv_value_get_string(tlv, info->format, 16, value_str,
                                    value_str_len);

  case EMV_TAG_56_TRACK1_DATA:
    info->tag_name = "Track 1 Data";
    info->tag_desc = "Contains the data objects of the track 1 according to "
                     "ISO/IEC 7813 Structure B, excluding start sentinel, end "
                     "sentinel and Longitudinal Redundancy Check (LRC)";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 76, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_57_TRACK2_EQUIVALENT_DATA:
    ...
    info->format = EMV_FORMAT_B;
    return emv_track2_equivalent_data_get_string(tlv->value, tlv->length,
                                                 value_str, value_str_len);

  case EMV_TAG_5A_APPLICATION_PAN:
    info->tag_name = "Application Primary Account Number (PAN)";
    info->tag_desc = "Valid cardholder account number";
    info->format = EMV_FORMAT_CN;
    return emv_tlv_value_get_string(tlv, info->format, 19, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_6F_FCI_TEMPLATE:
    ...
    info->tag_desc = "Identifies the FCI template according to ISO/IEC 7816-4";
  ...
  case EMV_TAG_71_ISSUER_SCRIPT_TEMPLATE_1:
    ...
    info->tag_desc = "Contains proprietary issuer data for "
  ...
  case EMV_TAG_73_DIRECTORY_DISCRETIONARY_TEMPLATE:
    ...
    info->tag_desc = "Issuer discretionary part of the directory according to "
                     "ISO/IEC 7816-4";
    info->format = EMV_FORMAT_VAR;
    return 0;
  ...
  case EMV_TAG_80_RESPONSE_MESSAGE_TEMPLATE_FORMAT_1:
    ...
    info->tag_desc = "Contains the data objects (without tags and lengths) "
  ...
  case EMV_TAG_81_AMOUNT_AUTHORISED_BINARY:
    ...
    info->format = EMV_FORMAT_B;
    return emv_amount_get_string(tlv->value, tlv->length, value_str,
                                 value_str_len);
  ...
  case EMV_TAG_82_APPLICATION_INTERCHANGE_PROFILE:
    ...
    info->format = EMV_FORMAT_B;
    return emv_aip_get_string_list(tlv->value, tlv->length, value_str,
                                   value_str_len);

  case EMV_TAG_83_COMMAND_TEMPLATE:
    info->tag_name = "Command Template";
    info->tag_desc = "Identifies the data field of a command message";
    info->format = EMV_FORMAT_VAR;
    return 0;
  ...
  case EMV_TAG_84_DF_NAME:
    ...
    info->tag_desc =
        ...
        "in ISO/IEC 7816-4";
    info->format = EMV_FORMAT_B;
    if ((tlv->length == strlen(EMV_PSE) &&
         strncmp((const char *)tlv->value, EMV_PSE, strlen(EMV_PSE))) ||
        (tlv->length == strlen(EMV_PPSE) &&
         strncmp((const char *)tlv->value, EMV_PPSE, strlen(EMV_PPSE)))) {
  ...
  case EMV_TAG_86_ISSUER_SCRIPT_COMMAND:
    ...
    info->format = EMV_FORMAT_VAR;
    return emv_capdu_get_string(tlv->value, tlv->length, value_str,
                                value_str_len);
  ...
  case EMV_TAG_89_AUTHORISATION_CODE:
    ...
    info->tag_desc = "Value generated by the authorisation authority "
  ...
  case EMV_TAG_8A_AUTHORISATION_RESPONSE_CODE:
    ...
    return emv_auth_response_code_get_string(tlv->value, tlv->length, value_str,
                                             value_str_len);
  ...
  case EMV_TAG_8E_CVM_LIST:
    ...
    info->tag_desc = "Identifies a method of verification of the cardholder "
                     "supported by the application";
    info->format = EMV_FORMAT_B;
    return emv_cvm_list_get_string_list(tlv->value, tlv->length, value_str,
                                        value_str_len);

  case EMV_TAG_8F_CERTIFICATION_AUTHORITY_PUBLIC_KEY_INDEX:
    info->tag_name = "Certification Authority Public Key (CAPK) Index";
    info->tag_desc = "Identifies the certification authority's public key in "
  ...
  case EMV_TAG_90_ISSUER_PUBLIC_KEY_CERTIFICATE:
    ...
    info->tag_desc = "Issuer public key certified by a certification authority";
    info->format = EMV_FORMAT_B;
    return 0;

  case EMV_TAG_91_ISSUER_AUTHENTICATION_DATA:
    info->tag_name = "Issuer Authentication Data";
    info->tag_desc = "Data sent to the ICC for online issuer authentication";
    info->format = EMV_FORMAT_B;
    return emv_issuer_auth_data_get_string_list(tlv->value, tlv->length,
                                                value_str, value_str_len);

  case EMV_TAG_92_ISSUER_PUBLIC_KEY_REMAINDER:
    info->tag_name = "Issuer Public Key Remainder";
    info->tag_desc = "Remaining digits of the Issuer Public Key Modulus";
    info->format = EMV_FORMAT_B;
    return 0;

  case EMV_TAG_93_SIGNED_STATIC_APPLICATION_DATA:
    info->tag_name = "Signed Static Application Data";
    info->tag_desc = "Digital signature on critical application "
                     "parameters for SDA";
    info->format = EMV_FORMAT_B;
    return 0;
  ...
  case EMV_TAG_94_APPLICATION_FILE_LOCATOR:
    ...
    info->tag_desc = "Indicates the location (SFI, range of records) of the "
                     "Application Elementary Files (AEFs) related to a given "
                     "application";
    info->format = EMV_FORMAT_VAR;
    return emv_afl_get_string_list(tlv->value, tlv->length, value_str,
                                   value_str_len);
  ...
  case EMV_TAG_95_TERMINAL_VERIFICATION_RESULTS:
    ...
    info->format = EMV_FORMAT_B;
    return emv_tvr_get_string_list(tlv->value, tlv->length, value_str,
                                   value_str_len);

  case EMV_TAG_9A_TRANSACTION_DATE:
    info->tag_name = "Transaction Date";
    info->tag_desc = "Local date that the transaction was authorised";
    info->format = EMV_FORMAT_N;
    return emv_date_get_string(tlv->value, tlv->length, value_str,
                               value_str_len);

  case EMV_TAG_9B_TRANSACTION_STATUS_INFORMATION:
    info->tag_name = "Transaction Status Information (TSI)";
    info->tag_desc = "Indicates the functions performed in a transaction";
    info->format = EMV_FORMAT_B;
    return emv_tsi_get_string_list(tlv->value, tlv->length, value_str,
                                   value_str_len);
  ...
  case EMV_TAG_9C_TRANSACTION_TYPE:
    ...
    return emv_transaction_type_get_string(tlv->value[0], value_str,
                                           value_str_len);
  ...
  case EMV_TAG_5F20_CARDHOLDER_NAME:
    ...
    info->tag_desc = "Indicates cardholder name according to ISO 7813";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 26, value_str,
                                    value_str_len);

  case EMV_TAG_5F24_APPLICATION_EXPIRATION_DATE:
    info->tag_name = "Application Expiration Date";
    info->tag_desc = "Date after which application expires";
    info->format = EMV_FORMAT_N;
    return emv_date_get_string(tlv->value, tlv->length, value_str,
                               value_str_len);
  ...
  case EMV_TAG_5F25_APPLICATION_EFFECTIVE_DATE:
    ...
    info->tag_desc = "Date from which the application may be used";
    info->format = EMV_FORMAT_N;
    return emv_date_get_string(tlv->value, tlv->length, value_str,
                               value_str_len);
  ...
  case EMV_TAG_5F28_ISSUER_COUNTRY_CODE:
    ...
    info->format = EMV_FORMAT_N;
    return emv_country_numeric_code_get_string(tlv->value, tlv->length,
                                               value_str, value_str_len);
  ...
  case EMV_TAG_5F2A_TRANSACTION_CURRENCY_CODE:
    ...
    info->tag_desc =
        ...
        "ISO 4217";
    info->format = EMV_FORMAT_N;
    return emv_currency_numeric_code_get_string(tlv->value, tlv->length,
                                                value_str, value_str_len);

  case EMV_TAG_5F2D_LANGUAGE_PREFERENCE:
    info->tag_name = "Language Preference";
    info->tag_desc = "1-4 languages stored in order of preference, each "
                     "represented by 2 alphabetical characters according to "
                     "ISO 639";
    info->format = EMV_FORMAT_AN;
    return emv_language_preference_get_string_list(tlv->value, tlv->length,
                                                   value_str, value_str_len);

  case EMV_TAG_5F30_SERVICE_CODE:
    info->tag_name = "Service Code";
    info->tag_desc = "Service code as defined in ISO/IEC 7813 for "
                     "track 1 and track 2";
    info->format = EMV_FORMAT_N;
    return emv_tlv_value_get_string(tlv, info->format, 3, value_str,
                                    value_str_len);

  case EMV_TAG_5F34_APPLICATION_PAN_SEQUENCE_NUMBER:
    info->tag_name = "Application Primary Account Number (PAN) Sequence Number";
    info->tag_desc = "Identifies and differentiates cards with the same PAN";
    info->format = EMV_FORMAT_N;
    return 0;
  ...
  case EMV_TAG_5F50_ISSUER_URL:
    ...
    info->tag_desc =
        ...
        "on the Internet";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 0, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_5F53_IBAN:
    ...
    // format 'an'.
    return emv_tlv_value_get_string(tlv, EMV_FORMAT_AN, 34, value_str,
                                    value_str_len);

  case EMV_TAG_5F54_BANK_IDENTIFIER_CODE:
    info->tag_name = "Bank Identifier Code (BIC)";
    info->tag_desc = "Uniquely identifies a bank as defined in ISO 9362.";
    info->format = EMV_FORMAT_VAR;
    return emv_tlv_value_get_string(tlv, EMV_FORMAT_AN, 11, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_5F55_ISSUER_COUNTRY_CODE_ALPHA2:
    ...
    r = emv_country_alpha2_code_get_string(tlv->value, tlv->length, value_str,
                                           value_str_len);
    if (r || (value_str && value_str_len && !value_str[0])) {
      return emv_tlv_value_get_string(tlv, info->format, 2, value_str,
                                      value_str_len);
    }
    return 0;
  ...
  case EMV_TAG_5F56_ISSUER_COUNTRY_CODE_ALPHA3:
    ...
    info->format = EMV_FORMAT_A;
    // Lookup country code; fallback to format "a" string
    r = emv_country_alpha3_code_get_string(tlv->value, tlv->length, value_str,
                                           value_str_len);
    if (r || (value_str && value_str_len && !value_str[0])) {
      return emv_tlv_value_get_string(tlv, info->format, 3, value_str,
                                      value_str_len);
  ...
  case EMV_TAG_5F57_ACCOUNT_TYPE:
    ...
    info->tag_desc = "Indicates the type of account selected on the "
  ...
  case EMV_TAG_9F01_ACQUIRER_IDENTIFIER:
    ...
    info->format = EMV_FORMAT_N;
    return emv_tlv_value_get_string(tlv, info->format, 11, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_9F02_AMOUNT_AUTHORISED_NUMERIC:
    ...
    info->tag_desc =
        "Authorised amount of the transaction (excluding adjustments)";
    info->format = EMV_FORMAT_N;
    return emv_tlv_value_get_string(tlv, info->format, 12, value_str,
                                    value_str_len);

  case EMV_TAG_9F03_AMOUNT_OTHER_NUMERIC:
    info->tag_name = "Amount, Other (Numeric)";
    info->tag_desc = "Secondary amount associated with the transaction "
  ...
  case EMV_TAG_9F05_APPLICATION_DISCRETIONARY_DATA:
    ...
    info->tag_desc = "Issuer or payment system specified data "
  ...
  case EMV_TAG_9F07_APPLICATION_USAGE_CONTROL:
    ...
    info->format = EMV_FORMAT_B;
    return emv_app_usage_control_get_string_list(tlv->value, tlv->length,
                                                 value_str, value_str_len);

  case EMV_TAG_9F08_APPLICATION_VERSION_NUMBER:
    info->tag_name = "Application Version Number";
    info->tag_desc = "Version number assigned by the payment system for the "
                     "application";
    info->format = EMV_FORMAT_B;
    return 0;
  ...
  case EMV_TAG_9F0A_ASRPD:
    ...
    info->tag_desc =
        ...
        "registration.";
    info->format = EMV_FORMAT_B;
    return emv_asrpd_get_string_list(tlv->value, tlv->length, value_str,
                                     value_str_len);

  case EMV_TAG_9F0B_CARDHOLDER_NAME_EXTENDED:
    info->tag_name = "Cardholder Name Extended";
    info->tag_desc = "Indicates the whole cardholder name when "
                     "greater than 26 characters using the same "
                     "coding convention as in ISO/IEC 7813";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 45, value_str,
                                    value_str_len);

  case EMV_TAG_9F0C_IINE:
    info->tag_name = "Issuer Identification Number Extended (IINE)";
    info->tag_desc = "The number that identifies the major industry "
                     ...
                     "objects present.";
    info->format = EMV_FORMAT_N;
    return emv_tlv_value_get_string(tlv, info->format, 8, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_9F10_ISSUER_APPLICATION_DATA:
    ...
    info->format = EMV_FORMAT_B;
    return emv_iad_get_string_list(tlv->value, tlv->length, value_str,
                                   value_str_len);

  case EMV_TAG_9F11_ISSUER_CODE_TABLE_INDEX:
    info->tag_name = "Issuer Code Table Index";
    info->tag_desc = "Indicates the code table according to ISO/IEC 8859 for "
  ...
  case EMV_TAG_9F12_APPLICATION_PREFERRED_NAME:
    ...
    info->tag_desc = "Preferred mnemonic associated with the AID";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 16, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_9F13_LAST_ONLINE_ATC_REGISTER:
    ...
    info->tag_desc = "Application Transaction Counter (ATC) "
  ...
  case EMV_TAG_9F14_LOWER_CONSECUTIVE_OFFLINE_LIMIT:
    ...
    info->tag_desc = "Issuer-specified preference for the maximum "
  ...
  case EMV_TAG_9F15_MCC:
    ...
    info->format = EMV_FORMAT_N;
    return emv_mcc_get_string(tlv->value, tlv->length, value_str,
                              value_str_len);

  case EMV_TAG_9F16_MERCHANT_IDENTIFIER:
    info->tag_name = "Merchant Identifier";
    info->tag_desc = "When concatenated with the Acquirer Identifier, uniquely "
                     "identifies a given merchant";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 15, value_str,
                                    value_str_len);

  case EMV_TAG_9F17_PIN_TRY_COUNTER:
    info->tag_name = "Personal Identification Number (PIN) Try Counter";
  ...
  case EMV_TAG_9F19_TOKEN_REQUESTOR_ID:
    ...
    info->tag_desc = "Uniquely identifies the pairing of the Token "
  ...
  case EMV_TAG_9F1B_TERMINAL_FLOOR_LIMIT:
    ...
    info->tag_desc = "Indicates the floor limit in the terminal in conjunction "
                     "with the AID";
    info->format = EMV_FORMAT_B;
    return emv_amount_get_string(tlv->value, tlv->length, value_str,
                                 value_str_len);
  ...
  case EMV_TAG_9F1D_TERMINAL_RISK_MANAGEMENT_DATA:
    ...
    info->tag_desc =
        ...
        "must be set to zero.";
    info->format = EMV_FORMAT_B;
    return emv_terminal_risk_management_data_get_string_list(
        tlv->value, tlv->length, value_str, value_str_len);
  ...
  case EMV_TAG_9F1F_TRACK1_DISCRETIONARY_DATA:
    ...
    info->tag_desc = "Discretionary part of track 1 according to ISO/IEC 7813";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 0, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_9F20_TRACK2_DISCRETIONARY_DATA:
    ...
    info->tag_desc = "Discretionary part of track 2 according to ISO/IEC 7813";
    info->format = EMV_FORMAT_CN;
    return emv_tlv_value_get_string(tlv, info->format, 0, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_9F21_TRANSACTION_TIME:
    ...
    info->tag_desc = "Local time that the transaction was authorised";
    info->format = EMV_FORMAT_N;
    return emv_time_get_string(tlv->value, tlv->length, value_str,
                               value_str_len);
  ...
  case EMV_TAG_9F24_PAYMENT_ACCOUNT_REFERENCE:
    ...
    info->tag_desc = "A non-financial reference assigned to each "
                     ...
                     "issuance.";
    info->format = EMV_FORMAT_AN;
    return emv_tlv_value_get_string(tlv, info->format, 29, value_str,
                                    value_str_len);

  case EMV_TAG_9F25_LAST_4_DIGITS_OF_PAN:
    info->tag_name = "Last 4 Digits of PAN";
    info->tag_desc = "The last four digits of the PAN, as defined in "
                     "the EMV Payment Tokenisation Framework";
    info->format = EMV_FORMAT_N;
    return emv_tlv_value_get_string(tlv, info->format, 4, value_str,
                                    value_str_len);

  case EMV_TAG_9F26_APPLICATION_CRYPTOGRAM:
    info->tag_name = "Application Cryptogram";
    info->tag_desc = "Cryptogram returned by the ICC in response of the "
                     "GENERATE AC command";
    info->format = EMV_FORMAT_B;
    return 0;
  ...
  case EMV_TAG_9F27_CRYPTOGRAM_INFORMATION_DATA:
    ...
    info->tag_desc = "Indicates the type of cryptogram and the actions to be "
  ...
  case EMV_TAG_9F33_TERMINAL_CAPABILITIES:
    ...
    info->tag_desc = "Indicates the card data input, CVM, and security "
                     "capabilities of the terminal";
    info->format = EMV_FORMAT_B;
    return emv_term_caps_get_string_list(tlv->value, tlv->length, value_str,
                                         value_str_len);

  case EMV_TAG_9F34_CVM_RESULTS:
    info->tag_name = "Cardholder Verification Method (CVM) Results";
    info->tag_desc = "Indicates the results of the last CVM performed";
    info->format = EMV_FORMAT_B;
    return emv_cvm_results_get_string_list(tlv->value, tlv->length, value_str,
                                           value_str_len);

  case EMV_TAG_9F35_TERMINAL_TYPE:
    info->tag_name = "Terminal Type";
    info->tag_desc = "Indicates the environment of the terminal, its "
    ...
    return emv_term_type_get_string_list(tlv->value[0], value_str,
                                         value_str_len);

  case EMV_TAG_9F36_APPLICATION_TRANSACTION_COUNTER:
    info->tag_name = "Application Transaction Counter (ATC)";
    info->tag_desc = "Counter maintained by the application in the ICC "
  ...
  case EMV_TAG_9F37_UNPREDICTABLE_NUMBER:
    ...
    info->tag_desc = "Value to provide variability and uniqueness to the "
  ...
  case EMV_TAG_9F39_POS_ENTRY_MODE:
    ...
    return emv_pos_entry_mode_get_string(tlv->value[0], value_str,
                                         value_str_len);

  case EMV_TAG_9F3A_AMOUNT_REFERENCE_CURRENCY:
    info->tag_name = "Amount, Reference Currency";
    info->tag_desc = "Authorised amount expressed in the reference currency";
    info->format = EMV_FORMAT_B;
    return emv_amount_get_string(tlv->value, tlv->length, value_str,
                                 value_str_len);
  ...
  case EMV_TAG_9F3B_APPLICATION_REFERENCE_CURRENCY:
    ...
    info->tag_desc =
        ...
        "to ISO 4217";
    info->format = EMV_FORMAT_N;
    return emv_app_reference_currency_get_string_list(tlv->value, tlv->length,
                                                      value_str, value_str_len);
  ...
  case EMV_TAG_9F40_ADDITIONAL_TERMINAL_CAPABILITIES:
    ...
    info->tag_desc = "Indicates the data input and output capabilities of the "
                     "terminal";
    info->format = EMV_FORMAT_B;
    return emv_addl_term_caps_get_string_list(tlv->value, tlv->length,
                                              value_str, value_str_len);
  ...
  case EMV_TAG_9F42_APPLICATION_CURRENCY_CODE:
    ...
    info->tag_desc = "Indicates the currency in which the account is managed "
  ...
  case EMV_TAG_9F46_ICC_PUBLIC_KEY_CERTIFICATE:
    ...
    info->tag_desc = "ICC Public Key certified by the issuer";
    info->format = EMV_FORMAT_B;
    return 0;

  case EMV_TAG_9F47_ICC_PUBLIC_KEY_EXPONENT:
    info->tag_name = "Integrated Circuit Card (ICC) Public Key Exponent";
    info->tag_desc = "ICC Public Key Exponent used for the verification of the "
  ...
  case EMV_TAG_9F48_ICC_PUBLIC_KEY_REMAINDER:
    ...
    info->tag_desc = "Remaining digits of the ICC Public Key Modulus";
    info->format = EMV_FORMAT_B;
    return 0;
  ...
  case EMV_TAG_9F4E_MERCHANT_NAME_AND_LOCATION:
    ...
    info->tag_desc = "Indicates the name and location of the merchant";
    info->format = EMV_FORMAT_ANS;
    return emv_tlv_value_get_string(tlv, info->format, 0, value_str,
                                    value_str_len);
  ...
  case EMV_TAG_9F66_TTQ:
    if (tlv->length == 4) {
      ...
      info->format = EMV_FORMAT_B;
      return emv_ttq_get_string_list(tlv->value, tlv->length, value_str,
                                     value_str_len);
    }
  ...
  case EMV_TAG_9F6C_CTQ:
    ...
    info->format = EMV_FORMAT_B;
    return emv_ctq_get_string_list(tlv->value, tlv->length, value_str,
                                   value_str_len);
  ...
  case AMEX_TAG_9F6D_CONTACTLESS_READER_CAPABILITIES:
    if (tlv->length == 1) {
      ...
      info->tag_desc = "A proprietary data element with bits 8, 7, and 4 only "
      ...
      return emv_amex_cl_reader_caps_get_string(tlv->value[0], value_str,
                                                value_str_len);
    }
  ...
  case 0x9F6E: // Used for different purposes by different kernels
    if (tlv->tag ==
            MASTERCARD_TAG_9F6E_THIRD_PARTY_DATA && // Helps IDE find this case
                                                    // statement
        tlv->length > 4 &&
        tlv->length <= 32) {
      ...
      info->tag_desc = "The Third Party data object may be used to carry "
                       "specific product information to be optionally used by "
                       "the terminal in processing transactions.";
      info->format = EMV_FORMAT_B;
      return emv_mastercard_third_party_data_get_string_list(
          tlv->value, tlv->length, value_str, value_str_len);
    }

    if (tlv->tag ==
            VISA_TAG_9F6E_FORM_FACTOR_INDICATOR && // Helps IDE find this case
                                                   // statement
        tlv->length == 4 &&
        tlv->value &&
        (tlv->value[0] & VISA_FFI_VERSION_MASK) ==
            VISA_FFI_VERSION_NUMBER_1 && // VCPS only defines version number 1
        !tlv->value[2] && // VCPS indicates that byte 3 is RFU and should be
                          // zero'd
        tlv->value[3] ==
            VISA_FFI_PAYMENT_TXN_TECHNOLOGY_CONTACTLESS // VCPS only defines
                                                        // contactless
    ) {
      ...
      info->format = EMV_FORMAT_B;
      return emv_visa_form_factor_indicator_get_string_list(
          tlv->value, tlv->length, value_str, value_str_len);
    }

    if (tlv->tag ==
            AMEX_TAG_9F6E_ENHANCED_CONTACTLESS_READER_CAPABILITIES && // Helps
                                                                      // IDE
                                                                      // find
                                                                      // this
                                                                      // case
        ...
        (tlv->value[0] & AMEX_ENH_CL_READER_CAPS_FULL_ONLINE_MODE_SUPPORTED) ==
            0 &&
      ...
      info->format = EMV_FORMAT_B;
      return emv_amex_enh_cl_reader_caps_get_string_list(
          tlv->value, tlv->length, value_str, value_str_len);
    }
...
static int emv_tlv_value_get_string(const struct emv_tlv_t *tlv,
  ...
  switch (format) {
  case EMV_FORMAT_A:
    r = emv_format_a_get_string(tlv->value, tlv->length, value_str,
                                value_str_len);
    if (r) {
  ...
  case EMV_FORMAT_AN:
    r = emv_format_an_get_string(tlv->value, tlv->length, value_str,
                                 value_str_len);
    if (r) {
      // Empty string on error
      value_str[0] = 0;
  ...
  case EMV_FORMAT_ANS:
    if (tlv->tag == EMV_TAG_50_APPLICATION_LABEL) {
      r = emv_format_ans_only_space_get_string(tlv->value, tlv->length,
                                               value_str, value_str_len);
    } else if (tlv->tag == EMV_TAG_9F12_APPLICATION_PREFERRED_NAME) {
      // TODO: Convert EMV_TAG_9F12_APPLICATION_PREFERRED_NAME from the
      // appropriate ISO/IEC 8859 code page to UTF-8
      memcpy(value_str, tlv->value, tlv->length);
      value_str[tlv->length] = 0; // NULL terminate
      return 0;
    } else {
      r = emv_format_ans_ccs_get_string(tlv->value, tlv->length, value_str,
                                        value_str_len);
    }
    if (r) {
      // Empty string on error
  ...
  case EMV_FORMAT_CN: {
    r = emv_format_cn_get_string(tlv->value, tlv->length, value_str,
                                 value_str_len);
    if (r) {
      // Empty string on error
      value_str[0] = 0;
  ...
  case EMV_FORMAT_N: {
    r = emv_format_n_get_string(tlv->value, tlv->length, value_str,
                                value_str_len);
    if (r) {
      // Empty string on error
      value_str[0] = 0;
...
int emv_format_a_get_string(const uint8_t *buf, size_t buf_len, char *str,
                            size_t str_len) {
...
int emv_format_an_get_string(const uint8_t *buf, size_t buf_len, char *str,
                             size_t str_len) {
  if (!buf || !buf_len || !str || !str_len) {
    return -1;
  }
...
int emv_format_ans_only_space_get_string(const uint8_t *buf, size_t buf_len,
                                         char *str, size_t str_len) {
  if (!buf || !buf_len || !str || !str_len) {
    return -1;
  }
...
int emv_format_ans_ccs_get_string(const uint8_t *buf, size_t buf_len, char *str,
                                  size_t str_len) {
  if (!buf || !buf_len || !str || !str_len) {
    return -1;
  }
...
int emv_format_cn_get_string(const uint8_t *buf, size_t buf_len, char *str,
                             size_t str_len) {
  if (!buf || !buf_len || !str || !str_len) {
    return -1;
  }
...
int emv_format_n_get_string(const uint8_t *buf, size_t buf_len, char *str,
                            size_t str_len) {
  int r;
  uint32_t value;
...
int emv_amount_get_string(const uint8_t *buf, size_t buf_len, char *str,
                          size_t str_len) {
  int r;
  uint32_t value;
...
int emv_date_get_string(const uint8_t *buf, size_t buf_len, char *str,
                        size_t str_len) {
  char date_str[7];
  size_t offset = 0;
...
int emv_time_get_string(const uint8_t *buf, size_t buf_len, char *str,
                        size_t str_len) {
  char time_str[7];
  size_t offset = 0;
...
int emv_transaction_type_get_string(uint8_t txn_type, char *str,
                                    size_t str_len) {
  if (!str || !str_len) {
    // Caller didn't want the value string
    return 0;
...
int emv_mcc_get_string(const uint8_t *mcc, size_t mcc_len, char *str,
                       size_t str_len) {
  int r;
...
int emv_term_type_get_string_list(uint8_t term_type, char *str,
                                  size_t str_len) {
  struct str_itr_t itr;
  ...
  case EMV_TERM_TYPE_ENV_ATTENDED_OFFLINE_WITH_ONLINE:
    emv_str_list_add(&itr,
                     "Environment: Attended, offline with online capability");
    break;
  ...
  case EMV_TERM_TYPE_ENV_UNATTENDED_OFFLINE_WITH_ONLINE:
    emv_str_list_add(&itr,
                     "Environment: Unattended, offline with online capability");
    break;
...
int emv_term_caps_get_string_list(const uint8_t *term_caps,
                                  size_t term_caps_len, char *str,
                                  size_t str_len) {
  struct str_itr_t itr;
  ...
  if ((term_caps[1] & EMV_TERM_CAPS_CVM_PLAINTEXT_PIN_OFFLINE)) {
    emv_str_list_add(&itr,
                     "CVM Capability: Plaintext PIN for ICC verification");
  }
  if ((term_caps[1] & EMV_TERM_CAPS_CVM_ENCIPHERED_PIN_ONLINE)) {
    emv_str_list_add(&itr,
                     "CVM Capability: Enciphered PIN for online verification");
  }
  ...
  if ((term_caps[1] & EMV_TERM_CAPS_CVM_ENCIPHERED_PIN_OFFLINE_RSA)) {
    emv_str_list_add(
        &itr,
        "CVM Capability: Enciphered PIN for offline verification (RSA ODE)");
  }
  ...
  if ((term_caps[1] & EMV_TERM_CAPS_CVM_ENCIPHERED_PIN_OFFLINE_ECC)) {
    emv_str_list_add(
        &itr,
        "CVM Capability: Enciphered PIN for offline verification (ECC ODE)");
  }
  ...
  if ((term_caps[2] & EMV_TERM_CAPS_SECURITY_SDA)) {
    emv_str_list_add(&itr,
                     "Security Capability: Static Data Authentication (SDA)");
  }
  if ((term_caps[2] & EMV_TERM_CAPS_SECURITY_DDA)) {
    emv_str_list_add(&itr,
                     "Security Capability: Dynamic Data Authentication (DDA)");
  }
  ...
  if ((term_caps[2] & EMV_TERM_CAPS_SECURITY_CDA)) {
    emv_str_list_add(&itr, "Security Capability: Combined DDA/Application "
                           "Cryptogram Generation (CDA)");
  }
  if ((term_caps[2] & EMV_TERM_CAPS_SECURITY_XDA)) {
    emv_str_list_add(&itr,
                     "Security Capability: Extended Data Authentication (XDA)");
  }
...
int emv_pos_entry_mode_get_string(uint8_t pos_entry_mode, char *str,
                                  size_t str_len) {
  if (!str || !str_len) {
    // Caller didn't want the value string
    return 0;
  ...
  case EMV_POS_ENTRY_MODE_MAG_WITH_CVV:
    strncpy(str, "Magnetic stripe as read from track 2. CVV can be checked.",
            str_len);
    str[str_len - 1] = 0;
    return 0;
  ...
  case EMV_POS_ENTRY_MODE_ICC_WITHOUT_CVV:
    strncpy(str, "Integrated circuit card (ICC). CVV may not be checked.",
            str_len);
    str[str_len - 1] = 0;
    return 0;
...
int emv_addl_term_caps_get_string_list(const uint8_t *addl_term_caps,
                                       size_t addl_term_caps_len, char *str,
                                       size_t str_len) {
  struct str_itr_t itr;
  ...
  if ((addl_term_caps[2] &
       EMV_ADDL_TERM_CAPS_INPUT_ALPHABETIC_AND_SPECIAL_KEYS)) {
    emv_str_list_add(&itr, "Terminal Data Input Capability: Alphabetic and "
  ...
  if (!(addl_term_caps[3] & EMV_ADDL_TERM_CAPS_OUTPUT_PRINT_OR_DISPLAY)) {
    emv_str_list_add(&itr, "Terminal Data Input Capability: No print, "
                           "electronic or display output");
  }
  if ((addl_term_caps[3] & EMV_ADDL_TERM_CAPS_OUTPUT_PRINT_ATTENDANT)) {
    emv_str_list_add(
        &itr,
        "Terminal Data Output Capability: Print or electronic, attendant");
  }
  if ((addl_term_caps[3] & EMV_ADDL_TERM_CAPS_OUTPUT_PRINT_CARDHOLDER)) {
    emv_str_list_add(
        &itr,
        "Terminal Data Output Capability: Print or electronic, cardholder");
  }
  if ((addl_term_caps[3] & EMV_ADDL_TERM_CAPS_OUTPUT_DISPLAY_ATTENDANT)) {
    emv_str_list_add(&itr,
                     "Terminal Data Output Capability: Display, attendant");
  }
  if ((addl_term_caps[3] & EMV_ADDL_TERM_CAPS_OUTPUT_DISPLAY_CARDHOLDER)) {
    emv_str_list_add(&itr,
                     "Terminal Data Output Capability: Display cardholder");
  }
...
int emv_aid_get_string(const uint8_t *aid, size_t aid_len, char *str,
                       size_t str_len) {
  int r;
  ...
  switch (info.product) {
  // Visa
  case EMV_CARD_PRODUCT_VISA_CREDIT_DEBIT:
    info_str = "Visa Credit/Debit";
    break;
  case EMV_CARD_PRODUCT_VISA_ELECTRON:
    info_str = "Visa Electron";
    break;
  case EMV_CARD_PRODUCT_VISA_VPAY:
    info_str = "V Pay";
    break;
  case EMV_CARD_PRODUCT_VISA_PLUS:
    info_str = "Visa Plus";
    break;
  case EMV_CARD_PRODUCT_VISA_USA_DEBIT:
    info_str = "Visa USA Debit";
    break;

  // Mastercard
  case EMV_CARD_PRODUCT_MASTERCARD_CREDIT_DEBIT:
    info_str = "Mastercard Credit/Debit";
    break;
  case EMV_CARD_PRODUCT_MASTERCARD_MAESTRO:
    info_str = "Maestro";
    break;
  case EMV_CARD_PRODUCT_MASTERCARD_CIRRUS:
    info_str = "Mastercard Cirrus";
    break;
  case EMV_CARD_PRODUCT_MASTERCARD_USA_DEBIT:
    info_str = "Mastercard USA Debit";
    break;
  case EMV_CARD_PRODUCT_MASTERCARD_MAESTRO_UK:
    info_str = "Maestro UK";
    break;
  case EMV_CARD_PRODUCT_MASTERCARD_TEST:
    info_str = "Mastercard Test Card";
    break;

  // American Express
  case EMV_CARD_PRODUCT_AMEX_CREDIT_DEBIT:
    info_str = "American Express Credit/Debit";
    break;
  case EMV_CARD_PRODUCT_AMEX_CHINA_CREDIT_DEBIT:
    info_str = "American Express (China Credit/Debit)";
    break;

  // Discover
  case EMV_CARD_PRODUCT_DISCOVER_CARD:
    info_str = "Discover Card";
    break;
  case EMV_CARD_PRODUCT_DISCOVER_USA_DEBIT:
    info_str = "Discover USA Debit";
    break;
  case EMV_CARD_PRODUCT_DISCOVER_ZIP:
    info_str = "Discover ZIP";
    break;

  // Cartes Bancaires (CB)
  case EMV_CARD_PRODUCT_CB_CREDIT_DEBIT:
    info_str = "Cartes Bancaires (CB) Credit/Debit";
    break;
  case EMV_CARD_PRODUCT_CB_DEBIT:
    info_str = "Cartes Bancaires (CB) Debit";
    break;

  // Dankort
  case EMV_CARD_PRODUCT_DANKORT_VISADANKORT:
    info_str = "Visa/Dankort";
    break;
  case EMV_CARD_PRODUCT_DANKORT_JSPEEDY:
    info_str = "Dankort (J/Speedy)";
    break;

  // UnionPay
  case EMV_CARD_PRODUCT_UNIONPAY_DEBIT:
    info_str = "UnionPay Debit";
    break;
  case EMV_CARD_PRODUCT_UNIONPAY_CREDIT:
    info_str = "UnionPay Credit";
    break;
  case EMV_CARD_PRODUCT_UNIONPAY_QUASI_CREDIT:
    info_str = "UnionPay Quasi-credit";
    break;
  case EMV_CARD_PRODUCT_UNIONPAY_ELECTRONIC_CASH:
    info_str = "UnionPay Electronic Cash";
    break;
  case EMV_CARD_PRODUCT_UNIONPAY_USA_DEBIT:
    info_str = "UnionPay USA Debit";
    break;

  // GIM-UEMOA
  case EMV_CARD_PRODUCT_GIMUEMOA_STANDARD:
    info_str = "GIM-UEMOA Standard";
    break;
  case EMV_CARD_PRODUCT_GIMUEMOA_PREPAID_ONLINE:
    info_str = "GIM-UEMOA Prepaye Online";
    break;
  case EMV_CARD_PRODUCT_GIMUEMOA_CLASSIC:
    info_str = "GIM-UEMOA Classic";
    break;
  case EMV_CARD_PRODUCT_GIMUEMOA_PREPAID_OFFLINE:
    info_str = "GIM-UEMOA Prepaye Possibile Offline";
    break;
  case EMV_CARD_PRODUCT_GIMUEMOA_RETRAIT:
    info_str = "GIM-UEMOA Retrait";
    break;
  case EMV_CARD_PRODUCT_GIMUEMOA_ELECTRONIC_WALLET:
    info_str = "GIM-UEMOA Porte Monnaie Electronique";
    break;

  // Deutsche Kreditwirtschaft
  case EMV_CARD_PRODUCT_DK_GIROCARD:
    info_str = "Deutsche Kreditwirtschaft (DK) Girocard";
    break;

  // eftpos (Australia)
  case EMV_CARD_PRODUCT_EFTPOS_SAVINGS:
    info_str = "eftpos (Australia) savings";
    break;
  case EMV_CARD_PRODUCT_EFTPOS_CHEQUE:
    info_str = "eftpos (Australia) cheque";
    break;

  // Mir
  case EMV_CARD_PRODUCT_MIR_CREDIT:
    info_str = "Mir Credit";
    break;
  case EMV_CARD_PRODUCT_MIR_DEBIT:
    info_str = "Mir Debit";
    break;
  ...
  if (!info_str) {
    ...
    case EMV_CARD_SCHEME_AMEX:
      info_str = "American Express";
      break;
    case EMV_CARD_SCHEME_DISCOVER:
      info_str = "Discover";
      break;
    case EMV_CARD_SCHEME_CB:
      info_str = "Cartes Bancaires (CB)";
      break;
    case EMV_CARD_SCHEME_JCB:
      info_str = "JCB";
      break;
    case EMV_CARD_SCHEME_DANKORT:
      info_str = "Dankort";
      break;
    case EMV_CARD_SCHEME_UNIONPAY:
      info_str = "UnionPay";
      break;
    case EMV_CARD_SCHEME_GIMUEMOA:
      info_str = "GIM-UEMOA";
      break;
    case EMV_CARD_SCHEME_DK:
      info_str = "Deutsche Kreditwirtschaft (DK)";
      break;
    case EMV_CARD_SCHEME_VERVE:
      info_str = "Verve";
      break;
    case EMV_CARD_SCHEME_EFTPOS:
      info_str = "eftpos (Australia)";
      break;
    case EMV_CARD_SCHEME_RUPAY:
      info_str = "RuPay";
      break;
    case EMV_CARD_SCHEME_MIR:
      info_str = "Mir";
      break;
    case EMV_CARD_SCHEME_MEEZA:
      info_str = "Meeza";
      break;
...
int emv_asrpd_get_string_list(const uint8_t *asrpd, size_t asrpd_len, char *str,
                              size_t str_len) {
  struct str_itr_t itr;
...
int emv_aip_get_string_list(const uint8_t *aip, size_t aip_len, char *str,
                            size_t str_len) {
  struct str_itr_t itr;
...
int emv_afl_get_string_list(const uint8_t *afl, size_t afl_len, char *str,
                            size_t str_len) {
  int r;
    ...
    if (afl_entry.first_record == afl_entry.last_record) {
      if (afl_entry.oda_record_count) {
        emv_str_list_add(
            ...
            afl_entry.sfi, afl_entry.first_record, afl_entry.oda_record_count);
      } else {
        emv_str_list_add(&str_itr, "SFI %u, record %u", afl_entry.sfi,
                         afl_entry.first_record);
      }
    } else {
      if (afl_entry.oda_record_count) {
        emv_str_list_add(&str_itr,
                         "SFI %u, record %u to %u, %u record%s used for "
                         "offline data authentication",
                         afl_entry.sfi, afl_entry.first_record,
                         afl_entry.last_record, afl_entry.oda_record_count,
                         afl_entry.oda_record_count > 1 ? "s" : "");
      } else {
        emv_str_list_add(&str_itr, "SFI %u, record %u to %u", afl_entry.sfi,
                         afl_entry.first_record, afl_entry.last_record);
      }
    }
  }
...
int emv_app_usage_control_get_string_list(const uint8_t *auc, size_t auc_len,
                                          char *str, size_t str_len) {
  struct str_itr_t itr;
...
int emv_track2_equivalent_data_get_string(const uint8_t *track2,
                                          size_t track2_len, char *str,
                                          size_t str_len) {
...
int emv_app_reference_currency_get_string_list(const uint8_t *arc,
                                               size_t arc_len, char *str,
                                               size_t str_len) {
  int r;
  struct str_itr_t str_itr;
  ...
  for (size_t i = 0; i < arc_len; i += 2) {
    ...
    r = emv_currency_numeric_code_get_string(arc + i, 2, currency_str,
                                             sizeof(currency_str));
    if (r) {
      return r;
    }
...
int emv_language_preference_get_string_list(const uint8_t *lp, size_t lp_len,
                                            char *str, size_t str_len) {
  int r;
  struct str_itr_t str_itr;
  ...
  for (size_t i = 0; i < lp_len; i += 2) {
    ...
    r = emv_language_alpha2_code_get_string(lp + i, 2, language_str,
                                            sizeof(language_str));
    if (r) {
      return r;
    }
...
int emv_account_type_get_string(uint8_t account_type, char *str,
                                size_t str_len) {
  if (!str || !str_len) {
    // Caller didn't want the value string
    return 0;
...
emv_cvm_cond_code_get_string(uint8_t cvm_cond_code,
  ...
  case EMV_CV_RULE_COND_NOT_CASH_OR_CASHBACK:
    cond_str = "If not unattended cash and not manual cash and not purchase "
               "with cashback";
    break;
...
int emv_cvm_list_get_string_list(const uint8_t *cvmlist, size_t cvmlist_len,
                                 char *str, size_t str_len) {
  int r;
  ...
  r = emv_cvmlist_itr_init(cvmlist, cvmlist_len, &cvmlist_amounts,
                           &cvmlist_itr);
  if (r) {
    return r;
  }
  ...
  while ((r = emv_cvmlist_itr_next(&cvmlist_itr, &cv_rule)) > 0) {
    ...
    r = emv_cvm_cond_code_get_string(cv_rule.cvm_cond, &cvmlist_amounts,
                                     cond_str, sizeof(cond_str));
    if (r) {
      return r;
    }
    ...
    emv_str_list_add(&str_itr, "%s; %s; %s", cond_str, cvm_str, proc_str);
  }

  return 0;
}

int emv_cvm_results_get_string_list(const uint8_t *cvmresults,
                                    size_t cvmresults_len, char *str,
                                    size_t str_len) {
  int r;
  struct str_itr_t itr;
  const char *cvm_str;
  ...
  // See EMV 4.4 Book 4, Annex A4, Table 33
  r = emv_cvm_cond_code_get_string(cvmresults[1], NULL, cond_str,
                                   sizeof(cond_str));
  if (r) {
    return r;
  }
...
int emv_tvr_get_string_list(const uint8_t *tvr, size_t tvr_len, char *str,
                            size_t str_len) {
  struct str_itr_t itr;
  ...
  if (tvr[0] & EMV_TVR_CDA_FAILED) {
    emv_str_list_add(
        &itr, "Combined DDA/Application Cryptogram Generation (CDA) failed");
  }
  ...
  if (tvr[2] & EMV_TVR_PIN_PAD_FAILED) {
    emv_str_list_add(
        &itr, "PIN entry required and PIN pad not present or not working");
  }
  if (tvr[2] & EMV_TVR_PIN_NOT_ENTERED) {
    emv_str_list_add(
        &itr, "PIN entry required, PIN pad present, but PIN was not entered");
  }
  ...
  if (tvr[2] & EMV_TVR_BIOMETRIC_CAPTURE_FAILED) {
    emv_str_list_add(
        &itr, "Biometric required but Biometric capture device not working");
  }
  if (tvr[2] & EMV_TVR_BIOMETRIC_SUBTYPE_BYPASSED) {
    emv_str_list_add(&itr, "Biometric required, Biometric capture device "
                           "present, but Biometric Subtype entry was bypassed");
  }
...
int emv_tsi_get_string_list(const uint8_t *tsi, size_t tsi_len, char *str,
                            size_t str_len) {
  struct str_itr_t itr;
...
int emv_cid_get_string_list(uint8_t cid, char *str, size_t str_len) {
  struct str_itr_t itr;
  ...
  case EMV_CID_APPLICATION_CRYPTOGRAM_TYPE_AAC:
    emv_str_list_add(&itr, "Application Cryptogram (AC) type: Application "
                           "Authentication Cryptogram (AAC)");
    break;

  case EMV_CID_APPLICATION_CRYPTOGRAM_TYPE_TC:
    emv_str_list_add(
        &itr, "Application Cryptogram (AC) type: Transaction Certificate (TC)");
    break;

  case EMV_CID_APPLICATION_CRYPTOGRAM_TYPE_ARQC:
    emv_str_list_add(&itr, "Application Cryptogram (AC) type: Authorisation "
  ...
  if (cid & EMV_CID_PAYMENT_SYSTEM_SPECIFIC_CRYPTOGRAM_MASK) {
    emv_str_list_add(&itr, "Payment System-specific cryptogram: 0x%02X",
...
static int emv_iad_ccd_append_string_list(const uint8_t *iad, size_t iad_len,
  ...
  if (!iad || iad_len != EMV_IAD_CCD_LEN || iad[0] != EMV_IAD_CCD_BYTE1 ||
  ...
  case EMV_IAD_CCD_CVR_BYTE1_2GAC_AAC:
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Second GENERATE AC returned AAC");
    break;

  case EMV_IAD_CCD_CVR_BYTE1_2GAC_TC:
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Second GENERATE AC returned TC");
    break;
  ...
  case EMV_IAD_CCD_CVR_BYTE1_2GAC_RFU:
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Second GENERATE AC RFU");
    break;
  }
  switch (cvr[0] & EMV_IAD_CCD_CVR_BYTE1_1GAC_MASK) {
  case EMV_IAD_CCD_CVR_BYTE1_1GAC_AAC:
    emv_str_list_add(
        itr, "Card Verification Results (CVR): First GENERATE AC returned AAC");
    break;

  case EMV_IAD_CCD_CVR_BYTE1_1GAC_TC:
    emv_str_list_add(
        itr, "Card Verification Results (CVR): First GENERATE AC returned TC");
    break;
  ...
  case EMV_IAD_CCD_CVR_BYTE1_1GAC_RFU:
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): First GENERATE AC RFU");
    break;
  }
  if (cvr[0] & EMV_IAD_CCD_CVR_BYTE1_CDA_PERFORMED) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Combined "
                     "DDA/Application Cryptogram Generation (CDA) Performed");
  }
  if (cvr[0] & EMV_IAD_CCD_CVR_BYTE1_OFFLINE_DDA_PERFORMED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline Dynamic "
                          "Data Authentication (DDA) Performed");
  }
  if (cvr[0] & EMV_IAD_CCD_CVR_BYTE1_ISSUER_AUTH_NOT_PERFORMED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Issuer Authentication Not Performed");
  }
  if (cvr[0] & EMV_IAD_CCD_CVR_BYTE1_ISSUER_AUTH_FAILED) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Issuer Authentication Failed");
  }
  ...
  emv_str_list_add(itr,
                   ...
                   (cvr[1] & EMV_IAD_CCD_CVR_BYTE2_PIN_TRY_COUNTER_MASK) >>
                       EMV_IAD_CCD_CVR_BYTE2_PIN_TRY_COUNTER_SHIFT);
  if (cvr[1] & EMV_IAD_CCD_CVR_BYTE2_OFFLINE_PIN_PERFORMED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Offline PIN Verification Performed");
  }
  if (cvr[1] & EMV_IAD_CCD_CVR_BYTE2_OFFLINE_PIN_NOT_SUCCESSFUL) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Offline PIN Verification "
             "Performed and PIN Not Successfully Verified");
  }
  if (cvr[1] & EMV_IAD_CCD_CVR_BYTE2_PIN_TRY_LIMIT_EXCEEDED) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): PIN Try Limit Exceeded");
  }
  if (cvr[1] & EMV_IAD_CCD_CVR_BYTE2_LAST_ONLINE_TXN_NOT_COMPLETED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Last Online "
  ...
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_L_OFFLINE_TXN_CNT_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Lower Offline "
                          "Transaction Count Limit Exceeded");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_U_OFFLINE_TXN_CNT_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Upper Offline "
                          "Transaction Count Limit Exceeded");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_L_OFFLINE_AMOUNT_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Lower Cumulative "
                          "Offline Amount Limit Exceeded");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_U_OFFLINE_AMOUNT_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Upper Cumulative "
                          "Offline Amount Limit Exceeded");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_ISSUER_DISCRETIONARY_BIT1) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Issuer-discretionary bit 1");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_ISSUER_DISCRETIONARY_BIT2) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Issuer-discretionary bit 2");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_ISSUER_DISCRETIONARY_BIT3) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Issuer-discretionary bit 3");
  }
  if (cvr[2] & EMV_IAD_CCD_CVR_BYTE3_ISSUER_DISCRETIONARY_BIT4) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Issuer-discretionary bit 4");
  }
  ...
  if (cvr[3] & EMV_IAD_CCD_CVR_BYTE4_SCRIPT_COUNT_MASK) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): %u Successfully Processed Issuer "
        "Script Commands Containing Secure Messaging",
        (cvr[3] & EMV_IAD_CCD_CVR_BYTE4_SCRIPT_COUNT_MASK) >>
            EMV_IAD_CCD_CVR_BYTE4_SCRIPT_COUNT_SHIFT);
  }
  if (cvr[3] & EMV_IAD_CCD_CVR_BYTE4_ISSUER_SCRIPT_PROCESSING_FAILED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Issuer Script Processing Failed");
  }
  if (cvr[3] & EMV_IAD_CCD_CVR_BYTE4_ODA_FAILED_ON_PREVIOUS_TXN) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline Data "
                          "Authentication Failed on Previous Transaction");
  }
  if (cvr[3] & EMV_IAD_CCD_CVR_BYTE4_GO_ONLINE_ON_NEXT_TXN) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Go Online on Next "
                          "Transaction Was Set");
  }
  if (cvr[3] & EMV_IAD_CCD_CVR_BYTE4_UNABLE_TO_GO_ONLINE) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Unable to go Online");
  }
...
static int emv_iad_mchip_append_string_list(const uint8_t *iad, size_t iad_len,
  ...
  // See M/Chip Requirements for Contact and Contactless, 15 March 2022,
  // Appendix B, Issuer Application Data, 9F10
  if (!iad ||
      (iad_len != EMV_IAD_MCHIP4_LEN && iad_len != EMV_IAD_MCHIPADV_LEN_20 &&
  ...
  if (cvr[0] & EMV_IAD_MCHIP_CVR_BYTE1_OFFLINE_ENCRYPTED_PIN_PERFORMED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline Encrypted "
                          "PIN Verification Performed");
  }
  if (cvr[0] & EMV_IAD_MCHIP_CVR_BYTE1_OFFLINE_PIN_SUCCESSFUL) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Offline PIN Verification Successful");
  }

  // Card Verification (CVR) Results byte 2
  if (cvr[1] & EMV_IAD_MCHIP_CVR_BYTE2_DDA) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Dynamic Data "
                          "Authentication (DDA) Returned");
  }
  if (cvr[1] & EMV_IAD_MCHIP_CVR_BYTE2_1GAC_CDA) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Combined DDA/Application "
  ...
  if (cvr[1] & EMV_IAD_MCHIP_CVR_BYTE2_ISSUER_AUTH_PERFORMED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Issuer Authentication Performed");
  }
  if (cvr[1] & EMV_IAD_MCHIP_CVR_BYTE2_CIAC_SKIPPED_ON_CAT3) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Card Issuer Action Codes (CIAC) "
  ...
  if (cvr[1] & EMV_IAD_MCHIP_CVR_BYTE2_OFFLINE_CHANGE_PIN_SUCCESSFUL) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline Change PIN "
                          "Result Successful");
  }
  ...
  if (cvr[2] & EMV_IAD_MCHIP_CVR_BYTE3_SCRIPT_COUNTER_MASK) {
    emv_str_list_add(itr,
                     ...
                     (cvr[2] & EMV_IAD_MCHIP_CVR_BYTE3_SCRIPT_COUNTER_MASK) >>
                         EMV_IAD_MCHIP_CVR_BYTE3_SCRIPT_COUNTER_SHIFT);
  }
  if (cvr[2] & EMV_IAD_MCHIP_CVR_BYTE3_PIN_TRY_COUNTER_MASK) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): PIN Try Counter is %u",
                     (cvr[2] & EMV_IAD_MCHIP_CVR_BYTE3_PIN_TRY_COUNTER_MASK) >>
                         EMV_IAD_MCHIP_CVR_BYTE3_PIN_TRY_COUNTER_SHIFT);
  }
  ...
  if (cvr[3] & EMV_IAD_MCHIP_CVR_BYTE4_OFFLINE_PIN_NOT_PERFORMED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline PIN "
                          "Verification Not Performed");
  }
  if (cvr[3] & EMV_IAD_MCHIP_CVR_BYTE4_OFFLINE_PIN_FAILED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Offline PIN Verification Failed");
  }
  ...
  if (cvr[3] & EMV_IAD_MCHIP_CVR_BYTE4_INTERNATIONAL_TXN) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): International Transaction");
  }
  ...
  if (iad_len == EMV_IAD_MCHIP4_LEN) {
    ...
    if (cvr[4] & EMV_IAD_MCHIP_CVR_BYTE5_L_CONSECUTIVE_LIMIT_EXCEEDED) {
      emv_str_list_add(itr, "Card Verification Results (CVR): Lower "
                            "Consecutive Offline Limit Exceeded");
    }
    if (cvr[4] & EMV_IAD_MCHIP_CVR_BYTE5_U_CONSECUTIVE_LIMIT_EXCEEDED) {
      emv_str_list_add(itr, "Card Verification Results (CVR): Upper "
                            "Consecutive Offline Limit Exceeded");
    }
    ...
    if (cvr[4] & EMV_IAD_MCHIP_CVR_BYTE5_L_CONSECUTIVE_LIMIT_EXCEEDED) {
      emv_str_list_add(itr, "Card Verification Results (CVR): Lower "
                            "Consecutive Counter 1 Limit Exceeded");
    }
    if (cvr[4] & EMV_IAD_MCHIP_CVR_BYTE5_U_CONSECUTIVE_LIMIT_EXCEEDED) {
      emv_str_list_add(itr, "Card Verification Results (CVR): Upper "
                            "Consecutive Counter 1 Limit Exceeded");
  ...
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_L_CONSECUTIVE_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Lower Consecutive "
                          "Counter 2 Limit Exceeded");
  }
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_U_CONSECUTIVE_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Upper Consecutive "
                          "Counter 2 Limit Exceeded");
  }
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_L_CUMULATIVE_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Lower Cumulative "
                          "Accumulator 2 Limit Exceeded");
  }
  ...
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_MTA_LIMIT_EXCEEDED) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): MTA Limit Exceeded");
  }
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_NUM_OF_DAYS_LIMIT_EXCEEDED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Number Of Days "
                          "Offline Limit Exceeded");
  }
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_MATCH_ADDITIONAL_CHECK_TABLE) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Match Found In "
                          "Additional Check Table");
  }
  if (cvr[5] & EMV_IAD_MCHIP_CVR_BYTE6_NO_MATCH_ADDITIONAL_CHECK_TABLE) {
    emv_str_list_add(itr, "Card Verification Results (CVR): No Match Found In "
                          "Additional Check Table");
  }
...
static int emv_iad_vsdc_0_1_3_append_string_list(const uint8_t *iad,
  ...
  // See Visa Contactless Payment Specification (VCPS) Supplemental
  // Requirements, version 2.2, January 2016, Appendix M
  if (!iad || iad_len < EMV_IAD_VSDC_0_1_3_MIN_LEN ||
      iad[0] != EMV_IAD_VSDC_0_1_3_BYTE1 ||
      iad[3] != EMV_IAD_VSDC_0_1_3_CVR_LEN ||
      ((iad[2] & EMV_IAD_VSDC_CVN_FORMAT_MASK) >>
               EMV_IAD_VSDC_CVN_FORMAT_SHIFT !=
           0 &&
       (iad[2] & EMV_IAD_VSDC_CVN_FORMAT_MASK) >>
               EMV_IAD_VSDC_CVN_FORMAT_SHIFT !=
           1 &&
  ...
  emv_str_list_add(itr, "Cryptogram Version Number (CVN): %02X (CVN%02u)",
                   iad[2], iad[2]);
  ...
  if (cvr[1] & EMV_IAD_VSDC_CVR_BYTE2_ISSUER_AUTH_FAILED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Issuer "
  ...
  if (cvr[2] & EMV_IAD_VSDC_CVR_BYTE3_VELOCITY_COUNTERS_EXCEEDED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Exceeded velocity checking counters");
  }
  ...
  if (cvr[2] & EMV_IAD_VSDC_CVR_BYTE3_LAST_ONLINE_ISSUER_AUTH_FAILED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Issuer "
                          "Authentication failure on last online transaction");
  }
  if (cvr[2] & EMV_IAD_VSDC_CVR_BYTE3_ISSUER_AUTH_NOT_PERFORMED) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Issuer Authentication "
                     "not performed after online authorization");
  }
  if (cvr[2] & EMV_IAD_VSDC_CVR_BYTE3_APPLICATION_BLOCKED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Application "
                          "blocked by card because PIN Try Limit exceeded");
  }
  if (cvr[2] & EMV_IAD_VSDC_CVR_BYTE3_LAST_OFFLINE_SDA_FAILED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline static "
  ...
  if (cvr[3] & EMV_IAD_VSDC_CVR_BYTE4_SCRIPT_COUNTER_MASK) {
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Script Counter is %u",
                     (cvr[3] & EMV_IAD_VSDC_CVR_BYTE4_SCRIPT_COUNTER_MASK) >>
                         EMV_IAD_VSDC_CVR_BYTE4_SCRIPT_COUNTER_SHIFT);
  }
  ...
  if (cvr[3] & EMV_IAD_VSDC_CVR_BYTE4_OFFLINE_DDA_PERFORMED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Offline dynamic "
                          "data authentication performed");
  }
  if (cvr[3] & EMV_IAD_VSDC_CVR_BYTE4_PIN_VERIFICATION_NOT_RECEIVED) {
    emv_str_list_add(itr, "Card Verification Results (CVR): PIN verification "
                          "command not received for a PIN-Expecting card");
  }
...
static int emv_iad_vsdc_2_4_append_string_list(const uint8_t *iad,
  ...
  emv_str_list_add(itr, "Cryptogram Version Number (CVN): %02X (CVN'%02X')",
                   iad[1], iad[1]);
  ...
  case EMV_IAD_VSDC_CVR_BYTE1_CVM_ENTITY_VMPA:
    emv_str_list_add(itr, "Card Verification Results (CVR): Visa Mobile "
                          "Payment Application (VMPA)");
    break;
  ...
  case EMV_IAD_VSDC_CVR_BYTE1_CVM_ENTITY_SE_APP:
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Co-residing SE app");
    break;
  ...
  case EMV_IAD_VSDC_CVR_BYTE1_CVM_ENTITY_MOBILE_APP:
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Mobile Application");
    break;
  ...
  case EMV_IAD_VSDC_CVR_BYTE1_CVM_ENTITY_CLOUD:
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Verified in the cloud");
    break;
  ...
  case EMV_IAD_VSDC_CVR_BYTE1_CVM_TYPE_MOBILE_DEVICE_PATTERN:
    emv_str_list_add(itr,
                     "Card Verification Results (CVR): Mobile device pattern");
    break;
  ...
  if (cvr[1] & EMV_IAD_VSDC_CVR_BYTE2_CDCVM_PERFORMED) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): CDCVM successfully performed");
  }
  ...
  if (cvr[4] & EMV_IAD_VSDC_CVR_BYTE5_CD_NOT_DEBUG_MODE) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Consumer Device is "
                          "not in debug mode");
  }
  ...
  if (cvr[4] & EMV_IAD_VSDC_CVR_BYTE5_MOBILE_APP_NOT_HOOKED) {
    emv_str_list_add(
        itr,
        "Card Verification Results (CVR): Mobile Application is not hooked");
  }
  ...
  if (cvr[4] & EMV_IAD_VSDC_CVR_BYTE5_CD_HAS_CONNECTIVITY) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Consumer Device "
                          "has data connectivity");
  }
  if (cvr[4] & EMV_IAD_VSDC_CVR_BYTE5_CD_IS_GENUINE) {
    emv_str_list_add(
        itr, "Card Verification Results (CVR): Consumer Device is genuine");
  }
  ...
  if (cvr[4] & EMV_IAD_VSDC_CVR_BYTE5_EMV_SESSION_KEY) {
    emv_str_list_add(itr, "Card Verification Results (CVR): Secure Messaging "
                          "uses EMV Session key-based derivation");
  }

  return 0;
}

int emv_iad_get_string_list(const uint8_t *iad, size_t iad_len, char *str,
                            size_t str_len) {
  struct str_itr_t itr;
...
int emv_terminal_risk_management_data_get_string_list(const uint8_t *trmd,
                                                      size_t trmd_len,
                                                      char *str,
                                                      size_t str_len) {
  struct str_itr_t itr;
  ...
  if (trmd[0] & EMV_TRMD_BYTE1_ENCIPHERED_PIN_OFFLINE_CONTACTLESS) {
    emv_str_list_add(
        &itr, "Enciphered PIN verification performed by ICC (Contactless)");
  }
  ...
  if (trmd[0] & EMV_TRMD_BYTE1_PLAINTEXT_PIN_OFFLINE_CONTACTLESS) {
    emv_str_list_add(
        &itr, "Plaintext PIN verification performed by ICC (Contactless)");
  }
  ...
  if ((trmd[1] & EMV_TRMD_BYTE2_RFU) || (trmd[2] & EMV_TRMD_BYTE3_RFU) ||
      (trmd[3] & EMV_TRMD_BYTE4_RFU) || trmd[4] || trmd[5] || trmd[6] ||
      trmd[7]) {
    emv_str_list_add(&itr, "RFU");
  }
...
int emv_ttq_get_string_list(const uint8_t *ttq, size_t ttq_len, char *str,
                            size_t str_len) {
  struct str_itr_t itr;
  ...
  if (ttq[0] & EMV_TTQ_ODA_FOR_ONLINE_AUTH_SUPPORTED) {
    ...
    emv_str_list_add(
        &itr,
        "Offline Data Authentication for Online Authorizations not supported");
  }
...
int emv_ctq_get_string_list(const uint8_t *ctq, size_t ctq_len, char *str,
                            size_t str_len) {
  struct str_itr_t itr;
  ...
  if (ctq[0] & EMV_CTQ_ONLINE_IF_ODA_FAILED) {
    emv_str_list_add(&itr, "Go Online if Offline Data Authentication Fails and "
                           "Reader is online capable");
  }
  if (ctq[0] & EMV_CTQ_SWITCH_INTERFACE_IF_ODA_FAILED) {
    emv_str_list_add(&itr, "Switch Interface if Offline Data Authentication "
  ...
  if (ctq[0] & EMV_CTQ_ATM_NOT_VALID) {
    // See Visa Contactless Payment Specification (VCPS) Supplemental
    // Requirements, version 2.2, January 2016, Annex D
...
int emv_amex_cl_reader_caps_get_string(uint8_t cl_reader_caps, char *str,
                                       size_t str_len) {
  if (!str || !str_len) {
    // Caller didn't want the value string
    return 0;
...
static const struct {
  ...
  const char *str;
    ...
    // See M/Chip Requirements for Contact and Contactless, 28 September 2017,
    // Chapter 5, Third Party Data, Device Type
    ...
    {"01", "Mobile Phone or Smartphone with Mobile Network Operator (MNO) "
           "controlled removable secure element (SIM or UICC) personalized for "
           "use with a mobile phone or smartphone"},
    {"02", "Key Fob"},
    {"03", "Watch using a contactless chip or a fixed (non-removable) secure "
           "element not controlled by the MNO"},
    {"04", "Mobile Tag"},
    {"05", "Wristband"},
    {"06", "Mobile Phone Case or Sleeve"},
    {"07", "Mobile phone or smartphone with a fixed (non-removable) secure "
           "element controlled by the MNO, for example, code division multiple "
           "access (CDMA)"},
    {"08",
     "Removable secure element not controlled by the MNO, for example, memory "
     "card personalized for used with a mobile phone or smartphone"},
    {"09", "Mobile Phone or smartphone with a fixed (non-removable) secure "
           "element not controlled by the MNO"},
    {"10", "MNO controlled removable secure element (SIM or UICC) personalized "
           "for use with a tablet or e-book"},
    {"11", "Tablet or e-book with a fixed (non-removable) secure element "
           "controlled by the MNO"},
    {"12", "Removable secure element not controlled by the MNO, for example, "
           "memory card personalized for use with a tablet or e-book"},
    {"13", "Tablet or e-book with fixed (non-removable) secure element not "
           "controlled by the MNO"},
    {"14", "Mobile phone or smartphone with a payment application running in a "
           "host processor"},
    {"15",
     "Tablet or e-book with a payment application running in a host processor"},
    {"16", "Mobile phone or smartphone with a payment application running in "
           "the Trusted Execution Environment (TEE) of a host processor"},
    {"17", "Tablet or e-book with a payment application running in the TEE of "
           "a host processor"},
    {"18",
    ...
    {"23", "Watch/Wristband (Watch or wristband, including a fitness band, "
           "smart strap, disposable band, watch add-on, and security/ID band)"},
    {"24", "Sticker"},
    {"25", "PC (PC or laptop)"},
    ...
    {"33", "Media/Gaming Device (Media or gaming device, including a set top "
           "box, media player, and television)"},
};

static const char *
emv_mastercard_device_type_get_string(const char *device_type) {
  for (size_t i = 0; i < sizeof(emv_mastercard_device_type_map) /
                             sizeof(emv_mastercard_device_type_map[0]);
       ++i) {
    if (strncmp(emv_mastercard_device_type_map[i].device_type, device_type,
                2) == 0) {
...
int emv_mastercard_third_party_data_get_string_list(const uint8_t *tpd,
                                                    size_t tpd_len, char *str,
                                                    size_t str_len) {
  int r;
  ...
  // See M/Chip Requirements for Contact and Contactless, 15 March 2022, Chapter
  // 5, Third Party Data, Table 12
  tpd_ptr = tpd;

  // First two byte are the ISO 3166-1 numeric country code
  memset(country_str, 0, sizeof(country_str));
  r = emv_country_numeric_code_get_string(tpd_ptr, 2, country_str,
                                          sizeof(country_str));
  ...
  if ((unique_id & 0x8000) == 0) {
    ...
    if (device_type_str) {
      emv_str_list_add(&itr, "Device Type: %s - %s", device_type,
                       device_type_str);
    } else {
  ...
  if (unique_id == 0x0000 && (tpd_ptr[0] != 0x00 || remaining_len != 1)) {
  ...
  if (is_product_extension) {
    ...
    if (product_identifier == 0x0001) {
      ...
      // See M/Chip Requirements for Contact and Contactless, 15 March 2022,
      // Chapter 5, Third Party Data, Table 14
      ...
      if (tpd_ptr[2] == 0x02) {
        ...
        emv_str_list_add(
            &itr, "Product Restriction Code: Good for fuel and other products");
      } else if (tpd_ptr[2]) {
    ...
    if (product_identifier == 0x0002) {
      ...
      // See M/Chip Requirements for Contact and Contactless, 15 March 2022,
      // Chapter 5, Third Party Data, Table 15
      ...
      case 1:
        emv_str_list_add(&itr,
                         "Concession Code: 01 - Senior Citizen, potentially "
                         "eligible for senior citizen discounts");
        break;
      case 2:
        emv_str_list_add(&itr, "Concession Code: 02 - Student, potentially "
                               "eligible for student-based discounts");
        break;
      case 3:
        emv_str_list_add(&itr,
                         "Concession Code: 03 - Active military and veterans, "
                         "potentially eligible for service member discounts");
        break;
      case 4:
        emv_str_list_add(&itr,
                         "Concession Code: 04 - Low Income household, "
                         "potentially eligible for means-based discounts");
        break;
      case 5:
        emv_str_list_add(&itr, "Concession Code: 05 - Disability, eligible for "
                               "paratransit and other disability discounts");
        break;
      case 6:
        emv_str_list_add(&itr,
                         "Concession Code: 06 - Minor Child, potentially "
                         "enables free travel for under 16 and young kids");
        break;
      case 7:
        emv_str_list_add(&itr,
                         "Concession Code: 07 - Transit Staff, potentially "
                         "enables free travel for transit staff");
        break;
      case 8:
        emv_str_list_add(&itr,
                         "Concession Code: 08 - City/Government/Preferred "
                         "Employees, potentially enables discounted travel for "
                         "federal and preferred employees");
        break;
      default:
        emv_str_list_add(&itr, "Concession Code: RFU");
        break;
      }
...
int emv_visa_form_factor_indicator_get_string_list(const uint8_t *ffi,
                                                   size_t ffi_len, char *str,
                                                   size_t str_len) {
  struct str_itr_t itr;
  ...
  if ((ffi[0] & VISA_FFI_VERSION_MASK) != VISA_FFI_VERSION_NUMBER_1) {
    emv_str_list_add(&itr, "Form Factor Indicator (FFI): Version Number %u",
                     ffi[0] >> VISA_FFI_VERSION_SHIFT);

    // This implementation only supports version 1
    return 0;
  }
  switch (ffi[0] & VISA_FFI_FORM_FACTOR_MASK) {
  case VISA_FFI_FORM_FACTOR_CARD:
    emv_str_list_add(&itr, "Consumer Payment Device Form Factor: Card");
    break;
  case VISA_FFI_FORM_FACTOR_MINI_CARD:
    emv_str_list_add(&itr, "Consumer Payment Device Form Factor: Mini-card");
    break;
  case VISA_FFI_FORM_FACTOR_NON_CARD:
    emv_str_list_add(
        &itr, "Consumer Payment Device Form Factor: Non-card Form Factor");
    break;
  case VISA_FFI_FORM_FACTOR_CONSUMER_MOBILE_PHONE:
    emv_str_list_add(
        &itr, "Consumer Payment Device Form Factor: Consumer mobile phone");
    break;
  case VISA_FFI_FORM_FACTOR_WRIST_WORN_DEVICE:
    emv_str_list_add(&itr,
                     "Consumer Payment Device Form Factor: Wrist-worn device");
    break;
  default:
    emv_str_list_add(&itr, "Consumer Payment Device Form Factor: Unknown");
    break;
  }
  ...
  if (ffi[1] & VISA_FFI_FEATURE_BIOMETRIC) {
    emv_str_list_add(&itr, "Consumer Payment Device Features: Biometric "
                           "Cardholder Verification Capable");
  }
  ...
  case VISA_FFI_PAYMENT_TXN_TECHNOLOGY_CONTACTLESS:
    emv_str_list_add(&itr,
                     "Payment Transaction Technology: Proximity Contactless "
                     "interface using ISO 14443 (including NFC)");
    break;
  case VISA_FFI_PAYMENT_TXN_TECHNOLOGY_NOT_VCPS:
    emv_str_list_add(&itr, "Payment Transaction Technology: Not used in VCPS");
    break;
  default:
    emv_str_list_add(&itr, "Payment Transaction Technology: Unknown");
    break;
  }
...
int emv_amex_enh_cl_reader_caps_get_string_list(
    const uint8_t *enh_cl_reader_caps, size_t enh_cl_reader_caps_len, char *str,
    size_t str_len) {
  struct str_itr_t itr;
  ...
  if (enh_cl_reader_caps_len != 4) {
    // Amex Enhanced Contactless Reader Capabilities (field 9F6E) must be 4
    // bytes
    return 1;
  }
  ...
  if (enh_cl_reader_caps[0] &
      AMEX_ENH_CL_READER_CAPS_MAGSTRIPE_MODE_SUPPORTED) {
    emv_str_list_add(&itr, "Contactless Mag-Stripe Mode supported");
  }
  if (enh_cl_reader_caps[0] &
      AMEX_ENH_CL_READER_CAPS_FULL_ONLINE_MODE_SUPPORTED) {
    emv_str_list_add(&itr, "Contactless EMV full online mode supported (legacy "
                           "feature and no longer supported)");
  }
  if (enh_cl_reader_caps[0] &
      AMEX_ENH_CL_READER_CAPS_PARTIAL_ONLINE_MODE_SUPPORTED) {
  ...
  case AMEX_ENH_CL_READER_CAPS_KERNEL_VERSION_22_23:
    emv_str_list_add(&itr, "C-4 kernel version 2.2 - 2.3");
    break;
  case AMEX_ENH_CL_READER_CAPS_KERNEL_VERSION_24_26:
    emv_str_list_add(&itr, "C-4 kernel version 2.4 - 2.6");
    break;
  case AMEX_ENH_CL_READER_CAPS_KERNEL_VERSION_27:
    emv_str_list_add(&itr, "C-4 kernel version 2.7");
    break;
  default:
    emv_str_list_add(&itr, "C-4 kernel version unknown");
    break;
  }
...
static const struct {
  ...
  const char *desc;
    ...
    {"90", "Cutoff is in process (switch ending a day's business and starting "
...
static const char *emv_arc_get_desc(const char *arc) {
  for (size_t i = 0; i < sizeof(emv_auth_response_code_map) /
                             sizeof(emv_auth_response_code_map[0]);
       ++i) {
...
int emv_auth_response_code_get_string(const void *arc, size_t arc_len,
                                      char *str, size_t str_len) {
  int r;
  const char *arc_desc;
...
static int emv_csu_append_string_list(const uint8_t *csu, size_t csu_len,
  ...
  if (csu[0] & EMV_CSU_BYTE1_PROPRIETARY_AUTHENTICATION_DATA_INCLUDED) {
    emv_str_list_add(
        itr,
        "Card Status Update (CSU): Proprietary Authentication Data Included");
  }
  if (csu[0] & EMV_CSU_BYTE1_PIN_TRY_COUNTER_MASK) {
    emv_str_list_add(itr, "Card Status Update (CSU): PIN Try Counter = %u",
                     csu[0] & EMV_CSU_BYTE1_PIN_TRY_COUNTER_MASK);
  }
  ...
  if (csu[1] & EMV_CSU_BYTE2_ISSUER_APPROVES_ONLINE_TRANSACTION) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): Issuer Approves Online Transaction");
  }
  ...
  if (csu[1] & EMV_CSU_BYTE2_GO_ONLINE_ON_NEXT_TXN) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): Set Go Online on Next Transaction");
  }
  if (csu[1] & EMV_CSU_BYTE2_CREATED_BY_PROXY_FOR_ISSUER) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): CSU Created by Proxy for the Issuer");
  }
  if ((csu[1] & EMV_CSU_BYTE2_UPDATE_COUNTERS_MASK) ==
      EMV_CSU_BYTE2_UPDATE_COUNTERS_DO_NOT_UPDATE) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): Do Not Update Offline Counters");
  }
  if ((csu[1] & EMV_CSU_BYTE2_UPDATE_COUNTERS_MASK) ==
      EMV_CSU_BYTE2_UPDATE_COUNTERS_UPPER_OFFLINE_LIMIT) {
    emv_str_list_add(itr, "ard Status Update (CSU): Set Offline Counters to "
                          "Upper Offline Limits");
  }
  if ((csu[1] & EMV_CSU_BYTE2_UPDATE_COUNTERS_MASK) ==
      EMV_CSU_BYTE2_UPDATE_COUNTERS_RESET) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): Reset Offline Counters to Zero");
  }
  if ((csu[1] & EMV_CSU_BYTE2_UPDATE_COUNTERS_MASK) ==
      EMV_CSU_BYTE2_UPDATE_COUNTERS_ADD_TO_OFFLINE) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): Add Transaction to Offline Counter");
  }
  ...
  if (csu[3] & EMV_CSU_BYTE4_ISSUER_DISCRETIONARY) {
    emv_str_list_add(
        itr, "Card Status Update (CSU): Issuer Discretionary 0x%02X", csu[3]);
  }

  // Card Status Update (CSU) RFU bits
  // See EMV 4.4 Book 3, Annex C10
  if (csu[0] & EMV_CSU_BYTE1_RFU || csu[2] & EMV_CSU_BYTE3_RFU) {
...
int emv_issuer_auth_data_get_string_list(const uint8_t *iad, size_t iad_len,
                                         char *str, size_t str_len) {
  int r;
  struct str_itr_t itr;
  char arc_str[3];
  ...
  if (iad_len == 10) {
    ...
    if (r == 0) {
      ...
      // 8-byte ARPC followed by 2-character ARPC Response Code resembling
      // Authorisation Response Code See Visa Contactless Payment Specification
      // (VCPS) Supplemental Requirements, version 2.2, January 2016, Annex D
      emv_str_list_add(&itr,
                       "Authorisation Response Cryptogram (ARPC): "
                       "%02X%02X%02X%02X%02X%02X%02X%02X",
                       iad[0], iad[1], iad[2], iad[3], iad[4], iad[5], iad[6],
                       iad[7]);
      ...
      emv_str_list_add(&itr,
                       ...
                       iad[0], iad[1], iad[2], iad[3], iad[4], iad[5], iad[6],
                       iad[7]);
      emv_str_list_add(&itr, "M/Chip ARPC Response Code: %02X%02X", iad[8],
                       iad[9]);
  ...
  if ((iad[4] & EMV_CSU_BYTE1_RFU) == 0 && (iad[6] & EMV_CSU_BYTE3_RFU) == 0) {
    // Likely CCD, Visa CVN18 or Visa CVN'22'
    // 4-byte ARPC followed by 4-byte CSU and optional Proprietary
    // Authentication Data See EMV 4.4 Book 2, 8.2.2 See EMV 4.4 Book 3, Annex
    // C10 See Visa Contactless Payment Specification (VCPS) Supplemental
    // Requirements, version 2.2, January 2016, Annex D
    emv_str_list_add(
        &itr, "Authorisation Response Cryptogram (ARPC): %02X%02X%02X%02X",
        iad[0], iad[1], iad[2], iad[3]);
    emv_csu_append_string_list(iad + 4, 4, &itr);

    if (iad_len > 8) { // If extra data after CSU
      if ((iad[4] &
           EMV_CSU_BYTE1_PROPRIETARY_AUTHENTICATION_DATA_INCLUDED)) { // CSU
                                                                      // indicates
                                                                      // Proprietary
                                                                      // Authentication
                                                                      // Data
        emv_str_list_add(&itr, "Proprietary Authentication Data: %zu bytes",
                         iad_len - 8);
      } else if (iad_len == 10) {
...
static int emv_capdu_get_data_get_string(const uint8_t *c_apdu,
                                         size_t c_apdu_len, char *str,
                                         size_t str_len) {
  if ((c_apdu[0] & ISO7816_CLA_PROPRIETARY) == 0 || c_apdu[1] != 0xCA) {
    // Not GET DATA
    return -3;
  }
...
int emv_capdu_get_string(const uint8_t *c_apdu, size_t c_apdu_len, char *str,
                         size_t str_len) {
  ...
  if ((c_apdu[0] & ISO7816_CLA_PROPRIETARY) != 0) {
    ...
    case 0x1E:
      ins_str = "APPLICATION BLOCK";
      break;
    // See EMV 4.4 Book 3, 6.5.2.2
    case 0x18:
      ins_str = "APPLICATION UNBLOCK";
      break;
    ...
    // See EMV 4.4 Book 3, 6.5.7.2
    case 0xCA:
      return emv_capdu_get_data_get_string(c_apdu, c_apdu_len, str, str_len);
    // See EMV 4.4 Book 3, 6.5.8.2
    case 0xA8:
      ins_str = "GET PROCESSING OPTIONS";
      break;
    // See EMV 4.4 Book 3, 6.5.10.2
    case 0x24:
      ins_str = "PIN CHANGE/UNBLOCK";
      break;
    }
    ...
    s = iso7816_capdu_get_string(c_apdu, c_apdu_len, str, str_len);
    if (!s) {
