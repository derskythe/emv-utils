...
static struct emv_tlv_t *emv_tlv_alloc(unsigned int tag, unsigned int length,
                                       const uint8_t *value, uint8_t flags);
...
int emv_tlv_list_push(struct emv_tlv_list_t *list, unsigned int tag,
                      unsigned int length, const uint8_t *value,
                      uint8_t flags) {
  struct emv_tlv_t *tlv;
...
bool emv_tlv_list_has_duplicate(const struct emv_tlv_list_t *list) {
  ...
  for (const struct emv_tlv_t *tlv = list->front; tlv != NULL;
       tlv = tlv->next) {
    for (const struct emv_tlv_t *tlv2 = tlv->next; tlv2 != NULL;
         tlv2 = tlv2->next) {
      if (tlv->tag == tlv2->tag) {
        return true;
      }
