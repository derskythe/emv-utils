...
bool iso8859_is_supported(unsigned int codepage) {
  if (codepage < 1 || codepage > 15 || codepage == 12) {
...
int iso8859_to_utf8(unsigned int codepage, const uint8_t *iso8859,
  ...
  try {
    std::string iso8859_str(reinterpret_cast<const char *>(iso8859),
                            iso8859_len);
    std::stringstream charset_stream;
    charset_stream << "ISO-8859-" << codepage;
    std::string utf8_str =
        boost::locale::conv::to_utf<char>(iso8859_str, charset_stream.str());
    if (utf8_str.empty()) {
      return 2;
    }
