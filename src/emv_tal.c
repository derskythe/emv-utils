...
// Helper functions
static int emv_tal_parse_aef_record(struct emv_tlv_list_t *pse_tlv_list,
                                    const void *aef_record,
                                    size_t aef_record_len,
                                    const struct emv_tlv_list_t *supported_aids,
                                    struct emv_app_list_t *app_list);
static int emv_tal_read_sfi_records(struct emv_ttl_t *ttl,
                                    const struct emv_afl_entry_t *afl_entry,
                                    struct emv_tlv_list_t *list);

int emv_tal_read_pse(struct emv_ttl_t *ttl,
                     const struct emv_tlv_list_t *supported_aids,
                     struct emv_app_list_t *app_list) {
  int r;
  ...
  r = emv_ttl_select_by_df_name(ttl, EMV_PSE, strlen(EMV_PSE), fci, &fci_len,
                                &sw1sw2);
  if (r) {
  ...
  // Find Short File Identifier (SFI) for PSE directory Application Elementary
  // File (AEF) See EMV 4.4 Book 1, 11.3.4, table 8
  pse_sfi = emv_tlv_list_find_const(&pse_tlv_list, EMV_TAG_88_SFI);
  if (!pse_sfi) {
    emv_debug_trace_msg("emv_tlv_list_find_const() failed; pse_sfi=%p",
                        pse_sfi);
  ...
  for (uint8_t record_number = 1;; ++record_number) {
    ...
    // Read PSE AEF record
    // See EMV 4.4 Book 1, 12.2.3
    r = emv_ttl_read_record(ttl, pse_sfi->value[0], record_number, aef_record,
                            &aef_record_len, &sw1sw2);
    if (r) {
    ...
    r = emv_tal_parse_aef_record(&pse_tlv_list, aef_record, aef_record_len,
                                 supported_aids, app_list);
    if (r) {
...
static int emv_tal_parse_aef_record(struct emv_tlv_list_t *pse_tlv_list,
  ...
  if (aef_template_tlv.tag != EMV_TAG_70_DATA_TEMPLATE) {
    ...
    emv_debug_error("Unexpected data element 0x%02X in PSE AEF record",
                    tlv.tag);
    return EMV_TAL_RESULT_PSE_AEF_INVALID;
  }

  // NOTE: AEF template (field 70) may contain multiple Application Templates
  // (field 61) See EMV 4.4 Book 1, 10.1.4 See EMV 4.4 Book 1, 12.2.3

  r = iso8825_ber_itr_init(aef_template_tlv.value, aef_template_tlv.length,
                           &itr);
  if (r) {
  ...
  while ((r = iso8825_ber_itr_next(&itr, &tlv)) > 0) {
    ...
    if (tlv.tag != EMV_TAG_61_APPLICATION_TEMPLATE) {
      ...
      // See EMV 4.4 Book 1, 12.2.3
      emv_debug_error("Unexpected data element 0x%02X in AEF template",
                      tlv.tag);
      continue;
    }
...
int emv_tal_find_supported_apps(struct emv_ttl_t *ttl,
                                const struct emv_tlv_list_t *supported_aids,
                                struct emv_app_list_t *app_list) {
  int r;
  ...
  while (aid != NULL) {
    ...
    if (exact_match) {
      ...
      r = emv_ttl_select_by_df_name(ttl, aid->value, aid->length, fci, &fci_len,
                                    &sw1sw2);
      if (r) {
        emv_debug_trace_msg("emv_ttl_select_by_df_name() failed; r=%d", r);
      ...
      r = emv_ttl_select_by_df_name_next(ttl, aid->value, aid->length, fci,
                                         &fci_len, &sw1sw2);
      if (r) {
    ...
    if (aid->length == app->aid->length) {
      ...
      if (aid->flags == EMV_ASI_PARTIAL_MATCH) {
        ...
        // Partial match not allowed; ignore app and continue to next supported
        // AID See EMV 4.4 Book 1, 12.3.3, step 6
        emv_debug_info(
            "Application is not supported: partial match not allowed");
        emv_app_free(app);
        app = NULL;
...
int emv_tal_select_app(struct emv_ttl_t *ttl, const uint8_t *aid,
                       size_t aid_len, struct emv_app_t **selected_app) {
  int r;
  uint8_t fci[EMV_RAPDU_DATA_MAX];
  size_t fci_len = sizeof(fci);
  uint16_t sw1sw2;
  ...
  *selected_app = NULL;
  ...
  if (app->aid->length != aid_len || memcmp(app->aid->value, aid, aid_len)) {
...
int emv_tal_get_processing_options(struct emv_ttl_t *ttl, const void *data,
                                   size_t data_len, struct emv_tlv_list_t *list,
  ...
  r = emv_ttl_get_processing_options(ttl, data, data_len, gpo_response,
                                     &gpo_response_len, &sw1sw2);
  if (r) {
  ...
  if (gpo_tlv.tag == EMV_TAG_80_RESPONSE_MESSAGE_TEMPLATE_FORMAT_1) {
    ...
    if (gpo_tlv.length < 6 ||             // AIP and at least one AFL entry
      ...
      // Parse error; terminate session
      // See EMV 4.4 Book 3, 6.5.8.4
      emv_debug_error("Invalid GPO response format 1 length of %u",
                      gpo_tlv.length);
      return EMV_TAL_ERROR_GPO_PARSE_FAILED;
    }
    ...
    r = emv_tlv_list_push(&gpo_list, EMV_TAG_82_APPLICATION_INTERCHANGE_PROFILE,
                          2, gpo_tlv.value, 0);
    if (r) {
    ...
    r = emv_tlv_list_push(&gpo_list, EMV_TAG_94_APPLICATION_FILE_LOCATOR,
                          gpo_tlv.length - 2, gpo_tlv.value + 2, 0);
    if (r) {
      emv_debug_trace_msg("emv_tlv_list_push() failed; r=%d", r);
  ...
  tlv = emv_tlv_list_find_const(&gpo_list,
...
static int emv_tal_read_sfi_records(struct emv_ttl_t *ttl,
  ...
  for (uint8_t record_number = afl_entry->first_record;
       record_number <= afl_entry->last_record; ++record_number) {
    ...
    // READ RECORD
    // See EMV 4.4 Book 3, 10.2
    emv_debug_info("READ RECORD from SFI %u, record %u", afl_entry->sfi,
                   record_number);
    r = emv_ttl_read_record(ttl, afl_entry->sfi, record_number, record,
                            &record_len, &sw1sw2);
    if (r) {
      emv_debug_trace_msg("emv_ttl_read_record() failed; r=%d", r);
      // TTL failure; terminate session
    ...
    if (record[0] != EMV_TAG_70_DATA_TEMPLATE) {
      ...
      // See EMV 4.4 Book 3, 10.3 (page 98)
      emv_debug_error(
          "Offline data authentication not possible due to invalid record");
    ...
    if (afl_entry->sfi >= 1 && afl_entry->sfi <= 10) {
      ...
      if (record_template.tag != EMV_TAG_70_DATA_TEMPLATE) {
        ...
        // See EMV 4.4 Book 3, 6.5.11.4
        emv_debug_error("Invalid record template tag 0x%02X",
                        record_template.tag);
        return EMV_TAL_ERROR_READ_RECORD_INVALID;
      }
      if (record_template.length + 3 <
          record_len) { // 3 bytes for ID and 2-byte length
        // Record should contain a single record template without
        // additional data after it
        // See EMV 4.4 Book 3, 6.5.11.4
        emv_debug_error("Invalid record template length %u",
                        record_template.length);
        return EMV_TAL_ERROR_READ_RECORD_INVALID;
      }

      if (!oda_record_invalid) {
        emv_debug_info(
            "Record template content used for offline data authentication");
...
int emv_tal_read_afl_records(struct emv_ttl_t *ttl, const uint8_t *afl,
                             size_t afl_len, struct emv_tlv_list_t *list) {
  int r;
