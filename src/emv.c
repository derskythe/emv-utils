...
const char *emv_lib_version_string(void) { return EMV_UTILS_VERSION_STRING; }

const char *emv_error_get_string(enum emv_error_t error) {
  switch (error) {
  case EMV_ERROR_INTERNAL:
    return "Internal error";
  case EMV_ERROR_INVALID_PARAMETER:
...
const char *emv_outcome_get_string(enum emv_outcome_t outcome) {
  ...
  switch (outcome) {
  case EMV_OUTCOME_CARD_ERROR:
    return "Card error"; // Message 06
  case EMV_OUTCOME_CARD_BLOCKED:
    return "Card blocked"; // Not in EMV specification
  case EMV_OUTCOME_NOT_ACCEPTED:
    return "Not accepted"; // Message 0C
  case EMV_OUTCOME_TRY_AGAIN:
    return "Try again"; // Message 13
  case EMV_OUTCOME_GPO_NOT_ACCEPTED:
    return "Not accepted"; // Message 0C
  }
...
int emv_atr_parse(const void *atr, size_t atr_len) {
  ...
  if (atr_info.TA[1]) {                                      // TA1 is present
    if (atr_info.TA[2] &&                                    // TA2 is present
        ...
        (*atr_info.TA[1] < 0x11 ||
         *atr_info.TA[1] > 0x13) // TA1 must be in the range 0x11 to 0x13
    ) {
  ...
  if (TD2_protocol == ISO7816_PROTOCOL_T1) {
    ...
    if (atr_info.TB[3]) { // TB3 is present
      ...
      if (((*atr_info.TB[3] & ISO7816_ATR_TBi_BWI_MASK) >>
           ISO7816_ATR_TBi_BWI_SHIFT) > 4) {
...
int emv_build_candidate_list(struct emv_ttl_t *ttl,
  ...
  if (!ttl || !supported_aids || !app_list) {
    emv_debug_trace_msg("ttl=%p, supported_aids=%p, app_list=%p", ttl,
...
int emv_select_application(struct emv_ttl_t *ttl,
  ...
  if (!ttl || !app_list || !selected_app) {
    emv_debug_trace_msg("ttl=%p, app_list=%p, index=%u, selected_app=%p", ttl,
                        app_list, index, selected_app);
    emv_debug_error("Invalid parameter");
    return EMV_ERROR_INVALID_PARAMETER;
  }
  *selected_app = NULL;
  ...
  r = emv_tal_select_app(ttl, current_aid, current_aid_len, selected_app);
  if (r) {
...
int emv_initiate_application_processing(struct emv_ttl_t *ttl,
  ...
  if (!ttl || !selected_app || !source1) {
    emv_debug_trace_msg("ttl=%p, selected_app=%p, source1=%p, source2=%p", ttl,
                        selected_app, source1, source2);
    emv_debug_error("Invalid parameter");
    return EMV_ERROR_INVALID_PARAMETER;
  }
  ...
  if (pdol) {
    ...
    if (dol_data_len < 0) {
      emv_debug_trace_msg(
          "emv_dol_compute_data_length() failed; dol_data_len=%d",
          dol_data_len);
    ...
    r = emv_dol_build_data(pdol->value, pdol->length, source1, source2,
                           gpo_data + gpo_data_offset, &gpo_data_len);
    if (r) {
  ...
  r = emv_tal_get_processing_options(ttl, gpo_data, gpo_data_len, &gpo_output,
                                     NULL, NULL);
  if (r) {
...
int emv_read_application_data(struct emv_ttl_t *ttl,
  ...
  for (const struct emv_tlv_t *tlv = record_data.front; tlv != NULL;
       tlv = tlv->next) {
