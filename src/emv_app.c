...
// Helper functions
static int
emv_app_extract_display_name(struct emv_app_t *app,
...
struct emv_app_t *emv_app_create_from_pse(struct emv_tlv_list_t *pse_tlv_list,
  ...
  app->aid =
      emv_tlv_list_find_const(&app->tlv_list, EMV_TAG_4F_APPLICATION_DF_NAME);
  if (!app->aid) {
    // Invalid FCI
    goto error;
  }
...
emv_app_extract_display_name(struct emv_app_t *app,
  ...
  if (pse_tlv_list) {
    issuer_code_table_index = emv_tlv_list_find_const(
        pse_tlv_list, EMV_TAG_9F11_ISSUER_CODE_TABLE_INDEX);
  } else {
    issuer_code_table_index = emv_tlv_list_find_const(
  ...
  if (issuer_code_table) {
    ...
    tlv = emv_tlv_list_find_const(&app->tlv_list,
                                  EMV_TAG_9F12_APPLICATION_PREFERRED_NAME);
    if (tlv) {
      ...
      char app_preferred_name[16 +
                              1]; // Ensure enough space for NULL termination

      // Copy only non-control characters
      r = emv_format_ans_to_non_control_str(tlv->value, tlv->length,
                                            app_preferred_name,
                                            sizeof(app_preferred_name));
      if (r == 0) {
        // Convert ISO 8859 to UTF-8
        r = iso8859_to_utf8(issuer_code_table,
                            (const uint8_t *)app_preferred_name,
                            strlen(app_preferred_name), app->display_name,
  ...
  if (tlv) {
    ...
    r = emv_format_ans_to_alnum_space_str(
        tlv->value, tlv->length, app->display_name, sizeof(app->display_name));
    return 0;
  }
  ...
  if (app->aid) {
    return emv_format_b_to_str(app->aid->value, app->aid->length,
                               app->display_name, sizeof(app->display_name));
  }
...
static int emv_app_extract_priority_indicator(struct emv_app_t *app) {
  ...
  tlv = emv_tlv_list_find_const(&app->tlv_list,
                                EMV_TAG_87_APPLICATION_PRIORITY_INDICATOR);
  if (!tlv || tlv->length < 1) {
  ...
  app->confirmation_required =
      tlv->value[0] & EMV_APP_PRIORITY_INDICATOR_CONF_REQUIRED;
...
bool emv_app_is_supported(const struct emv_app_t *app,
  ...
  for (tlv = supported_aids->front; tlv != NULL; tlv = tlv->next) {
    if (tlv->flags == EMV_ASI_EXACT_MATCH && tlv->length == app->aid->length &&
...
struct emv_app_t *emv_app_list_remove_index(struct emv_app_list_t *list,
                                            unsigned int index) {
...
static int emv_app_list_insert(struct emv_app_list_t *list,
                               struct emv_app_t *pos, struct emv_app_t *app) {
  if (!emv_app_list_is_valid(list)) {
    return -1;
  }
  if (!pos) {
...
static int emv_app_list_push_front(struct emv_app_list_t *list,
                                   struct emv_app_t *app) {
  if (!emv_app_list_is_valid(list)) {
    return -1;
  }
...
int emv_app_list_sort_priority(struct emv_app_list_t *list) {
  ...
  while ((app = emv_app_list_pop(list))) {
    ...
    for (struct emv_app_t *cur = sorted_list.front; cur != NULL;
         cur = cur->next) {
      if (!cur->priority) {
        break;
      }
...
bool emv_app_list_selection_is_required(const struct emv_app_list_t *list) {
  ...
  for (const struct emv_app_t *app = list->front; app != NULL;
       app = app->next) {
